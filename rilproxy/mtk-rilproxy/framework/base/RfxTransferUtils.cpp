...
static void parcelToCallList(RIL_Token t, RIL_Errno e, int id, Parcel* p, int slotId) {
    ...
    if (p->dataAvail() > 0) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            pCallLists[i]->forwardedNumber = strdupReadString(p);
        }
    }
...
static void parcelToNetworkScanResult(RIL_Token t, RIL_Errno e, int id, Parcel* p, int slotId) {
    ...
    if (p->dataSize() == 2) {
        ...
        if (p->dataAvail() > 0) {
            ...
            num = entry.network_infos_length;
            ...
            for (int i = 0; i< num; i++) {
                ...
                switch (entry.network_infos[i].cellInfoType) {
                    case RIL_CELL_INFO_TYPE_GSM: {
                        ...
                        p->readInt32(&entry.network_infos[i].CellInfo.gsm.cellIdentityGsm.mnc_len);
                    ...
                    case RIL_CELL_INFO_TYPE_WCDMA: {
                        ...
                        p->readInt32(&entry.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.mnc_len);
                    ...
                    case RIL_CELL_INFO_TYPE_LTE: {
                        ...
                        p->readInt32(&entry.network_infos[i].CellInfo.lte.cellIdentityLte.mnc_len);
                    ...
                    case RIL_CELL_INFO_TYPE_TD_SCDMA: {
                        ...
                        p->readInt32(&entry.network_infos[i].CellInfo.tdscdma.cellIdentityTdscdma.mnc_len);
...
static void parcelToVoiceState(RIL_Token t, RIL_Errno e, int id, Parcel* p, int slotId) {
    ...
    if (p->dataAvail() > 0) {
        ...
        if (countStrings >= 15) {
            ...
            if (pStrings[16] != NULL) {
                int max_copied = sizeof(p_cur->rplmn) - 1;
                strncpy(p_cur->rplmn, pStrings[16], max_copied);
                p_cur->rplmn[max_copied] = '\0';
            } else {
                p_cur->rplmn[0] = '\0';
            }
        }
...
static void parcelToCarrierRestrictions(RIL_Token t, RIL_Errno e, int id, Parcel* p, int slotId) {
    ...
    if (p->dataAvail() > 0) {
        ...
        RFX_ASSERT(entry.allowed_carriers != NULL);
        ...
        RFX_ASSERT(entry.excluded_carriers != NULL);
...
static void parcelToDataState(RIL_Token t, RIL_Errno e, int id, Parcel* p, int slotId) {
    ...
    if (p->dataAvail() > 0) {
        ...
        if (countStrings >= 6) {
            ...
            if (pStrings[6] != NULL) {
                int max_copied = sizeof(p_cur->rplmn) - 1;
                strncpy(p_cur->rplmn, pStrings[6], max_copied);
                p_cur->rplmn[max_copied] = '\0';
            } else {
                p_cur->rplmn[0] = '\0';
            }
