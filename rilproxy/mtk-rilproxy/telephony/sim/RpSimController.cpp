...
#endif
static const char PROPERTY_SIM_POWER_STATE[4][30] = {
    "vendor.ril.sim.power.state",
    "vendor.ril.sim.power.state.2",
    "vendor.ril.sim.power.state.3",
    "vendor.ril.sim.power.state.4",
};
...
RpSimController::RpSimController() :
    ...
    mC2kSimCtrl(NULL),
    mTimerHandler(0),
    mUiccApplicationEnable(0),
    mCacheUiccEnableMsg(NULL) {
...
//        RIL_REQUEST_SIM_OPEN_CHANNEL_WITH_SW,
    ...
    const int request_id_common_list[] = {
    ...
    const int urc_id_common_list[] = {
        RIL_UNSOL_ICCID_CHANGED,
    ...
    registerToHandleRequest(request_id_common_list, (sizeof(request_id_common_list)/sizeof(int)));
    registerToHandleUrc(urc_id_common_list, (sizeof(urc_id_common_list)/sizeof(int)));
...
bool RpSimController::onPreviewMessage(const sp<RfxMessage>& message) {
    ...
    if (message->getType() == REQUEST && message->getId() == RIL_REQUEST_GET_SIM_STATUS) {
        ...
        if (mC2kSimCtrl->onPreviewCheckRequestGetImsi(message)) {
            ...
            message->getId() == RIL_REQUEST_ENABLE_UICC_APPLICATIONS) {
...
//    case RIL_REQUEST_SIM_OPEN_CHANNEL_WITH_SW:
    ...
    case RIL_REQUEST_ENTER_SIM_DEPERSONALIZATION: {
            handleEnterSimDepersonalizationReq(message);
            break;
        }
    case RIL_REQUEST_ENABLE_UICC_APPLICATIONS: {
            handleEnableUiccApplicationsReq(message);
            break;
        }
    case RIL_REQUEST_ARE_UICC_APPLICATIONS_ENABLED: {
            handleAreUiccApplicationsEnabledReq(message);
            break;
        }
    default:
        return false;
...
//    case RIL_REQUEST_SIM_OPEN_CHANNEL_WITH_SW:
    ...
    case RIL_REQUEST_ENTER_SIM_DEPERSONALIZATION: {
            handleEnterSimDepersonalizationRsp(message);
            break;
        }
    case RIL_REQUEST_ENABLE_UICC_APPLICATIONS: {
            handleEnableUiccApplicationsRsp(message);
            break;
        }
    case RIL_REQUEST_ARE_UICC_APPLICATIONS_ENABLED: {
            handleAreUiccApplicationsEnabledRsp(message);
            break;
        }
    default:
        return false;
...
RIL_CardStatus_v8* RpSimController::decodeGetSimStatusResp() {
    ...
    if (cardStatus->card_state == RIL_CARDSTATE_ABSENT) {
        ...
        if ((nws_mode == NWS_MODE_CDMALTE) && (nwRatController->getAppFamilyType() ==
            ...
            // C2K
            if (cardStatus->cdma_subscription_app_index >= 0) {
                appState =
...
void RpSimController::handleGetSimStatusReq(const sp<RfxMessage>& message) {
    ...
    if (mCurrentGetSimStatusReq == 0) {
        property_get(PROPERTY_UICC_APPLICATIONS_ENABLE_STATE[getSlotId()], tmp, "");
        int state = -1;
        if (strlen(tmp) > 0) {
            state = atoi(tmp);
            RLOGE("[RpSimController] handleGetSimStatusReq enable state is %d", state);
            if (state == UICC_APPLICATIONS_ENABLE_STATE_DISABLED ||
                    state == UICC_APPLICATIONS_ENABLE_STATE_DISABLING) {
                RLOGD("[RpSimController] handleGetSimStatusReq, UICC APPLICATIONS DISABLED");
                ...
                p->writeInt32(RIL_CARDSTATE_PRESENT);
                ...
                writeStringToParcel(p, iccid.string());
                writeStringToParcel(p, NULL);
        ...
        property_get(PROPERTY_SIM_POWER_STATE[getSlotId()], tmp, "");
        state = -1;
        if (strlen(tmp) > 0) {
            state = atoi(tmp);
            RLOGE("[RpSimController] handleGetSimStatusReq sim power state is %d", state);
            if (state == SIM_POWER_STATE_V16_SIM_OFF ||
                    state == SIM_POWER_STATE_PASS_THROUGH) {
                String8 iccid = getStatusManager()->getString8Value(RFX_STATUS_KEY_SIM_ICCID);
        ...
        if ((supportCardType(ICC_CSIM) || supportCardType(ICC_RUIM)) && (getSlotId() ==
            ...
            getStatusManager()->setString8Value(RFX_STATUS_KEY_SIM_ICCID, String8(noSim));
            sendIccidUrc(noSim);
            ...
            if (mGetSimStatusErr == RIL_E_SUCCESS) {
                ...
                if (mGetSimStatusErr != RIL_E_SUCCESS) {
                    ...
                    for (int i = 0; i < cardStatus->num_applications; i++) {
                        ...
                        //RFX_ASSERT(pApp->aid_ptr != NULL); allow null
                        writeStringToParcel(p, (const char*)(pApp->aid_ptr));
                        //RFX_ASSERT(pApp->app_label_ptr != NULL); allow null
                    ...
                    //RFX_ASSERT(cardStatus->atr != NULL); allow null
                    writeStringToParcel(p, (const char*)cardStatus->atr);
                    //RFX_ASSERT(cardStatus->iccId != NULL); allow null
                    writeStringToParcel(p, (const char*)cardStatus->iccId);
                    // Parameters add from radio hidl v1.4.
                    //RFX_ASSERT(cardStatus->eid != NULL); allow null
...
void RpSimController::handleEnterSimDepersonalizationReq(
        const sp<RfxMessage>& message) {
    RLOGD("[RpSimController] handleEnterSimDepersonalizationReq (slot %d)", getSlotId());

    RILD_RadioTechnology_Group dest = choiceDestViaCurrCardType();

    if (dest == RADIO_TECH_GROUP_GSM && mGsmSimCtrl != NULL) {
        mGsmSimCtrl->handleEnterSimDepersonalizationReq(message);
    } else {
        RLOGE("[RpSimController] handleEnterSimDepersonalizationReq, can't dispatch the request!");
    }
}

void RpSimController::handleEnterSimDepersonalizationRsp(
        const sp<RfxMessage>& message) {
    RLOGD("[RpSimController] handleEnterSimDepersonalizationRsp (slot %d)", getSlotId());

    RILD_RadioTechnology_Group src = message->getSource();

    if (src == RADIO_TECH_GROUP_GSM && mGsmSimCtrl != NULL) {
        mGsmSimCtrl->handleEnterSimDepersonalizationRsp(message);
    } else if (src == RADIO_TECH_GROUP_C2K && mC2kSimCtrl != NULL) {
        mC2kSimCtrl->handleEnterSimDepersonalizationRsp(message);
    } else {
        RLOGE("[RpSimController] handleEnterSimDepersonalizationRsp, can't dispatch the response!");
    }
}

void RpSimController::handleEnableUiccApplicationsReq(
        const sp<RfxMessage>& message) {
    RLOGD("[RpSimController] handleEnableUiccApplicationsReq (slot %d)", getSlotId());
    int length = 0;
    int enable = 0;
    Parcel* p = message->getParcel();
    size_t dataPos = p->dataPosition();
    ...
    if (!isEnableUiccApplicationsSupported()
        ...
        RLOGE("[RpSimController] Not support or under processing, slot:%d, msgslot:%d, enable:%d",
                getSlotId(), message->getSlotId(), enable);
    ...
    if (enable == 0) {
        property_set(PROPERTY_UICC_APPLICATIONS_ENABLE_STATE[getSlotId()], String8::format("%d",
...
void RpSimController::handleEnableUiccApplicationsRsp(
        const sp<RfxMessage>& message) {
    RIL_Errno result = message->getError();

    RLOGD("[RpSimController] enableUiccRsp result: %d, state: %d, responseState %d, slot: %d",
            result,
            getStatusManager()->getIntValue(RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_STATE),
            getStatusManager()->getIntValue(
            RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_RESPONSE_STATE), message->getSlotId());
    ...
    if (!isEnableUiccApplicationsProcessing()) {
        ...
        RLOGE("[RpSimController] enableUiccRsp error or normal case, slot:%d, id:%d",
                message->getSlotId(), message->getId());
        if (result != RIL_E_SUCCESS) {
            result = RIL_E_SUCCESS;
        }
        sp<RfxMessage> response = RfxMessage::obtainResponse(result, message, true);
        responseToRilj(response);
        return;
    }

    mCacheUiccEnableMsg = RfxMessage::obtainResponse(result, message, true);
}

void RpSimController::handleUiccApplicationsEnablementChanged(const sp<RfxMessage>& message) {
    int length = 0;
    int enable = 0;
    Parcel* p = message->getParcel();
    size_t dataPos = p->dataPosition();
    ...
    RLOGD("[RpSimController] uiccEnablementChanged enable: %d, state: %d, responseState",
            enable,
            getStatusManager()->getIntValue(RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_STATE),
            getStatusManager()->getIntValue(
            RFX_STATUS_KEY_AOSP_UICC_APPLICATIONS_ENABLE_RESPONSE_STATE));
    ...
    if ((mCacheUiccEnableMsg != NULL)) {
        // Request's response trigger
        RLOGD("[RpSimController] uiccEnablementChanged Normal case ..., %d, %d, %d, %d, %d",
                mCacheUiccEnableMsg->getSlotId(), message->getSlotId(), enable,
                mCacheUiccEnableMsg->getError(), mCacheUiccEnableMsg->getId());
        sp<RfxMessage> response = RfxMessage::obtainResponse(result, mCacheUiccEnableMsg, true);
        responseToRilj(response);
        mCacheUiccEnableMsg = NULL;
        responseToRilj(message);
        return;
...
bool RpSimController::isEnableUiccApplicationsSupported() {
    int supported = 0;
    String8 supportedProperty(PROPERTY_UICC_APPLICATIONS_ENABLE_SUPPORT);

    char tmp[PROPERTY_VALUE_MAX] = {0};
    property_get(supportedProperty.string(), tmp, "0");
    supported = atoi(tmp);
    return (supported == 1) ? true : false;
}

bool RpSimController::isEnableUiccApplicationsProcessing() {
...
void RpSimController::handleAreUiccApplicationsEnabledReq(
        const sp<RfxMessage>& message) {
    RLOGD("[RpSimController] handleAreUiccApplicationsEnabledReq (slot %d)", getSlotId());

    if (handleFakeAreUiccApplicationsEnabledRsp(message)) {
        return;
    }
    ...
    if (dest == RADIO_TECH_GROUP_GSM && mGsmSimCtrl != NULL) {
        mGsmSimCtrl->handleAreUiccApplicationsEnabledReq(message);
    } else if (dest == RADIO_TECH_GROUP_C2K && mC2kSimCtrl != NULL) {
        mC2kSimCtrl->handleAreUiccApplicationsEnabledReq(message);
    } else {
        RLOGE("[RpSimController] handleAreUiccApplicationsEnabledReq, can't dispatch the req");
    }
}

void RpSimController::handleAreUiccApplicationsEnabledRsp(
        const sp<RfxMessage>& message) {
    RLOGD("[RpSimController] handleAreUiccApplicationsEnabledRsp (slot %d)", getSlotId());

    RILD_RadioTechnology_Group src = message->getSource();

    if (src == RADIO_TECH_GROUP_GSM && mGsmSimCtrl != NULL) {
        mGsmSimCtrl->handleAreUiccApplicationsEnabledRsp(message);
    } else if (src == RADIO_TECH_GROUP_C2K && mC2kSimCtrl != NULL) {
        mC2kSimCtrl->handleAreUiccApplicationsEnabledRsp(message);
    } else {
        RLOGE("[RpSimController] handleAreUiccApplicationsEnabledRsp, can't dispatch the rsp");
    }
}

// For V test case, modem doesn't support
bool RpSimController::handleFakeEnableUiccApplicationsRsp(
        const sp<RfxMessage>& message) {
    RLOGD("[RpSimController] handleFakeEnableUiccApplicationsRsp (slot %d)", getSlotId());
    int length = 0;
    ...
    p->readInt32(&length);
    if (length == 1) {
        p->readInt32(&mUiccApplicationEnable);
    }
    if (getStatusManager()->getIntValue(RFX_STATUS_KEY_SIM_STATE) == RFX_SIM_STATE_ABSENT) {
        rilError = RIL_E_SIM_ABSENT;
    } else {
        rilError = RIL_E_SUCCESS;
    }
    sp<RfxMessage> voidResponse = RfxMessage::obtainResponse(rilError, message);
    responseToRilj(voidResponse);
    return true;
}

bool RpSimController::handleFakeAreUiccApplicationsEnabledRsp(
        const sp<RfxMessage>& message) {
    RLOGD("[RpSimController] handleFakeAreUiccApplicationsEnabledRsp (slot %d)", getSlotId());
    int length = 0;
    RIL_Errno rilError = RIL_E_SUCCESS;
    if (getStatusManager()->getIntValue(RFX_STATUS_KEY_SIM_STATE) == RFX_SIM_STATE_ABSENT) {
        rilError = RIL_E_SIM_ABSENT;
    ...
    sp<RfxMessage> response = RfxMessage::obtainResponse(rilError, message);
    Parcel* p = response->getParcel();
    p->writeInt32(mUiccApplicationEnable);
    responseToRilj(response);
    return true;
}
...
void RpSimController::handleIccidChanged(const sp<RfxMessage>& message) {
    Parcel *p = message->getParcel();
    size_t pos = p->dataPosition();
    String8 iccId = String8(p->readString16());
    p->setDataPosition(pos);

    getStatusManager()->setString8Value(RFX_STATUS_KEY_SIM_ICCID, iccId);
    responseToRilj(message);
}
