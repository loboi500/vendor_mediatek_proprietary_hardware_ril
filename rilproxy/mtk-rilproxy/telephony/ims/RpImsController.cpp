...
// last hashed iccid
char RpImsController::sLastBootIccId[4][PROPERTY_VALUE_MAX] = {{0}, {0}, {0}, {0}};
...
void RpImsController::setDefaultVolteState(int slot_id, const char new_iccid[],int card_type) {
    ...
    char hashedNewIccIdStr[PROPERTY_VALUE_MAX] = { 0 };
    ...
    if (card_type == CT_3G_UIM_CARD ||
        ...
        property_set(PROPERTY_LAST_ICCID_SIM[slot_id], hashedNewIccIdStr);
...
bool RpImsController::isOp09SimCard(int slot_id, const char icc_id[], int card_type) {
    RFX_UNUSED(slot_id);
...
const std::string RpImsController::hash256(const char* str) {
    char buf[2];
