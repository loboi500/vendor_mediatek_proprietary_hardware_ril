...
void RpNwStateController::combineVoiceRegState(const sp<RfxMessage>& msg) {
    ...
    len = sprintf(stgBuf, "%d", mVoiceRegState);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%x", gsm_voice_reg_state_cache.lac);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%x", gsm_voice_reg_state_cache.cid);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", mVoiceRadioTech);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.base_station_id);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.base_station_latitude);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.base_station_longitude);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.css);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.system_id);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.network_id);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.roaming_ind);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.prl);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.prl_roaming_ind);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", gsm_voice_reg_state_cache.deny_reason);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);
    ...
    len = sprintf(stgBuf, "%d", cdma_voice_reg_state_cache.network_exsit);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineVoiceRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);
...
void RpNwStateController::combineDataRegState(const sp<RfxMessage>& msg) {
    ...
    len = sprintf(stgBuf, "%d", mDataRegState);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    if (gsm_data_reg_state_cache.lac != -1) {
        len = sprintf(stgBuf, "%x", gsm_data_reg_state_cache.lac);
        if (len < 0) {
            logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
        }
    } else {
        len = sprintf(stgBuf, "%d", gsm_data_reg_state_cache.lac);
        if (len < 0) {
            logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
        }
    }
    ...
    if (gsm_data_reg_state_cache.cid != -1) {
        len = sprintf(stgBuf, "%x", gsm_data_reg_state_cache.cid);
        if (len < 0) {
            logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
        }
    } else {
        len = sprintf(stgBuf, "%d", gsm_data_reg_state_cache.cid);
        if (len < 0) {
            logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
        }
    }
    ...
    len = sprintf(stgBuf, "%d", mDataRadioTech);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", gsm_data_reg_state_cache.deny_reason);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", gsm_data_reg_state_cache.max_data_call);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);
    ...
    len = sprintf(stgBuf, "%d", gsm_data_reg_state_cache.isEmcBearerSupported);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "combineDataRegState sprintf fail");
    }
    writeStringToParcel(p, stgBuf);
...
void RpNwStateController::updateFemtoCell(const sp<RfxMessage>& urc) {
    ...
    len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.domain);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.state);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.plmn_id);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
    }
    writeStringToParcel(p, stgBuf);
    ...
    len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.act);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
    }
    writeStringToParcel(p, stgBuf);

    len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.isFemtocell);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
    }
    writeStringToParcel(p, stgBuf);
    ...
    if (gsm_femto_cell_cache.isFemtocell == 1) {
        len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.csg_id);
        if (len < 0) {
            logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
        }
        writeStringToParcel(p, stgBuf);

        len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.csg_icon_type);
        if (len < 0) {
            logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
        }
        writeStringToParcel(p, stgBuf);
    ...
    len = sprintf(stgBuf, "%d", gsm_femto_cell_cache.cause);
    if (len < 0) {
        logE(RP_NW_STATE_TAG, "updateFemtoCell sprintf fail");
    }
    writeStringToParcel(p, stgBuf);
