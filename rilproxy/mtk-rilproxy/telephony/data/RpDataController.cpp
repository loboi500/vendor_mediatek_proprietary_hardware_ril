...
void RpDataController::onInit() {
    ...
    getStatusManager()->registerStatusChanged(RFX_STATUS_CONNECTION_STATE,
            RfxStatusChangeCallback(this, &RpDataController::onConnectionStateChanged));
}
...
void RpDataController::handleRequestDefault(const sp<RfxMessage>& request) {
    ...
    if (RpDataUtils::isPreferredDataMode()
        ...
        parcel->readInt32();  // conent length
        String16 idString = parcel->readString16();  // interfaceId string
        String16 reason = parcel->readString16(); // reason string
        message->resetParcelDataStartPos();
        int id = atoi(String8(idString).string());
        if (id < 0) {
            if (RIL_DATA_PROFILE_IMS == -id) {
                parcel->writeInt32(2);
                parcel->writeString16(String16("4")); // interface id
                parcel->writeString16(reason); // reason
                goto success;
            }
...
void RpDataController::handleSetupDataRequestAlt(const sp<RfxMessage>& request) {
    ...
    if ((mInterfaceId >= 0) && (m_slot_id >=0 && m_slot_id <MAX_SIM_COUNT) &&
...
void RpDataController::onConnectionStateChanged(RfxStatusKeyEnum key,
        RfxVariant old_value, RfxVariant value) {
    ...
    if (oldState && !newState) {
        ...
        sendDeactAllDataCallRequest(m_slot_id);
    }
}
