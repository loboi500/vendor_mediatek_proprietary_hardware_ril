...
void registerSignal()
{
    ...
    if ((isInternalLoad() == 0) && (isUserLoad() == 1)) {
        ...
        if (signal(SIGSEGV, signalTreatment) == SIG_ERR) {
            RLOGE("signal SIGSEGV failed");
        }
        if (signal(SIGABRT, signalTreatment) == SIG_ERR) {
            RLOGE("signal SIGABRT failed");
        }
        if (signal(SIGILL, signalTreatment) == SIG_ERR) {
            RLOGE("signal SIGILL failed");
        }
        if (signal(SIGPIPE, signalTreatment) == SIG_ERR) {
            RLOGE("signal SIGPIPE failed");
        }
        if (signal(SIGBUS, signalTreatment) == SIG_ERR) {
            RLOGE("signal SIGBUS failed");
        }
        if (signal(SIGFPE, signalTreatment) == SIG_ERR) {
            RLOGE("signal SIGFPE failed");
        }
    }
}
