...
void onNewCommandConnect(RIL_SOCKET_ID socket_id, android::ClientType clientType) {
    ...
    if (thread_id != process_id) {
        RLOGE("onNewCommandConnect tid %d not match pid %d", thread_id, process_id);
        RLOGE("trigger TRM");
    ...
    int ret = 0;
    RLOGD("connect lock s_pendingUrcMutex, slot %d, client %d", slotId, clientType);
    ret = pthread_mutex_lock(&(s_pendingUrcMutex[slotId][clientType]));
    assert(ret == 0);
    bIsCallbackReady[slotId][clientType] = true;
    RLOGD("connect unlock s_pendingUrcMutex, slot %d, client %d", slotId, clientType);
    ...
    RLOGD("onNewCommandConnect slotId %d client %d", slotId, clientType);
}
...
void onCommandDisconnect(RIL_SOCKET_ID socket_id, android::ClientType clientType) {
    ...
    ClientId client = getClientBySlot((int) socket_id);
    int ret = 0;
    RLOGD("Disconnect lock s_pendingUrcMutex, slot %d, client %d", slotId, clientType);
    ret = pthread_mutex_lock(&(s_pendingUrcMutex[slotId][clientType]));
    assert(ret == 0);
    bIsCallbackReady[(unsigned int)slotId][clientType] = false;
    RLOGD("Disconnect unlock s_pendingUrcMutex, slot %d, client %d", slotId, clientType);
    ret = pthread_mutex_unlock(&(s_pendingUrcMutex[slotId][clientType]));
    assert(ret == 0);
    if (CLIENT_RILJ == client) {
        CALL_UPDATECONNECTIONSTATE(slotId, CONNECTION_STATE_OFF);
    }
    RLOGD("onCommandDisconnect slotId %d client %d", slotId, clientType);
}
...
eventLoop(void *param) {
    ...
    error = pthread_mutex_lock(&s_startupMutex);
    assert(error == 0);

    s_started = 1;
    pthread_cond_broadcast(&s_startupCond);

    error = pthread_mutex_unlock(&s_startupMutex);
    assert(error == 0);
...
RIL_register (const RIL_RadioFunctions *callbacks) {
    ...
    // initialize cache urc mutex before providing services.
    ...
    for (int i = 0; i < MAX_SIM_COUNT; i++) {
        for (int j = 0; j < CLIENT_TYPE_COUNT; j++) {
            RLOGD("init s_pendingUrcMutex, slot %d, client %d", i, j);
...
int checkAndDequeueRequestInfoIfAck(struct RequestInfo *pRI, bool isAck) {
    ...
    error = pthread_mutex_lock(pendingRequestsMutexHook);
    assert(error == 0);
    ...
    error = pthread_mutex_unlock(pendingRequestsMutexHook);
    assert(error == 0);
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_SET_MODEM_CONFIG: return "RIL_REQUEST_SET_MODEM_CONFIG";
...
static bool cacheUrcByClient(int unsolResponse, const void *data, size_t datalen
    ...
    int error = 0;
    RLOGD("cache lock s_pendingUrcMutex, slot %d, client %d", socket_id, targetClient);
    error = pthread_mutex_lock(&(s_pendingUrcMutex[socket_id][targetClient]));
    assert(error == 0);
    ...
    RLOGD("New pended URC:%d, RILD:%s, :%s, client %d",
            ...
            requestToString(urcCur->id), targetClient);
unlock :
    RLOGD("cache unlock s_pendingUrcMutex, slot %d, client %d", socket_id, targetClient);
    error = pthread_mutex_unlock(&(s_pendingUrcMutex[socket_id][targetClient]));
    assert(error == 0);
    return ret;
}
...
void sendUrc(RIL_SOCKET_ID socket_id, AtResponseList** urcCached, android::ClientType clientType) {
    ...
    int ret = 0;
    RLOGD("send lock s_pendingUrcMutex, slot %d, client %d", socket_id, clientType);
    ...
    *urcCached = urc;
    RLOGD("send unlock s_pendingUrcMutex, slot %d, client %d", socket_id, clientType);
