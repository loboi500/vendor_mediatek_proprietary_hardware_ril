...
#else
#define SRC_FILE_OFFSET (0)
#define ErrMsg(format, ...) while(debug_fp) {char dt[64];time_t t;struct tm tt;t=time(NULL);localtime_r(&t, &tt);strftime(dt,sizeof(dt),"%F %T",&tt);fprintf(debug_fp, "[%s] (%s:%d) "format"\n",dt,&__FILE__[SRC_FILE_OFFSET],__LINE__,##__VA_ARGS__); fflush(debug_fp); break;}
#define DbgMsg(format, ...) while(debug_fp) {char dt[64];time_t t;struct tm tt;t=time(NULL);localtime_r(&t, &tt);strftime(dt,sizeof(dt),"%F %T",&tt);fprintf(debug_fp, "[%s] (%s:%d) "format"\n",dt,&__FILE__[SRC_FILE_OFFSET],__LINE__,##__VA_ARGS__); fflush(debug_fp); break;}
