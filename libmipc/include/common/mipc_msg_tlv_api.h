...
{
    if (index >= MIPC_MAX_SYS_THERMAL_SENSOR_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_sys_thermal_sensor_config_e_struct4)) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_SYS_THERMAL_SENSOR_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
static inline mipc_msg_api_result_enum mipc_sys_set_thermal_actuator_req_add_id(mipc_msg_t *msg_ptr, enum mipc_sys_thermal_actuator_id_const_enum value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
static inline mipc_msg_api_result_enum mipc_sys_get_thermal_actuator_info_req_add_id(mipc_msg_t *msg_ptr, enum mipc_sys_thermal_actuator_id_const_enum value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
static inline mipc_sys_thermal_actuator_id_const_enum mipc_sys_get_thermal_actuator_info_cnf_get_id(mipc_msg_t *msg_ptr, mipc_sys_thermal_actuator_id_const_enum def_val)
{
    if (!msg_ptr) return def_val;
    return (mipc_sys_thermal_actuator_id_const_enum)mipc_msg_get_val_uint32(msg_ptr, MIPC_SYS_GET_THERMAL_ACTUATOR_INFO_CNF_T_ID, def_val);
}
...
static inline mipc_msg_api_result_enum mipc_sys_get_thermal_actuator_req_add_actuator_id(mipc_msg_t *msg_ptr, enum mipc_sys_thermal_actuator_id_const_enum value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_SYS_THERMAL_TRIP_POINT_NUM_IN_ONE_SENSOR || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
static inline mipc_msg_api_result_enum mipc_sys_set_thermal_runtime_req_add_trip_change(mipc_msg_t *msg_ptr, uint16_t len, mipc_sys_thermal_trip_change_struct4 *value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_sys_thermal_trip_change_struct4)) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_SYS_THERMAL_ACTUATOR_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
static inline mipc_msg_api_result_enum mipc_apn_add_profile_list_req_add_profile_list(mipc_msg_t *msg_ptr, uint16_t len, mipc_apn_profile_v2_struct4 *value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_apn_profile_v2_struct4)) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_tlv(msg_ptr, MIPC_APN_ADD_PROFILE_LIST_REQ_T_PROFILE_LIST, len, (const void *)value);
}
...
static inline mipc_msg_api_result_enum mipc_data_act_call_req_add_ursp_traffic_desc(mipc_msg_t *msg_ptr, uint16_t len, mipc_ursp_traffic_desc_struct_struct4 *value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_ursp_traffic_desc_struct_struct4)) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_CID_LIST_LEN || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_S_NSSAI_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_RULE_LIST_LEN || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_ursp_rule_struct_struct4)) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_idx(msg_ptr, array, index, len, (const void *)value);
}
...
{
    if (index >= MIPC_MAX_RULE_LIST_LEN || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_CHANNEL_LOCK_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_idx(msg_ptr, array, index, len, (const void *)value);
}
...
{
    if (index >= MIPC_MAX_UE_OS_ID_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_os_id_info_struct4)) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_idx(msg_ptr, array, index, len, (const void *)value);
}
...
{
    if (index >= MIPC_MAX_PHYSICAL_CHANNEL_CONFIGS_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_NWSCN_RECORD_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_idx(msg_ptr, array, index, len, (const void *)value);
}
...
{
    if (index >= MIPC_MAX_NWSCN_PLMN_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_idx(msg_ptr, array, index, len, (const void *)value);
}
...
{
    if (index >= MIPC_MAX_SIM_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_SIM_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
static inline mipc_msg_api_result_enum mipc_sim_sml_set_allowed_carriers_req_add_allowed_carriers(mipc_msg_t *msg_ptr, uint16_t len, mipc_sim_carrier_struct_struct4 *value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_sim_carrier_struct_struct4)) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_SMS_CBM_MSGID_RANGE_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_SMS_CBM_MSGID_SINGLE_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_SMS_CBM_DCS_RANGE_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    mipc_msg_add_idx_uint16(msg_ptr, array, index, value);
    return MIPC_MSG_API_RESULT_SUCCESS;
}
...
{
    if (index >= MIPC_MAX_SMS_CBM_DCS_SINGLE_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_SMS_CBM_MSGID_RANGE_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_SMS_CBM_MSGID_SINGLE_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_SMS_CBM_DCS_RANGE_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_SMS_CBM_DCS_SINGLE_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_SMS_CBM_LANGUAGE_SINGLE_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_CALL_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_CALL_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_CALL_CONFERENCE_PARTICIPANT_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_CALL_CONFERENCE_PARTICIPANT_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_CALL_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len >  MIPC_MAX_DIAL_ADDRESS_LEN) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_idx(msg_ptr, array, index, len, (const void *)value);
...
static inline mipc_msg_api_result_enum mipc_ims_set_eireg_req_add_reg_state(mipc_msg_t *msg_ptr, uint8_t value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
static inline mipc_msg_api_result_enum mipc_ims_set_eireg_req_add_reg_type(mipc_msg_t *msg_ptr, uint8_t value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
static inline mipc_msg_api_result_enum mipc_ims_set_eireg_req_add_dereg_cause(mipc_msg_t *msg_ptr, uint8_t value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
static inline mipc_msg_api_result_enum mipc_ims_set_eireg_req_add_ims_rat(mipc_msg_t *msg_ptr, uint8_t value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
static inline mipc_msg_api_result_enum mipc_ims_set_eireg_req_add_sip_uri_type(mipc_msg_t *msg_ptr, uint8_t value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_WFC_THRESHOLD_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_GRP_ID_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    mipc_msg_add_idx_uint32(msg_ptr, array, index, value);
    return MIPC_MSG_API_RESULT_SUCCESS;
}
...
{
    if (index >= MIPC_MAX_GRP_ID_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
static inline mipc_msg_api_result_enum mipc_phb_set_phb_entry_req_add_entry(mipc_msg_t *msg_ptr, uint16_t len, mipc_phb_entry_struct4 *value)
{
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len != sizeof(mipc_phb_entry_struct4)) return MIPC_MSG_API_RESULT_FAIL;
...
{
    if (index >= MIPC_MAX_NWSCN_RESULT_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_SMS_SEG_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_DEL_SPI_NUM || index < 0) return 0;
    if (!msg_ptr) { if (val_len_ptr) {*val_len_ptr = 0;} return 0;}
...
{
    if (index >= MIPC_MAX_WFC_THRESHOLD_NUM || index < 0) return def_val;
    if (!msg_ptr) return def_val;
...
{
    if (index >= MIPC_MAX_WFC_DNS_NUM || index < 0) return MIPC_MSG_API_RESULT_FAIL;
    if (!msg_ptr) return MIPC_MSG_API_RESULT_FAIL;
    if (!value) return MIPC_MSG_API_RESULT_FAIL;
    if (len >  MIPC_MAX_WFC_IP_LEN) return MIPC_MSG_API_RESULT_FAIL;
    return mipc_msg_add_idx(msg_ptr, array, index, len, (const void *)value);
