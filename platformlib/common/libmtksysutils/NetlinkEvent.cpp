...
NetlinkEvent::NetlinkEvent() {
    ...
    mtk_property_get("ro.build.type", property_value, "0");
...
bool NetlinkEvent::parseIfAddrMessage(const struct nlmsghdr *nh) {
    ...
    if (cacheinfo) {
         SLOGD("parseIfAddrMessage: if:%s; ADDRESS %s;VALID=%u;", ifname,
                mIsUserBuild ? "xxx" : addrstr, cacheinfo->ifa_valid);
    } else {
         SLOGD("parseIfAddrMessage: if:%s; ADDRESS %s;", ifname, mIsUserBuild ? "xxx" : addrstr);
    }
    return true;
}
...
void clearRaInfoFlag(char *buff) {
    ...
    int n = snprintf(proc, sizeof(proc), "/proc/sys/net/ipv6/conf/%s/ra_info_flag",
        buff);
    if (n < 0) {
        SLOGE("Failed to snprintf");
        return;
    }
