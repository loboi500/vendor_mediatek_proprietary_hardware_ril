...
{
    if (!gEmptyStringBuf) {
        mtkLogE(LOG_TAG, "String doesn't initialize");
        initialize_string8();
    }
...
{
    if (gEmptyStringBuf) {
        mtkLogE(LOG_TAG, "String has initialized");
        return;
    }
    ...
    if (buf == NULL) {
        mtkAssert((char*)"String8 initialize memory fail!");
    }
