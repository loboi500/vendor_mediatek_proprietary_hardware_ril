...
int getMdMaxSimCount() {
    char property_value[MTK_PROPERTY_VALUE_MAX] = { 0 };
    mtk_property_get("ro.vendor.num_md_protocol", property_value, "0");
    return atoi(property_value);
}
...
int isOp20Support() {
    ...
    char optr_value[MTK_PROPERTY_VALUE_MAX] = {0};
    mtk_property_get("persist.vendor.operator.optr", optr_value, "0");
    if (strcmp(optr_value, OPERATOR_OP20) == 0) {
        ret = 1;
    }
    return ret;
}
...
bool isPcoStateUrcCustomized() {
    return (MTK_PCO_STATE_URC_CUSTOMIZED == 1) ? true : false;
}
