...
int main(int argc, char **argv) {
    ...
    if(prop_mux_start[0] == '0') {
        ...
        property_set("vendor.gsm.ril.eboot", "0");
...
int mtkInit()
{
    ...
    if (signal(SIGUSR1, signal_treatment) == SIG_ERR) {
        RLOGE("Setup SIGUSR1 fail");
    }
    if (signal(SIGUSR2, signal_treatment) == SIG_ERR) {  // for MD state change
        RLOGE("Setup SIGUSR2 fail");
    }
    if ((isInternalLoad() == 0) && (isUserLoad() == 1)) {
        RLOGD("Setup signal handling");
        if (signal(SIGSEGV, signal_treatment) == SIG_ERR) {
            RLOGE("Setup SIGSEGV fail");
        }
        if (signal(SIGABRT, signal_treatment) == SIG_ERR) {
            RLOGE("Setup SIGABRT fail");
        }
        if (signal(SIGILL, signal_treatment) == SIG_ERR) {
            RLOGE("Setup SIGILL fail");
        }
        if (signal(SIGPIPE, signal_treatment) == SIG_ERR) {
            RLOGE("Setup SIGPIPE fail");
        }
        if (signal(SIGBUS, signal_treatment) == SIG_ERR) {
            RLOGE("Setup SIGBUS fail");
        }
        if (signal(SIGFPE, signal_treatment) == SIG_ERR) {
            RLOGE("Setup SIGFPE fail");
        }
    }

    ret = snprintf(rild_pid, sizeof(rild_pid), "%d", getpid()); //get pid
    if (ret < 0) {
        RLOGD("Failed to get rild pid");
        return 0;
    }
