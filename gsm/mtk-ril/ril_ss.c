...
static void handleOnUssi(const char **data, size_t datalen, RIL_SOCKET_ID rid) {
    ...
    if (finalData == NULL) {
        LOGE("handleOnUssi, malloc failed!");
        return;
    }

    finalData[0] = finalData[1] = NULL;

    asprintf(&finalData[0], "%d", ussdMode);  // USSD mode
    if (finalData[0] == NULL) {
        free(finalData);
        LOGE("handleOnUssi, finalData[0] failed!");
        return;
    }
    ...
    if (finalData[1] == NULL) {
        free(finalData[0]);
        free(finalData);
        LOGE("handleOnUssi, finalData[1] failed!");
        return;
    }
...
static char* encryptString(char* input) {
    ...
    if (sprintf(pivotStr, "%d", pivot) < 0) {
        LOGE("encryptString, sprintf failed!");
        return NULL;
    }
