...
extern char s_logicalModemId[RIL_SOCKET_NUM][MAX_UUID_LENGTH];
...
void requestSetModemConfig(void *data, size_t datalen, RIL_Token t);
...
static bool simSwitchIsAllSimInitDone() {
    ...
    if (sim_count <= 0 || sim_count > getMdMaxSimCount()) {
        RLOGI("simSwitchIsAllSimInitDone, sim count error: %d", sim_count);
        return false;
    }
...
extern void requestGetRadioCapability(void * data __unused, size_t datalen __unused, RIL_Token t)
{
    ...
    if (rc != NULL) {
        ...
        LOGD("requestGetRadioCapability calloc error");
        assert(1);
    }
}
...
extern void requestSetRadioCapability(void * data, size_t datalen __unused, RIL_Token t)
{
    ...
    if (sprintf(sRcSessionId,"%d",rc.session) < 0) {
        LOGD("requestSetRadioCapability sprintf error");
    }


    RIL_RadioCapability* responseRc = (RIL_RadioCapability*) malloc(sizeof(RIL_RadioCapability));
    if (responseRc == NULL) {
        LOGD("requestSetRadioCapability malloc error");
...
extern void bootupGetIccid(RILChannelCtx *p_channel) {
        ...
        if (at_get_cme_error(p_response) == CME_SUCCESS) {
            ...
            if (err >= 0) {
                ...
                if (err >= 0) {
                    ...
                    RLOGD("bootupGetIccid,rid: %d, iccid: %s", rid, hashedIccid);
    ...
    if (!result) {
        ...
        char simEnabled[PROPERTY_VALUE_MAX] = {0};
        char *isRuimCard;
        property_get(PROPERTY_RIL_FULL_UICC_TYPE[rid], cardType, "");
        property_get(PROPERTY_UICC_APPLICATIONS_ENABLE_STATE[rid], simEnabled, "");
        RLOGD("%s: %s", PROPERTY_RIL_FULL_UICC_TYPE[rid], cardType);
        RLOGD("%s: %s", PROPERTY_UICC_APPLICATIONS_ENABLE_STATE[rid], simEnabled);
        ...
        } else if (strcmp(simEnabled, "2") != 0) {
             property_set(PROPERTY_ICCID_SIM[rid], "N/A");
             RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, "N/A", strlen("N/A"), rid);
        }
    }
}
...
extern int triggerCCCIIoctlEx(int request, int *param)
{
    ...
    property_get("ro.vendor.mtk_protocol1_rat_config", prop_value, "");
    ...
    if (enableMd1) {
        ret = snprintf(dev_node, 32, "%s", ccci_get_node_name(USR_RILD_IOCTL, MD_SYS1));
        if (ret < 0) {
            RLOGE("dev_node get node name fail");
            return -1;
        }
        ...
        ret = snprintf(dev_node, 32, "%s", ccci_get_node_name(USR_RILD_IOCTL, MD_SYS5));
        if (ret < 0) {
            RLOGE("dev_node get node name fail");
            return -1;
        }
...
extern int rilOemMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request)
    {
        ...
        case RIL_REQUEST_SET_MODEM_CONFIG:
            requestSetModemConfig(data, datalen, t);
            break;
        default:
...
void getLogicModemId() {
    ...
    for (i = 0; i < getMdMaxSimCount(); i++) {
        ret = sprintf(s_logicalModemId[i], "modem_sys%d_ps%d", mdSys, (i+1));
        if (ret < 0) {
            LOGD("s_logicalModemId sprintf fail");
            return;
        }
    }
    // swap if need
...
void requestSetModemConfig(void *data __unused, size_t datalen __unused, RIL_Token t){
    int mode = ((int *)data)[0];
    ATResponse *p_response = NULL;
    int err;
    RILChannelCtx* p_channel = getRILChannelCtxFromToken(t);
    ...
    if (mode == 1) {
        ...
        int main_sim = queryMainProtocol(RIL_SIM);
        if (main_sim != 1) {
            if (RatConfig_isLteFddSupported() || RatConfig_isLteTddSupported()) {
                err = at_send_command("AT+ES3G=1,7", &p_response, p_channel);
            } else {
                err = at_send_command("AT+ES3G=1,3", &p_response, p_channel);
            }
            if (err < 0 || p_response->success == 0) {
                RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
                at_response_free(p_response);
        ...
        // Set c2k capability to sim1
        char tempStr[PROPERTY_VALUE_MAX] = { 0 };
        property_get("persist.vendor.radio.c_capability_slot", tempStr, "1");
        if (atoi(tempStr) > 1) {
            property_set("persist.vendor.radio.c_capability_slot", "1");
        }
    }
    RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
    at_response_free(p_response);
