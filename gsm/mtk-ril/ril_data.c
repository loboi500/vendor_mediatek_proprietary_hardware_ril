...
void setRspEranType(int eran_type, MTK_RIL_Data_Call_Response_v11 *pRspData,
        mal_datamngr_data_call_info_req_t* responseToMal, RIL_SOCKET_ID rid, int profileId);
...
void requestDeactiveDataCall(void * data, size_t datalen, RIL_Token t)
{
    ...
    if (reason == DEACT_REASON_AOSP_HANDOVER) {
        LOGD("[%d][%s] Not deactivate data call for handover case", rid, __FUNCTION__);
        RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
        return;
    }
...
void requestSetInitialAttachApn(void * data, size_t datalen __unused, RIL_Token t)
{
    ...
    if (param->apn == NULL) {
        ...
        if (param->apn == NULL) {
            LOGE("IA: requestSetInitialAttachApn apn memory alloc error");
        }
    }
    if (param->username == NULL) {
        param->username = calloc(1, sizeof(char));
        if (param->username == NULL) {
            LOGE("IA: requestSetInitialAttachApn username memory alloc error");
        }
    }
    if (param->password == NULL) {
        param->password = calloc(1, sizeof(char));
        if (param->password == NULL) {
            LOGE("IA: requestSetInitialAttachApn password memory alloc error");
        }
    }
 ...
 and not to trigger PS re-attach");
        ...
        if (strlen(iaProperty) != 0) {
            ...
            if (strlen(param->password) == 0) {
                n_snprintf = snprintf(iaParameter, PROPERTY_VALUE_MAX*2, "%s,%s,%d,%s,%d,%s,%s",
                        iccid, param->protocol, param->authtype, param->username,
                        canHandleIms, param->roamingProtocol, param->apn);
                if (n_snprintf < 0) {
                    LOGE("IA: requestSetInitialAttachApn iaParameter snprintf error");
                    goto error;
                }
            } else {
                ...
                n_snprintf = snprintf(iaParameter, PROPERTY_VALUE_MAX*2, "%s,%s,%d,%s,%d,%s,%s", "",
                        ...
                        canHandleIms, param->roamingProtocol, param->apn);
                if (n_snprintf < 0) {
                    LOGE("IA: requestSetInitialAttachApn iaParameter1 snprintf error");
                    goto error;
                }
            }
            ...
            if (isOp12Support()) {
                if (strcmp(param->apn, "VZWIMS") == 0 || strcmp(param->apn, "IMS") == 0) {
                    ...
                    // check VZWIMS
                    if (strlen(param->password) == 0) {
                        n_snprintf = snprintf(iaParameter, sizeof(iaParameter), "%s,%s,%d,%s,%d,%s",
                                iccid, param->protocol, param->authtype, param->username,
                                param->canHandleIms, "VZWIMS");
                        if (n_snprintf < 0) {
                            LOGE("IA: requestSetInitialAttachApn iaParameter2 snprintf error");
                            goto error;
                        }
                    } else {
                        //when password is set, iccid is not recorded
                        //so we do not need to compare iccid
                        n_snprintf = snprintf(iaParameter, sizeof(iaParameter),
                                "%s,%s,%d,%s,%d,%s", "", param->protocol, param->authtype,
                                param->username, param->canHandleIms, "VZWIMS");
                        if (n_snprintf < 0) {
                            LOGE("IA: requestSetInitialAttachApn iaParameter3 snprintf error");
                            goto error;
                        }
                    }
                    ...
                    if (strlen(param->password) == 0) {
                        n_snprintf = snprintf(iaParameter, sizeof(iaParameter), "%s,%s,%d,%s,%d,%s",
                                iccid, param->protocol, param->authtype, param->username,
                                param->canHandleIms, "IMS");
                        if (n_snprintf < 0) {
                            LOGE("IA: requestSetInitialAttachApn iaParameter4 snprintf error");
                            goto error;
                        }
                    } else {
                        //when password is set, iccid is not recorded
                        ...
                        n_snprintf = snprintf(iaParameter, sizeof(iaParameter), "%s,%s,%d,%s,%d,%s", "",
                                ...
                                param->canHandleIms, "IMS");
                        if (n_snprintf < 0) {
                            LOGE("IA: requestSetInitialAttachApn iaParameter5 snprintf error");
                            goto error;
                        }
                    }
                    if (strcmp(iaParameter, iaProperty) == 0) {
            ...
            n_snprintf = snprintf(iaParameter, PROPERTY_VALUE_MAX*2, "%s,%s,%d,%s,%d,%s,%s,%s",
                    iccid, param->protocol, param->authtype, param->username, canHandleIms,
                    param->roamingProtocol, param->apn, param->password);
            if (n_snprintf < 0) {
                LOGE("IA: requestSetInitialAttachApn iaParameter6 snprintf error");
                goto error;
            }
            if (strcmp(iaParameter, iaProperty) != 0) {
...
set patch to invalid value");
            ...
            if (triggerPsAttach && queryCurrentConnType(pChannel) &&
                ...
                if (pInfo == NULL) {
                    LOGE("IA: requestSetInitialAttachApn OnReAttachInfo calloc error");
                    goto error;
                }
    ...
    if (0 == isFindNoDataPdn) {
        ...
        if (isATCmdRspErr(err, p_response)) {
            ...
            for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
                ...
                if (out != NULL && pdn_info[rid][activatedCid].cid == cid
                        && pdn_info[rid][activatedCid].active == DATA_STATE_LINKDOWN) {
                    strncpy(pdn_info[rid][activatedCid].apn, out, MAX_APN_NAME_LENGTH - 1);
...
void requestSetFdMode(void * data, size_t datalen, RIL_Token t) {
    ...
    if (paramNumber == 1) {
        n_snprintf = snprintf(cmdLine, sizeof(cmdLine), "AT+EFD=%d", ((int *)data)[1]);
        if (n_snprintf < 0) {
            LOGE("requestSetFdMode cmdLine snprintf error");
            goto error;
        }
    } else if (paramNumber == 2) {
        n_snprintf = snprintf(cmdLine, sizeof(cmdLine), "AT+EFD=%d,%d",
                ((int *)data)[1], ((int *)data)[2]);
        if (n_snprintf < 0) {
            LOGE("requestSetFdMode cmdLine1 snprintf error");
            goto error;
        }
        ...
        // Set timer property.
        n_snprintf = snprintf(timerValue, sizeof(timerValue), "%d", parameter2);
        if (n_snprintf < 0) {
            LOGE("requestSetFdMode timerValue snprintf error");
            goto error;
        }
        property_set(PROPERTY_FD_TIMER[parameter1], timerValue);

        n_snprintf = snprintf(cmdLine, sizeof(cmdLine), "AT+EFD=%d,%d,%d",
                ((int *)data)[1], parameter1, parameter2);
        if (n_snprintf < 0) {
            LOGE("requestSetFdMode cmdLine2 snprintf error");
            goto error;
        }
    } else {
...
void requestDataAttach(void* data, size_t datalen, RIL_Token t) {
    ...
    if (isATCmdRspErr(err, p_response)) {
        ...
        while (retry < MAX_RETRY_PS_ATTACH) {
            ...
            err = at_send_command("AT+EGTYPE=4", &p_response, DATA_CHANNEL_CTX);
            if (p_response != NULL && isATCmdRspErr(err, p_response)) {
                retry++;
...
int doAttachBeforeRadioOn(RIL_SOCKET_ID rid, int onOff, RILChannelCtx *pChannel) {
    return combineAttachAndFollowOn(rid, onOff, pChannel);
}
...
void initialCidTable() {
    ...
    for (i = 0; i < simCount; i++) {
        if (pdn_info_wifi[i] == NULL) {
            ...
            if (pdn_info_wifi[i] == NULL) {
                LOGE("[%s] calloc for pdn_info_wifi[i] failed!", __FUNCTION__);
                return;
            }
            int j = 0;
...
void getCcmniIfname() {
    int n_snprintf = snprintf(s_ccmni_ifname, sizeof(s_ccmni_ifname), "%s", CCMNI_IFNAME_CCMNI);
    if (n_snprintf < 0) {
        LOGE("getCcmniIfname s_ccmni_ifname snprintf error");
        return;
    }
...
void getL2pValue(RIL_Token t) {
    ...
    if(bSupportCcmni == true) {
        n_snprintf = snprintf(s_l2p_value, sizeof(s_l2p_value), "%s", L2P_VALUE_CCMNI);
        if (n_snprintf < 0) {
            LOGE("getL2pValue s_l2p_value snprintf error");
            goto error;
        }
    } else {
        n_snprintf = snprintf(s_l2p_value, sizeof(s_l2p_value), "%s", L2P_VALUE_UPS);
        if (n_snprintf < 0) {
            LOGE("getL2pValue s_l2p_value1 snprintf error");
            goto error;
        }
    }
...
error:
    ...
    n_snprintf = snprintf(s_l2p_value, sizeof(s_l2p_value), "%s", L2P_VALUE_UPS);
    if (n_snprintf < 0) {
        LOGE("getL2pValue s_l2p_value2 snprintf error");
    }
...
int updatePdnAddressByCid(int cid, RILChannelCtx* rilchnlctx)
{
    ...
    if (get_address_type(out) == IPV4) {
        n_snprintf = snprintf(pdn_info[rid][responseCid].addressV4,
                sizeof(pdn_info[rid][responseCid].addressV4), "%s", out);
        if (n_snprintf < 0) {
            LOGE("updatePdnAddressByCid addressV4 snprintf error");
            goto error;
        }
    } else {
        /// M: Ims Data Framework @{
    ...
    if ( at_tok_hasmore(&line)) {
        ...
        if (get_address_type(out) == IPV4) {
            n_snprintf = snprintf(pdn_info[rid][responseCid].addressV4,
                    sizeof(pdn_info[rid][responseCid].addressV4), "%s", out);
            if (n_snprintf < 0) {
                LOGE("updatePdnAddressByCid addressV4 2 snprintf error");
                goto error;
            }
        } else {
            /// M: Ims Data Framework @{
...
int updateActiveStatus(RILChannelCtx* rilchnlctx) {
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
        ...
        if (pdn_info[rid][responseCid].active > 0) {
            ...
            n_snprintf = snprintf(activeCidsBuf + actChars, charBufSize - actChars,
                    "%d:%d, ", responseCid, pdn_info[rid][responseCid].primaryCid);
            if (n_snprintf < 0) {
                LOGE("updateActiveStatus activeCidsBuf snprintf error");
                break;
            }
            actChars += n_snprintf;
        } else {
            ...
            if (inactChars < charBufSize) {
                n_snprintf = snprintf(inactiveCidsBuf + inactChars, charBufSize - inactChars,
                        "%d:%d, ", responseCid, pdn_info[rid][responseCid].primaryCid);
                if (n_snprintf < 0) {
                    LOGE("updateActiveStatus inactiveCidsBuf snprintf error");
                    break;
                }
                inactChars += n_snprintf;
            }
        }
    }
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
        ...
        for(i = 0; i<length; i++) {
            ...
            if (dotCount > 7 && innerDotCount == 16) {//the end of IPv6 address
                ...
                pkFilter->mask[sizeof(pkFilter->mask)-1] = '\0';
...
int updateDefaultBearerInfo(RILChannelCtx* rilchnlctx)
{
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL; p_cur = p_cur->p_next) {
        ...
        if (strlen(apn) > 0)
            strncpy(pdn_info[rid][responseCid].apn, apn, MAX_APN_NAME_LENGTH - 1);
    ...
    switch (tft->operation) {
        ...
        case 4: //replace packet filter, this is used when modifying bearer
            ...
            for (i=0; i<tft->tftParameter.linkedPfNumber; i++) {
                ...
                int n_asprintf = asprintf(&cmd, "AT+EGLNKPF=%d,%d", cid, tft->tftParameter.linkedPfList[i]);
                if (n_asprintf < 0) {
                    LOGE("configureTft asprintf error");
                }
                if (cmd == NULL) goto end;
                err = at_send_command(cmd, &p_response, rilchnlctx);
                free(cmd);
        ...
        case 5: //delete packet filter, this is used when modify bearer
            ...
            for (i=0; i<tft->pfNumber; i++) {
                int n_sprintf = asprintf(&cmd, "AT+EGDELPF=%d,%d", cid, tft->pfList[i].id);
                if (n_sprintf < 0) {
                    LOGE("configureTft asprintf error");
                }
                if (cmd == NULL) goto end;
...
void updateNetworkInterface(int interfaceId, int mask, RIL_SOCKET_ID rid) {
    ...
    if (rsp_ptr == NULL) {
        LOGE("updateNetworkInterface rsp_ptr memory alloc error");
    }

    memset(&ifr, 0, sizeof(struct ifreq));
    int n_snprintf = snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s%d",
            s_ccmni_ifname, interfaceId);
    if (n_snprintf < 0) {
        LOGE("updateNetworkInterface ifr_name snprintf error");
    }
...
void configureNetworkInterface(int interfaceId, int isUp, RIL_SOCKET_ID rid)
{
    ...
    memset(&ifr, 0, sizeof(struct ifreq));
    n_snprintf = snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s%d", s_ccmni_ifname, interfaceId);
    if (n_snprintf < 0) {
        LOGE("configureNetworkInterface ifr_name snprintf error");
    }
...
void convertToGlobalAddress(char* addressV6, char* prefix) {
    ...
    if (ret >= 0) {
        n_snprintf = snprintf(result, sizeof(result), "%s:%x:%x:%x:%x",
            prefix, htons(ip.s6_addr16[4]),htons(ip.s6_addr16[5]),
            htons(ip.s6_addr16[6]),htons(ip.s6_addr16[7]));
        if (n_snprintf < 0) {
            LOGE("convertToGlobalAddress result snprintf error");
            return;
        }
        strncpy(addressV6, result, MAX_IPV6_ADDRESS_LENGTH - 1);
    } else {
        ...
        mal_datamngr_data_call_info_req_t* responseToMal, RIL_SOCKET_ID rid)
{
    ...
    responseToMal->eran_type = getRatByCid(rid, cid);
...
int createDataResponse(int interfaceId, int protocol, MTK_RIL_Data_Call_Response_v11* response, RIL_SOCKET_ID rid)
{
    ...
    for (i = 0; i < WIFI_MAX_PDP_NUM; i++) {
        if (pdn_info_wifi[rid][i].interfaceId == interfaceId &&
            ...
            if (interfaceId >= 0 && hoLtePdnSuccess[rid][interfaceId] == true) {
...
void createDedicateDataResponse(int cid, MALRIL_Dedicate_Data_Call_Struct* response, RIL_SOCKET_ID rid)
{
    ...
    for (i=0; i<MAX_PCSCF_NUMBER; i++) {
        if (strlen(pdnInfo->pcscf[i]) > 0) {
            ...
            int nResult =0;
            if (i == 0)
                nResult = sprintf(response->pcscf + strlen(response->pcscf), "%s", pdnInfo->pcscf[i]);
            else
                nResult = sprintf(response->pcscf + strlen(response->pcscf), " %s", pdnInfo->pcscf[i]);
            if (nResult < 0)
                LOGE("createDedicateDataResponse result snprintf error");
        }
    }
}
...
void dumpMalDataResponse(mal_datamngr_data_call_info_req_t* response, const char* reason)
{
   LOGD("%s data call response: status=%d, suggestedRetryTime=%d, interfaceId=%d, active=%d, mtu=%d, "
         "type=%s, ifname=%s, gateways=%s, pcscf=%s, eran_type=%d",
            reason == NULL ? "dumpDataResponse" : reason,
            response->status, response->suggestedRetryTime,
            response->cid, response->active,
            response->mtu, response->type,
            response->ifname,response->gateways,
...
void dumpDedicateDataResponse(MALRIL_Dedicate_Data_Call_Struct* response, const char* reason)
{
    ...
    for (i=0; i<response->tft.tftParameter.linkedPfNumber; i++) {
        char str[5] = {0};
        n_snprintf = snprintf(str, sizeof(str), "%d ", response->tft.tftParameter.linkedPfList[i]);
        if (n_snprintf < 0) {
            LOGE("dumpDedicateDataResponse result snprintf error");
            return;
        }
...
QOS[qci=%d, dlGbr=%d, ulGbr=%d, dlMbr=%d, ulMbr=%d], PF number=%d, TftParameter[linked: %s]",
    ...
    if (response->tft.pfNumber > 0) {
        for (i=0; i<response->tft.pfNumber; i++) {
                ...
                localPortLow=%d, localPortHigh=%d, remotePortLow=%d, remotePortHigh=%d, \
                flowLabel=%05x, direction=%d]",
...
int apnMatcher (const char* out, const char* apn) {
    ...
    if (out == NULL || apn == NULL) {
        LOGE("apnMatcher: out or apn is NULL");
        return -1;
    }
...
int queryMatchedPdnWithSameApn(const char* apn, int* matchedCidList, RILChannelCtx* rilchnlctx)
{
    ...
    if (updateDefaultBearerInfo(rilchnlctx) < 0) {
        LOGE("[%d]queryMatchedPdnWithSameApn updateDefaultBearerInfo failed", rid);
        return matchedCount;
    }

    if (apn == NULL) {
        LOGE("[%d]queryMatchedPdnWithSameApn apn is NULL", rid);
        return matchedCount;
    }
            ...
            for (j = 0; j < pdnInfoSize; j++) {
                ...
                if (j == cid && (pdn_info[rid][j].active == DATA_STATE_LINKDOWN ||
                    ...
                    if (((out != NULL && strlen(out) > 0 && apnMatcher(out, apn) == 0) ||
                              ...
                              ((REPLACE_APN_FLAG == replaceApnFlag) && (cid == 0) && (apnMatcher(replaceApn, apn) == 0)) ||
                              (0 == strcmp(apn, ""))) {
...
void convertIPv6AddressToDotFormat(char* address, int maxLength)
{
    int ret = 1;
    int n_snprintf = 0;
    int addrLength = sizeof(char) * maxLength;
    ...
    if (ret >= 0) {
        n_snprintf = snprintf(address, addrLength,
            "%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d.%d",
            v6Address.s6_addr[0], v6Address.s6_addr[1], v6Address.s6_addr[2],
            v6Address.s6_addr[3], v6Address.s6_addr[4], v6Address.s6_addr[5],
            v6Address.s6_addr[6], v6Address.s6_addr[7], v6Address.s6_addr[8],
            v6Address.s6_addr[9], v6Address.s6_addr[10], v6Address.s6_addr[11],
            v6Address.s6_addr[12], v6Address.s6_addr[13], v6Address.s6_addr[14],
            v6Address.s6_addr[15]);
        if (n_snprintf < 0) {
            LOGE("convertIPv6AddressToDotFormat snprintf error");
        }
    }
}
...
char* getConfigTftATCmd(int cid, PktFilter* pktFilter)
{
    ...
    if (cmd == NULL) {
        LOGE("getConfigTftATCmd cmd memory alloc error");
        return NULL;
    }
    ...
    //R8
    //+CGTFT=<cid>[,<pktFilterId>,<evalPrecIdx>[,<addr_n_mask>[,<protocolNum>
    // [,<dstPortRange>[,<srcPortRange>[,<spi>[,<tos_n_mask>[, <flow_label>]]]]]]]]
    ...
    n_snprintf = snprintf(cmd, cmdSize - 1, "AT+CGTFT=%d,%d,%d",
            cid, pktFilter->id, pktFilter->precedence);
    if (n_snprintf < 0) {
        LOGE("getConfigTftATCmd cmd snprintf error");
        return cmd;
    }
    ...
    if ((pktFilter->bitmap & PKT_BITMASK_IMC_BMP_V4_ADDR) || (pktFilter->bitmap & PKT_BITMASK_IMC_BMP_V6_ADDR)) {
        ...
        n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1, ",\"%s.%s\"",
            strcmp(pktFilter->address, "") ? pktFilter->address : NULL_IPV4_ADDRESS,
            strcmp(pktFilter->mask, "") ? pktFilter->mask : NULL_IPV4_ADDRESS);
        if (n_snprintf < 0) {
            LOGE("getConfigTftATCmd cmd1 snprintf error");
            return cmd;
        }
    }
    else {
    ...
    if (pktFilter->bitmap & PKT_BITMASK_IMC_BMP_PROTOCOL) {
        n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1, ",%d",
                pktFilter->protocolNextHeader);
        if (n_snprintf < 0) {
            LOGE("getConfigTftATCmd cmd2 snprintf error");
            return cmd;
        }
    } else {
        strncpy(cmd + strlen(cmd), ",", cmdSize - strlen(cmd) - 1);
    ...
    if (max_pdn_support_4G > 0) {
        if ((pktFilter->bitmap & PKT_BITMASK_IMC_BMP_LOCAL_PORT_SINGLE) ||
            ...
            n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1, ",\"%d.%d\"",
                pktFilter->localPortLow, pktFilter->localPortHigh);
            if (n_snprintf < 0) {
                LOGE("getConfigTftATCmd cmd3 snprintf error");
                return cmd;
            }
        } else {
            strncpy(cmd + strlen(cmd), ",\"\"", cmdSize - strlen(cmd) - 1);
    ...
    if ((pktFilter->bitmap & PKT_BITMASK_IMC_BMP_REMOTE_PORT_SINGLE) ||
        ...
        n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1, ",\"%d.%d\"",
            pktFilter->remotePortLow, pktFilter->remotePortHigh);
        if (n_snprintf < 0) {
            LOGE("getConfigTftATCmd cmd4 snprintf error");
            return cmd;
        }
    }
    else {
    ...
    if (pktFilter->bitmap & PKT_BITMASK_IMC_BMP_SPI) {
        n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1,
                ",%08x", pktFilter->spi);
        if (n_snprintf < 0) {
            LOGE("getConfigTftATCmd cmd5 snprintf error");
            return cmd;
        }
    } else {
        strncpy(cmd + strlen(cmd), ",", cmdSize - strlen(cmd) - 1);
    ...
    if (pktFilter->bitmap & PKT_BITMASK_IMC_BMP_TOS) {
        n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1, ",\"%d.%d\"",
                pktFilter->tos, pktFilter->tosMask);
        if (n_snprintf < 0) {
            LOGE("getConfigTftATCmd cmd6 snprintf error");
            return cmd;
        }
    } else {
        strncpy(cmd + strlen(cmd), ",\"\"", cmdSize - strlen(cmd) - 1);
    ...
    if (pktFilter->bitmap & PKT_BITMASK_IMC_BMP_FLOW_LABEL) {
        n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1, ",%05x",
                pktFilter->flowLabel);
        if (n_snprintf < 0) {
            LOGE("getConfigTftATCmd cmd7 snprintf error");
            return cmd;
        }
    } else {
        strncpy(cmd + strlen(cmd), ",", cmdSize - strlen(cmd) - 1);
    ...
    if (max_pdn_support_4G > 0) {
        n_snprintf = snprintf(cmd + strlen(cmd), cmdSize - strlen(cmd) - 1, ",%d",
                pktFilter->direction);
        if (n_snprintf < 0) {
            LOGE("getConfigTftATCmd cmd8 snprintf error");
            return cmd;
        }
    }
...
int clearQosTftContext(int cid, RILChannelCtx* rilchnlctx)
{
    ...
    if (max_pdn_support_4G > 0) {
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+CGEQOS=%d", cid);
        if (n_snprintf < 0) {
            LOGE("clearQosTftContext cmd snprintf error");
            return failCause;
        }
    } else {
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+CGEQREQ=%d", cid);
        if (n_snprintf < 0) {
            LOGE("clearQosTftContext cmd1 snprintf error");
            return failCause;
        }
    }
    ...
    memset(cmd, 0, sizeof(cmd));
    n_snprintf =snprintf(cmd, sizeof(cmd), "AT+CGTFT=%d", cid);
    if (n_snprintf < 0) {
        LOGE("clearQosTftContext cmd2 snprintf error");
        return failCause;
    }
    failCause = at_send_command(cmd, &p_response, rilchnlctx);
    if (isATCmdRspErr(err, p_response)) {
    ...
    memset(cmd, 0, sizeof(cmd));
    n_snprintf = snprintf(cmd, sizeof(cmd), "AT+EGDELPF=%d", cid);
    if (n_snprintf < 0) {
        LOGE("clearQosTftContext cmd3 snprintf error");
        return failCause;
    }
    failCause = at_send_command(cmd, &p_response, rilchnlctx);
    if (isATCmdRspErr(err, p_response)) {
    ...
    memset(cmd, 0, sizeof(cmd));
    n_snprintf = snprintf(cmd, sizeof(cmd), "AT+EGLNKPF=%d", cid);
    if (n_snprintf < 0) {
        LOGE("clearQosTftContext cmd4 snprintf error");
        return failCause;
    }
    failCause = at_send_command(cmd, &p_response, rilchnlctx);
    if (isATCmdRspErr(err, p_response)) {
...
void notifyLinkdownDedicatedBearer(const int def_bearer_cid, const RIL_SOCKET_ID rid) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (pResponse == NULL) {
            LOGe("[%d][%s] malloc pResponse error", rid, __FUNCTION__);
...
int convert_ipv6_address(char* pdp_addr, char* ipv6_addr, int isLinkLocal) {
    ...
    while (p) {
        ...
        value = atoi(p);
        n_snprintf = snprintf(tmpOutput, MAX_IPV6_ADDRESS_LENGTH, "%02X", value);
        if (n_snprintf < 0) {
            LOGE("convert_ipv6_address tmpOutput snprintf error");
            return 0;
        }
        ...
        n_snprintf = snprintf(tmpOutput, MAX_IPV6_ADDRESS_LENGTH, "%02X:", value);
        if (n_snprintf < 0) {
            LOGE("convert_ipv6_address tmpOutput1 snprintf error");
            return 0;
        }
        strncat(ipv6_addr + strlen(ipv6_addr), tmpOutput,
                MAX_IPV6_ADDRESS_LENGTH - strlen(ipv6_addr) - 1);
    ...
    RIL_Token t)
{
    ...
    if (response == NULL || responseToMal == NULL) {
        LOGE("[%d][%s] response calloc error", rid, __FUNCTION__);
        goto error;
    }
    ...
    int profileId = atoi(pReqSetupConfig->profile);
    int handoverInterfaceId = 0;
    if (isHandoverRequest(profileId, &handoverInterfaceId, rid)) {
        createDataResponse(handoverInterfaceId, protocol, response, rid);
    ...
    if (activatedCidList == NULL) {
        LOGE("[%d][%s] activatedCidList calloc error", rid, __FUNCTION__);
        goto error;
    }
    ...
    setRspEranType(epdgConfig.eranType, response, responseToMal, rid, profileId);
    dumpDataResponse(response, "requestSetupDataCallImsEmergency dump response");
    ...
    RIL_Token t)
{
    ...
    int handoverInterfaceId = 0;
    if (isHandoverRequest(profileId, &handoverInterfaceId, rid)) {
        int tempCid = createDataResponse(handoverInterfaceId, protocol, response, rid);
        dumpDataResponse(response, "requestSetupDataCallOverIPv6 dump response");

        createMalDataResponse(handoverInterfaceId, tempCid, response, responseToMal, rid);
        setRspEranType(epdgConfig.eranType, response, responseToMal, rid, profileId);
    ...
    if (queryResult > 0 && isCidTypeMatched(activatedCidList[0], protocol, DATA_CHANNEL_CTX)) {
        ...
        char iaApn[PROPERTY_VALUE_MAX] = {0};
        getMSimProperty(iaSimId, PROPERTY_IA_APN, iaApn);
        if (requestedApn != NULL && strcasecmp(requestedApn, iaApn) != 0 &&
    ...
    RIL_Token t)
{
    ...
    createMalDataResponse(interfaceId, cid, response, responseToMal, rid);
    makeDedicatedDataResponse(concatenatedBearerNum, activatedPdnNum, activatedCidList, responseToMal, rid);
    setRspEranType(epdgConfig.eranType, response, responseToMal, rid, atoi(pReqSetupConfig->profile));
...
void requestOrSendDataCallListIpv6(RILChannelCtx* rilchnlctx, RIL_Token *t, RIL_SOCKET_ID rid)
{
    ...
    if (t != NULL) {
        ...
        if (sim_id != SIM_ID_INVALID && rsp_ptr != NULL) {
...
int syncApnTableToMd(void* data, size_t datalen, RIL_Token t) {
    ...
    memset(cmd, 0, sizeof(cmd));
    n_snprintf = snprintf(cmd, sizeof(cmd), "AT+EAPNSYNC");
    if (n_snprintf < 0) {
        LOGE("syncApnTableToMd cmd snprintf error");
        return 0;
    }
    ...
    for (int i = 0; i < num; i++) {
        ...
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+EAPNSYNC=%d,\"%s\",%d", i, apn, inactiveTimer);
        if (n_snprintf < 0) {
            LOGE("syncApnTableToMd cmd1 snprintf error");
            return 0;
        }
        err = at_send_command(cmd, &p_response, pChannel);
        if (isATCmdRspErr(err, p_response)) {
        ...
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+VZWAPNE=%d,%d,\"%s\",\"%s\",\"%s\",\"%s\",%d",
                i, apnClass, apn, protocol, apnBearer, apnEnable, apnTime);
        if (n_snprintf < 0) {
            LOGE("syncApnTableToMd cmd2 snprintf error");
            return 0;
        }
        err = at_send_command(cmd, &p_response, pChannel);
        if (isATCmdRspErr(err, p_response)) {
        ...
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+VZWAPNETMR=%d,%d,%d,%d,%d",
                i, maxConns, maxConnsTime, waitTime, throttlingTime);
        if (n_snprintf < 0) {
            LOGE("syncApnTableToMd cmd3 snprintf error");
            return 0;
        }
        err = at_send_command(cmd, &p_response, pChannel);
        if (isATCmdRspErr(err, p_response)) {
...
void syncDisabledApnToMd(RILChannelCtx *pChannel) {
    ...
    if (fPtr == NULL) {
        ...
        if (fclose(fPtr) == EOF) {
            LOGE("syncDisabledApnToMd(), fclose fptr error");
        }
    }
}
// M: [VzW] Data Framework @}
...
int syncApnTableToRds(void* data, size_t datalen, RIL_Token t) {
    ...
    if (apnConfig == NULL) {
        LOGD("syncApnTableToRds apnConfig alloc memory error");
        return 0;
    }
    rds_apn_cfg_table_t* apnTable = (rds_apn_cfg_table_t*) calloc(dataCount, sizeof(rds_apn_cfg_table_t));
    if (apnTable == NULL) {
        LOGD("syncApnTableToRds apnTable alloc memory error");
        FREEIF(apnConfig);
        return 0;
    }
    RIL_SOCKET_ID rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));
    ...
    char *apnTypes = (char *)calloc(MAX_LENGTH_APN_TYPES_TO_RDS, sizeof(char));
    if (apnTypes == NULL) {
        LOGD("syncApnTableToRds apnTypes alloc memory error");
        FREEIF(apnConfig);
        FREEIF(apnTable);
        return 0;
    }
    char *tempProileId = (char *)calloc(10, sizeof(char));
    if (tempProileId == NULL) {
        LOGD("syncApnTableToRds tempProileId alloc memory error");
        FREEIF(apnConfig);
        FREEIF(apnTable);
        FREEIF(apnTypes);
        return 0;
            ...
            if (apnTypesBitmask == RIL_APN_TYPE_UNKNOWN ||
                ...
                for (int j = 0; j < RIL_APN_TYPE_COUNT; j++) {
                    if (apnTypesBitmask & (1 << j)) {
                        ...
                        if (profileId >= 0) {
                            if (countOfApnType == 0) {
                                n_snprintf = snprintf(apnTypes, MAX_LENGTH_APN_TYPES_TO_RDS - 1,
                                        "%d", profileId);
                                if (n_snprintf < 0) {
                                    LOGE("syncApnTableToRds apnTypes snprintf error");
                                }
                            } else {
                                strncat(apnTypes, symbolStr, strlen(symbolStr));
                                n_snprintf = snprintf(tempProileId, (10 * sizeof(char)) - 1,
                                        "%d", profileId);
                                if (n_snprintf < 0) {
                                    LOGE("syncApnTableToRds tempProileId snprintf error");
                                }
...
void requestStopKeepalive(void* data, size_t datalen, RIL_Token t) {
    ...
    n_snprintf = snprintf(cmdLine, sizeof(cmdLine), "AT+EKALIVE=0,%d", sessionHandle);
    if (n_snprintf < 0) {
        LOGE("requestStopKeepalive cmdLine snprintf error");
        goto error;
    }
...
void parseAndSendPcoStatus(char* line, RIL_SOCKET_ID rid, int cid, char* apnname) {
    ...
    char pcoIdStr[MAX_PCO_LENGTH];
    char plmn[MAX_PLMN_LENGTH];
    int pcoContent = -1;
    ...
    if (at_tok_hasmore(&line)) {
        ...
        strncpy(pcoIdStr, token, MAX_PCO_LENGTH - 1);
        ...
        strncpy(plmn, token, MAX_PLMN_LENGTH - 1);
        ...
        if (pcoData.contents == NULL) {
            LOGE("[%s] malloc for pcoData.contents failed!", __FUNCTION__);
            return;
        }
...
int getAddressTypeByCid(int cid, RIL_SOCKET_ID rid) {
    ...
    /* Use AT+CGPADDR=cid to query the ip address assigned to this PDP context indicated via this cid */
    asprintf(&cmd, "AT+CGPADDR=%d", cid);
    ret = at_send_command_singleline(cmd, "+CGPADDR:", &p_response, rilchnlctx);
    free(cmd);

    /* Parse the response to get the ip address */
    if (isATCmdRspErr(ret, p_response)) {
        goto error;
    }
    line = p_response->p_intermediates->line;
    ret = at_tok_start(&line);
    ...
    /* Get 1st parameter: CID */
    ret = at_tok_nextint(&line, &responseCid);
    if (ret < 0)
        goto error;
        ...
        int authType, const char *username, const char* password, RILChannelCtx *pChannel)
{
    ...
    if (max_pdn_support_4G > 0) {
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+CGDCONT=%d,\"%s\",\"%s\",,0,0,0,%d,%d,%d",
                availableCid, pszIPStr, requestedApn, emergency_ind,
                pcscf_discovery_flag, signalingFlag);
        if (n_snprintf < 0) {
            LOGE("definePdnCtx cmd snprintf error");
        }
        if (requestType == REQUEST_TYPE_HANDOVER) {
            memset(cmd, 0, sizeof(cmd));
            // new AT spec
            n_snprintf = snprintf(cmd, sizeof(cmd), "AT+CGDCONT=%d,\"%s\",\"%s\",,0,0,0,%d,%d,%d",
                    availableCid, pszIPStr, requestedApn, requestType,
                    pcscf_discovery_flag, signalingFlag);
            if (n_snprintf < 0) {
                LOGE("definePdnCtx cmd1 snprintf error");
                return 0;
            }
        ...
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+CGDCONT=%d,\"%s\",\"%s\",,0,0",
                availableCid, pszIPStr, requestedApn);
        if (n_snprintf < 0) {
            LOGE("definePdnCtx cmd2 snprintf error");
        }
    }
    ...
    if (g_iaRoamingProtocol[iaSimId] != NULL) {
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+EGDCONT=%d,\"%s\"",
                availableCid, g_iaRoamingProtocol[iaSimId]);
        if (n_snprintf < 0) {
            LOGE("definePdnCtx cmd3 snprintf error");
        }
...
int bindPdnToIntf(int activatedPdnNum,int interfaceId,int cid,RILChannelCtx * pChannel)
{
    ...
    if (pdn_info[rid][cid].interfaceId == INVALID_CID
        ...
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+EGLD=%d", cid);
        if (n_snprintf < 0) {
            LOGE("bindPdnToIntf cmd snprintf error");
            nRet = 0;
            goto error;
        }
        err = at_send_command(cmd, &p_response, pChannel);
        ...
        memset(cmd, 0, sizeof(cmd));
        n_snprintf = snprintf(cmd, sizeof(cmd), "AT+CGDATA=\"%s\",%d,%d",
                s_l2p_value, cid, interfaceId+1); //The channel id is from 1~n
        if (n_snprintf < 0) {
            LOGE("bindPdnToIntf cmd1 snprintf error");
            nRet = 0;
            goto error;
        }
        err = at_send_command(cmd, &p_response, pChannel);
    ...
    if (getAttachApnSupport() == ATTACH_APN_NOT_SUPPORT) {
        ...
        if (strlen(iaProperty) > 0) {
            ...
            if (strlen(roamingProtocol) > 0) {
                ...
                if (g_iaRoamingProtocol[iaSimId] == NULL) {
                    LOGE("IA: defineAttachApnIfIACacheExisted g_iaRoamingProtocol calloc error");
                    pthread_mutex_unlock(&iaRoamingMutex[iaSimId]);
                    goto finish;
                }
...
void getIaCache(int iaSimId, char* cache)
{
    ...
    if (strlen(iaProperty) > 0) {
        n_snprintf = snprintf(cache, PROPERTY_VALUE_MAX * 2, "%s,%s", iaProperty, iaProperty2);
        if (n_snprintf < 0) {
            LOGE("getIaCache cache snprintf error");
            return;
        }
    } else {
        n_snprintf = snprintf(cache, 1, "%s", ""); //no IA cache
        if (n_snprintf < 0) {
            LOGE("getIaCache cache1 snprintf error");
            return;
        }
    }
        ...
        const char* roamingProtocol)
{
    ...
    if (iccid == NULL || strlen (iccid) == 0) {
        ...
        char iaProperty2[PROPERTY_VALUE_MAX] = {0};
        char maskIccid[PROPERTY_VALUE_MAX] = {0};
        n_snprintf = snprintf(iaProperty, sizeof(iaProperty), "%s,%s,%d,%s,%d,%s",
                iccid, protocol, authtype, username, canHandleIms, roamingProtocol);
        if (n_snprintf < 0) {
            LOGE("setIaCache iaProperty1 snprintf error");
            return;
        }
        n_snprintf = snprintf(iaProperty2, sizeof(iaProperty2), "%s", apn);
        if (n_snprintf < 0) {
            LOGE("setIaCache iaProperty2 snprintf error");
            return;
        }
...
void getTempIa(int iaSimId, char* cache)
{
    ...
    if (strlen(iaProperty) > 0) {
        n_snprintf = snprintf(cache, PROPERTY_VALUE_MAX * 2, "%s,%s,%s",
                iaProperty, iaProperty2, TEMP_IA_PASSWORD[iaSimId]);
        if (n_snprintf < 0) {
            LOGE("getTempIa cache snprintf error");
            return;
        }
    } else {
        n_snprintf = snprintf(cache, 1, "%s", ""); //no IA cache
        if (n_snprintf < 0) {
            LOGE("getTempIa cache1 snprintf error");
            return;
        }
    }
    givePrintableIccid(cache, maskIccid);
        ...
        const char* roamingProtocol)
{
    ...
    if (iccid == NULL || strlen (iccid) == 0) {
        ...
        char iaProperty2[PROPERTY_VALUE_MAX] = {0};
        char maskIccid[PROPERTY_VALUE_MAX] = {0};
        n_snprintf = snprintf(iaProperty, sizeof(iaProperty), "%s,%s,%d,%s,%d,%s", iccid, protocol,
                authtype, username, canHandleIms, roamingProtocol);
        if (n_snprintf < 0) {
            LOGE("setTempIa iaProperty snprintf error");
            return;
        }
        n_snprintf = snprintf(iaProperty2, sizeof(iaProperty2), "%s", apn);
        if (n_snprintf < 0) {
            LOGE("setTempIa iaProperty2 snprintf error");
            return;
        }
        if(strcmp(iaPropertyOld, iaProperty) == 0 && strcmp(iaProperty2Old, iaProperty2) == 0) {
            LOGD("IA: setTempIa(%d) cache no change", iaSimId);
...
void getIaReplaceApn(int iaSimId, char* buffer)
{
    ...
    if (strlen(iaProperty) > 0) {
        n_snprintf = snprintf(buffer, PROPERTY_VALUE_MAX, "%s", iaProperty);
        if (n_snprintf < 0) {
            LOGE("getIaReplaceApn buffer snprintf error");
            return;
        }
    } else {
        n_snprintf = snprintf(buffer, 1, "%s", ""); //no APN
        if (n_snprintf < 0) {
            LOGE("getIaReplaceApn buffer1 snprintf error");
            return;
        }
    }
...
int isInitialAttachApn(const char *requestedApn, RILChannelCtx *pChannel, int cid) {
    ...
    if (strlen(iaProperty) > 0) {
        n_snprintf = snprintf(apnParameter, PROPERTY_VALUE_MAX, "%s,%s", iccid, requestedApn);
        if (n_snprintf < 0) {
            LOGE("isInitialAttachApn apnParameter snprintf error");
            return false;
        }
...
int onPdnSyncFromSourceRat(void* param) {
    ...
    n_snprintf = snprintf(pdn_info[rid][cid].apn, sizeof(pdn_info[rid][cid].apn), "%s", out);
    if (n_snprintf < 0) {
        LOGE("onPdnSyncFromSourceRat apn snprintf error");
        goto error;
    }
    n_snprintf = snprintf(pSyncPdnInfo[cid].apn, sizeof(pSyncPdnInfo[cid].apn), "%s", out);
    if (n_snprintf < 0) {
        LOGE("onPdnSyncFromSourceRat apn1 snprintf error");
        goto error;
    }
    ...
    if (get_address_type(out) == IPV4) {
        n_snprintf = snprintf(pSyncPdnInfo[cid].addressV4,
                sizeof(pSyncPdnInfo[cid].addressV4), "%s", out);
        if (n_snprintf < 0) {
            LOGE("onPdnSyncFromSourceRat addressV4 snprintf error");
            goto error;
        }
    } else {
    ...
    if (at_tok_hasmore(&urc)) {
        ...
        if (get_address_type(out) == IPV4) {
            n_snprintf = snprintf(pSyncPdnInfo[cid].addressV4,
                    sizeof(pSyncPdnInfo[cid].addressV4), "%s", out);
            if (n_snprintf < 0) {
                LOGE("onPdnSyncFromSourceRat addressV4 1 snprintf error");
                goto error;
            }
        } else {
            convert_ipv6_address(out, pSyncPdnInfo[cid].addressV6, 1);
...
void bindNetworkInterfaceWithModem(int interfaceId, int modemId) {
    ...
    struct ifreq ifr;
    memset(&ifr, 0, sizeof(struct ifreq));
    int n_snprintf = snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s",
            getNetworkInterfaceName(interfaceId));
    if (n_snprintf < 0) {
        LOGE("bindNetworkInterfaceWithModem ifr_name snprintf error");
        return;
    }
...
void reconfigureNetworkInterface(int interfaceId, RIL_SOCKET_ID rid) {
    ...
    int n_snprintf = snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s",
            getNetworkInterfaceName(interfaceId));
    if (n_snprintf < 0) {
        LOGE("reconfigureNetworkInterface ifr_name snprintf error");
        return;
    }
...
void setNetworkTransmitState(int interfaceId, int state) {
    ...
    int n_snprintf = snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s",
            getNetworkInterfaceName(interfaceId));
    if (n_snprintf < 0) {
        LOGE("setNetworkTransmitState ifr_name snprintf error");
        return;
    }
...
int getImsParamInfo(RIL_SOCKET_ID rid, int intfId,
    ...
    if (isImsSupport() && pImsParam != NULL) {
        if (sizeof(mal_datamngr_ims_info_rsp_t) == sizeof(RIL_Default_Bearer_VA_Config_Struct)) {
            ...
            if (req_ptr == NULL) {
                LOGE("[%d][%s] req_ptr calloc error", rid, __FUNCTION__);
                return -1;
            }
...
void responseUnsolDataCallRspToMal(RIL_Token t, const mal_datamngr_data_call_info_req_t *pRespData,
    ...
    if (isEpdgSupport()) {
        ...
        if (rsp_ptr == NULL) {
            LOGE("[%d][%s] rsp_ptr calloc error", rid, __FUNCTION__);
            return;
        }
        // Let's forward it!
        int sim_id = getValidSimId(rid, __FUNCTION__);
        ...
        mal_datamngr_data_call_info_req_t* responseToMal, RIL_SOCKET_ID rid, int profileId)
{
    if (isEpdgSupport() && responseToMal != NULL) {
        ...
        int eran_type_old = RDS_RAN_UNSPEC;
        if (cid >= WIFI_CID_OFFSET) {
            eran_type_old = pdn_info_wifi[rid][cid - WIFI_CID_OFFSET].rat;
            pdn_info_wifi[rid][cid - WIFI_CID_OFFSET].rat = eran_type;
            pdn_info_wifi[rid][cid - WIFI_CID_OFFSET].profileId = profileId;
        } else {
            eran_type_old = pdn_info[rid][cid].rat;
            pdn_info[rid][cid].rat = eran_type;
            pdn_info[rid][cid].profileId = profileId;
        }
        responseToMal->eran_type = eran_type;

        LOGD("[%d][%s] profileId=%d, eran_type=%d", rid, __FUNCTION__, profileId, eran_type);
    }
}

void notifyQualifiedNetworkTypesChanged(RIL_SOCKET_ID rid, int profileId, int rat) {
    const struct timeval TIMEVAL_100_MS = {0, 100e3};

    LOGD("[%d][%s] profileId=%d, rat=%d", rid, __FUNCTION__, profileId, rat);

    if (profileId == RIL_DATA_PROFILE_IMS
            || profileId == RIL_DATA_PROFILE_VENDOR_MMS
            || profileId == RIL_DATA_PROFILE_VENDOR_EMERGENCY
            || profileId == RIL_DATA_PROFILE_VENDOR_ALL) {
        QualifiedNetworkTypesChangedInfo* pInfo =
                calloc(1, sizeof(QualifiedNetworkTypesChangedInfo));
        if (pInfo != NULL) {
            pInfo->rid = rid;
            pInfo->profileId = profileId;
            pInfo->rat = rat;
            RIL_requestProxyTimedCallback(sendQualifiedNetworkTypesChanged, pInfo,
                &TIMEVAL_100_MS, getRILChannelId(RIL_DATA, rid),
                "sendQualifiedNetworkTypesChanged");
        }
    }
}

void sendQualifiedNetworkTypesChanged(void* param) {
    QualifiedNetworkTypesChangedInfo* pInfo = (QualifiedNetworkTypesChangedInfo*)param;
    int rid = pInfo->rid;
    int response[3] = {0};
    int i = 0;

    response[0] = 1;
    response[1] = getApnTypeId(pInfo->profileId);
    response[2] = pInfo->rat;

    if (pInfo->profileId == RIL_DATA_PROFILE_IMS) {
        bool bNotifyMms = true;
        for (i = 0; i < WIFI_MAX_PDP_NUM; i++) {
            if (pdn_info_wifi[rid][i].profileId == RIL_DATA_PROFILE_VENDOR_MMS &&
                    !pdn_info_wifi[rid][i].isDedicateBearer &&
                    pdn_info_wifi[rid][i].active == DATA_STATE_ACTIVE) {
                bNotifyMms = false;
                break;
            }
        ...
        for (i = 0; i < pdnInfoSize; i++) {
            if (pdn_info[rid][i].profileId == RIL_DATA_PROFILE_VENDOR_MMS &&
                    !pdn_info[rid][i].isDedicateBearer &&
                    pdn_info[rid][i].active == DATA_STATE_ACTIVE) {
                bNotifyMms = false;
                break;
        ...
        if (bNotifyMms) {
            response[1] |= RIL_APN_TYPE_MMS;
        }
    } else if (pInfo->profileId == RIL_DATA_PROFILE_VENDOR_ALL) {
        for (i = 0; i < WIFI_MAX_PDP_NUM; i++) {
            if (!pdn_info_wifi[rid][i].isDedicateBearer &&
                    pdn_info_wifi[rid][i].active == DATA_STATE_ACTIVE) {
                if (pdn_info_wifi[rid][i].profileId == RIL_DATA_PROFILE_VENDOR_MMS) {
                    response[1] &= ~RIL_APN_TYPE_MMS;
                } else if (pdn_info_wifi[rid][i].profileId == RIL_DATA_PROFILE_IMS) {
                    response[1] &= ~RIL_APN_TYPE_IMS;
                } else if (pdn_info_wifi[rid][i].profileId == RIL_DATA_PROFILE_VENDOR_EMERGENCY) {
                    response[1] &= ~RIL_APN_TYPE_EMERGENCY;
                }
            }
        }
        for (i = 0; i < pdnInfoSize; i++) {
            if (!pdn_info[rid][i].isDedicateBearer &&
                ...
                if (pdn_info[rid][i].profileId == RIL_DATA_PROFILE_VENDOR_MMS) {
                    response[1] &= ~RIL_APN_TYPE_MMS;
                } else if (pdn_info[rid][i].profileId == RIL_DATA_PROFILE_IMS) {
                    response[1] &= ~RIL_APN_TYPE_IMS;
                } else if (pdn_info[rid][i].profileId == RIL_DATA_PROFILE_VENDOR_EMERGENCY) {
                    response[1] &= ~RIL_APN_TYPE_EMERGENCY;
                }
            }
        }
        if (response[1] == RIL_APN_TYPE_UNKNOWN) {
            LOGD("[%d][%s] No need to notify since the apn type is unknown", rid, __FUNCTION__);
            goto finish;
        }
    }

    LOGD("[%d][%s] apnType=%d, rat=%d", rid, __FUNCTION__, response[1], response[2]);

    RIL_UNSOL_RESPONSE(RIL_UNSOL_QUALIFIED_NETWORK_TYPES_CHANGED,
...
int getApnTypeId(const int profileId) {
    switch (profileId) {
        ...
        case RIL_DATA_PROFILE_VENDOR_ALL:
            return (RIL_APN_TYPE_IMS|RIL_APN_TYPE_MMS|RIL_APN_TYPE_EMERGENCY);
        default: return RIL_APN_TYPE_IMS;
    }
}

bool isHandoverRequest(int profileId, int *pHandoverInterfaceId, RIL_SOCKET_ID rid) {
    int i = 0;

    for (i = 0; i < WIFI_MAX_PDP_NUM; i++) {
        if (pdn_info_wifi[rid][i].profileId == profileId &&
                !pdn_info_wifi[rid][i].isDedicateBearer &&
                pdn_info_wifi[rid][i].active == DATA_STATE_ACTIVE) {
            *pHandoverInterfaceId = pdn_info_wifi[rid][i].interfaceId;
            return true;
        }
    }
    for (i = 0; i < pdnInfoSize; i++) {
        if (pdn_info[rid][i].profileId == profileId &&
                !pdn_info[rid][i].isDedicateBearer &&
                pdn_info[rid][i].active == DATA_STATE_ACTIVE) {
            *pHandoverInterfaceId = pdn_info[rid][i].interfaceId;
            return true;
        }
    }
    return false;
...
int queryEpdgRat(RIL_SOCKET_ID rid, reqSetupConf_t *pConfig, RIL_Default_Bearer_VA_Config_Struct* pDefaultBearerVaConfig)
{
    ...
    if (pConfig->pQueryRatRsp == NULL) {
        LOGE("[%s] pQueryRatRsp allocate error", __FUNCTION__);
        goto error;
    }
...
bool isPdnHoStartFromRds(RIL_SOCKET_ID rid, PdnInfo *pInfo) {
    ...
    do {
        ...
        if (ho_status == NULL) {
            LOGE("[%d][%s] ho_status calloc error", rid, __FUNCTION__);
            break;
        }
        int sim_id = getValidSimId(rid, __FUNCTION__);
        if (sim_id == SIM_ID_INVALID) {
            ...
            if (pPdnInfo != NULL) {
                ...
                if (isPdnHoStart(hoStatus.ucho_status)) {
                    ...
                    if (isHOSuccess(hoStatus.fgho_result)) {
                        ...
                        notifyQualifiedNetworkTypesChanged(rid, pPdnInfo->profileId, RDS_RAN_MOBILE_3GPP);
                ...
                if (isPdnHoStart(hoStatus.ucho_status)) {
                    ...
                    if (isHOSuccess(hoStatus.fgho_result)) {
                        ...
                        if (nWifiCid == INVALID_CID) {
                            ...
                            notifyQualifiedNetworkTypesChanged(rid, pPdnInfo->profileId, RDS_RAN_WIFI);
                        }
                    } else {
...
void requestWifiDisconnectInd(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (isEpdgSupport() && isImsSupport()) {
        ...
        notifyQualifiedNetworkTypesChanged(rid, RIL_DATA_PROFILE_VENDOR_ALL, RDS_RAN_MOBILE_3GPP);
    } else {
       LOGE("[%d][%s] no support epdg, do nothing!!", rid, __FUNCTION__);
    }
...
void requestWifiConnectInd(void *data, size_t datalen, RIL_Token t) {
    ...
    notifyQualifiedNetworkTypesChanged(rid, RIL_DATA_PROFILE_VENDOR_ALL, RDS_RAN_WIFI);
    ...
    RIL_Token t)
{
    ...
    int handoverInterfaceId = 0;
    if (isHandoverRequest(profileId, &handoverInterfaceId, rid)) {
        createDataResponse(handoverInterfaceId, pQueryRatRsp->resp.type, response, rid);
    ...
    if (strcmp(pPdnInfo->addressV6, "::") != 0) {
        ...
        if (ret >= 0) {
            int n_snprintf = snprintf(pPdnInfo->prefix, sizeof(pPdnInfo->prefix), "%x:%x:%x:%x",
                htons(ip.s6_addr16[0]), htons(ip.s6_addr16[1]), htons(ip.s6_addr16[2]),
                htons(ip.s6_addr16[3]));
            if (n_snprintf < 0) {
                LOGE("requestSetupDataCallOverEpdg prefix snprintf error");
            }
        }
        LOGD("[%d][%s] wifi pdn set prefix to %s", rid, __FUNCTION__, pPdnInfo->prefix);
    }

    setRspEranType(pQueryRatRsp->resp.eran_type, response, responseToMal, rid, profileId);
    dumpDataResponse(response, "requestSetupDataCallOverEpdg dump response");
...
void requestSetupDataCallHoEpdg(const reqSetupConf_t *pReqConf, const void *pDefaultBearerConf, RIL_Token t, const int serialId)
{
    ...
    int *activatedCidList = alloca(pdnInfoSize * sizeof(int));
    if (activatedCidList == NULL) {
        LOGD("[%s] activatedCidList alloc error", __FUNCTION__);
        return;
    }
    int isNoDataActive = 0;
    RIL_SOCKET_ID rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));
    ...
    if (rsp_ptr == NULL) {
        LOGE("[%s] allocate rsp_ptr space fail, return", __FUNCTION__);
        if (response != NULL) {
            free(response);
        }
        if (responseToMal != NULL) {
    ...
    makeDedicatedDataResponse(concatenatedBearerNum, activatedPdnNum, activatedCidList, responseToMal, rid);
    setRspEranType(RDS_RAN_MOBILE_3GPP, response, responseToMal, rid, atoi(pReqConf->profile));
...
void configureEpdgNetworkInterface(int interfaceId, int protocol, int isUp, RIL_SOCKET_ID rid) {
    ...
    int n_snprintf = snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s%d",
            s_ccmni_ifname, interfaceId);
    if (n_snprintf < 0) {
        LOGE("configureEpdgNetworkInterface ifr_name snprintf error");
        return;
    }
...
void requestLastDataCallFailCauseFromMal(void * data, size_t datalen, RIL_Token t)
{
    ...
    if (rsp_ptr == NULL) {
        LOGE("[%s] rsp_ptr alloc error", __FUNCTION__);
        RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
        return;
    }
...
void requestPcscfPcoFromMal(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (updateDefaultBearerInfo(DATA_CHANNEL_CTX) < 0) {
        ...
        pcscf= calloc(1, pcscfLength * sizeof(char) + 1);
        if (pcscf == NULL) {
            LOGE("[%d]requestPcscfPco pcscf alloc error", rid);
            RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
            return;
        }
        ...
        if (rsp_ptr == NULL) {
            LOGE("[%d]requestPcscfPco rsp_ptr alloc error", rid);
            RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
            FREEIF(pcscf);
            return;
        }
...
void querySuggestionTimeFromRds(const reqSetupConf_t *pReqSetupConfig,
    ...
    if (retryTimeReq == NULL || retryTimeRsp == NULL) {
        LOGE("[%s] retryTimeReq calloc error", __FUNCTION__);
        FREEIF(retryTimeReq);
        FREEIF(retryTimeRsp);
        return;
    }
...
void requestReuseImsDataCall(void *data, size_t datalen, RIL_Token t) {
    ...
    mal_datamngr_data_call_info_req_t* responseToMal = (mal_datamngr_data_call_info_req_t*)
            calloc(1, sizeof(mal_datamngr_data_call_info_req_t));
    if (response == NULL || responseToMal == NULL) {
        LOGE("[%d][%s] responseToMal calloc error", rid, __FUNCTION__);
        FREEIF(activatedCidList);
        FREEIF(response);
        FREEIF(responseToMal);
        return;
    ...
    if (activatedPdnNum > 0 &&
        ...
        setRspEranType(RDS_RAN_MOBILE_3GPP, response, responseToMal, rid, RIL_DATA_PROFILE_IMS);
...
void setUnsolResponseFilterLinkCapacityEstimate(bool enable, RIL_Token t) {
    RIL_LinkCapacityEstimate* lce = NULL;
    RIL_SOCKET_ID rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));
    LOGD("[%d][%s] enable=%d", rid, __FUNCTION__, enable);

    // Report default link capacity estimate value to framework.
    if (enable) {
        lce = (RIL_LinkCapacityEstimate*)calloc(1, sizeof(RIL_LinkCapacityEstimate));
        if (lce == NULL) {
            LOGE("[%s] callor error", __FUNCTION__);
            return;
        }

        // Set default value as UMTS rat,
        // aligh with definitions in CarrierConfigManager.java.
        lce->downlinkCapacityKbps = 115;
        lce->uplinkCapacityKbps = 115;
        RIL_UNSOL_RESPONSE(RIL_UNSOL_LINK_CAPACITY_ESTIMATE, lce,
                sizeof(RIL_LinkCapacityEstimate), rid);
        FREEIF(lce);
    }
}
...
int setFastdormancyState(int state, int slot) {
    ...
    int n_snprintf = snprintf(cmdLine, sizeof(cmdLine), "AT+EFD=%d", state);
    if (n_snprintf < 0) {
        LOGE("setFastdormancyState cmdLine snprintf error");
        return 0;
    }
...
int getRatByCid(RIL_SOCKET_ID rid, int cid) {
    if (cid >= WIFI_CID_OFFSET) {
        return pdn_info_wifi[rid][cid - WIFI_CID_OFFSET].rat;
    }
    return pdn_info[rid][cid].rat;
}
