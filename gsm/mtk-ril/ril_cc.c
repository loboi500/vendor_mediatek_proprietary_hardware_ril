...
#define MAX_PHONE_NUMBER_LEN  40
...
char cachedForwardNumber[MAX_GSMCALL_CONNECTIONS][MAX_PHONE_NUMBER_LEN + 1] = {{0}};
...
static int callFromCLCCLine(char *line, RIL_Call *p_call) {
    ...
    if ((p_call->index >= 1) && (p_call->index <= MAX_GSMCALL_CONNECTIONS)
            && p_call->isMT) {
        if (strlen(cachedForwardNumber[p_call->index-1]) > 0) {
            p_call->forwardedNumber = cachedForwardNumber[p_call->index-1];
        }
    }
    return 0;
...
void requestSeparateConnection(void *data, size_t datalen, RIL_Token t) {
    ...
    if (party > 0 && party < 10) {
        ret = sprintf(cmd, "AT+CHLD=2%d", party);
        if(ret < 0) {
            goto error;
        }
...
void notifyCallInformationForRedial(int callId, int msgType, int cause,
    ...
    if (sprintf(callInfo[0], "%d", callId) < 0) {
        LOGE("notifyCallInformationForRedial, sprintf fail - callId");
        return;
    ...
    if (sprintf(callInfo[1], "%d", msgType) < 0) {
        LOGE("notifyCallInformationForRedial, sprintf fail - msgType");
        return;
    }
    ...
    if (sprintf(callInfo[9], "%d", cause) < 0) {
        LOGE("notifyCallInformationForRedial, sprintf fail - cause");
        return;
    }
...
static void onHandleEfEccUrc(const char *s, RIL_SOCKET_ID rid) {
    ...
    nRet = snprintf(data, MAX_EF_ECC_LEN, "%d", count);
    if (nRet < 0 || rid >= SIM_COUNT) {
        return;
    }
    ...
    dataLen = strlen(data);
    sEccStr[rid] = calloc(1, sizeof(char)*(dataLen + 1));
    if (sEccStr[rid] == NULL) {
        goto error;
    }
    strncpy(sEccStr[rid], data, dataLen);
    sEccStr[rid][dataLen] = '\0';
...
void onIncomingCallIndication(char *urc, RIL_SOCKET_ID rid) {
    ...
    if (at_tok_hasmore(&urc)) {
        ...
        int call_id = atoi(data[0]);
        if (call_id > 0 && call_id <= MAX_GSMCALL_CONNECTIONS) {
            cachedForwardNumber[call_id - 1][MAX_PHONE_NUMBER_LEN] = '\0';
            strncpy(cachedForwardNumber[call_id - 1], data[5],
    ...
    switch (msg_type) {
        ...
        case 133:
            ...
            if ((disc_callID >= 1) && (disc_callID <= MAX_GSMCALL_CONNECTIONS)) {
                ...
                cachedForwardNumber[disc_callID-1][0] = '\0';
            }
...
void requestSetHvolteMode(void *data, size_t datalen, RIL_Token t) {
    ...
    if (sprintf(cmd, "AT+EHVOLTE=%d,%d", hvolteMode, subMode) < 0) {
        LOGE("requestSetHvolteMode, sprintf fail");
        RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
        return;
    }
