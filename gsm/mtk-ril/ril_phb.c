...
int selectPhbStorage(int type, RIL_Token t) {
    ...
    if (current_phb_storage[rid] != type) {
        ...
        if (asprintf(&cmd, "AT+CPBS=\"%s\"", storage) < 0) {
            result = 0;
            return result;
        }
...
void onPhbStateChanged(RIL_SOCKET_ID rid, int isPhbReady) {
    ...
    if (SIM_COUNT >= 2) {
        LOGI("onPhbStateChanged isSimInserted=%d, isModemResetStarted=%d",
                isSimInserted(rid), isModemResetStarted);
...
static int encodeUCS2_0x81(char *src, char *des, int maxLen) {
    ...
    for (i = 0; i < len; i++) {
        ...
        for (k = 0; k < SPEC_ENCODING_ARR_LEN; k++) {
            if (tmpAlphaId[i] == specialEncoding[k][0]) {
                DLOGD("Haman 0x81 found: i: %zu, tmpAlphaId: %x", i, tmpAlphaId[i]);
    ...
    for (i = 0; i < len; i++, des += 2) {
        if ((tmpAlphaId[i] & 0xFF00) != 0x1B00) {
            ...
            DLOGD("Haman 0x81: i: %d, tmpAlphaId: %x, plenx: %s", i, tmpAlphaId[i], pLen);
            des+=2;
            realLen++;
        }
    }
...
void requestClearPhbEntry(int index, RIL_Token t) {
    ...
    if (maxNum > 0) {
        if (snprintf(cmd, sizeof(cmd), "AT+EPBUM=2,5,%d,0", index) < 0) {
            LOGE("requestClearPhbEntry error");
            return;
        }
        len = strlen((char*)cmd);
...
void requestWritePhbEntry(void *data, size_t datalen, RIL_Token t) {
    ...
    if (selectPhbStorage(entry->type, t)) {
        if (entry->alphaId != NULL || entry->number != NULL) {
            ...
            if (i != len || asciiNotSupported) {
                ...
                if (bIsTc1() == true) {
                    ...
                    for (i = 0; i < strlen(temp); i++) {
                        if (temp[i] == '\\') {
                            LOGD("Use Ascii encoding, there is a '\\', index=%zu", i);
            ...
            if (asprintf(&cmd, "AT+EPBW=%d, \"%s\", %d, \"%s\", 0, %d",
                        entry->index, number, entry->ton, alphaId, encodeMethod) < 0) {
                goto error;
            }
        } else {
            // delete
            if (asprintf(&cmd, "AT+EPBW=%d", entry->index) < 0) {
                goto error;
            }
        }
...
void requestReadPhbEntry(void *data, size_t datalen, RIL_Token t) {
    ...
    if (selectPhbStorage(type, t)) {
        ...
        if (asprintf(&cmd, "AT+CPBR=%d, %d", bIndex, eIndex) < 0) {
            goto error;
        }
...
void requestQueryUPBCapability(void *data, size_t datalen, RIL_Token t) {
    ...
    RIL_onRequestComplete(t, RIL_E_SUCCESS, upbCap, 8*sizeof(int));
...
void requestEditUPBEntry(void *data, size_t datalen, RIL_Token t) {
    ...
    if (0 == strcmp(params[0], "4") || 0 == strcmp(params[0], "3") || 0 == strcmp(params[0], "2")) {
        ...
        line = (char*)params[3];
        len = strlen(line);
        DLOGD("requestEditUPBEntry: line is %s, len=%zu", line, len);
        ...
        if (asprintf(&cmd, "AT+EPBUM=2,%s,%s,%s,\"%s\",%d",
                params[0], params[2], params[1], str, encodeMethod) < 0) {
            goto error;
        }
        ...
        if (i != len || asciiNotSupported) {  // we think email doesn't get into here.
            ...
            if (encodeUCS2_0x81(line, str, sizeof(str)) == 0) {
                // UCS2 coding
                // encodeMethod = CPBW_ENCODE_UCS2;
                memcpy(str, line, ((sizeof(str) < strlen(line)) ? sizeof(str) : strlen(line)));
            }
        ...
        if (asprintf(&cmd, "AT+EPBUM=2,%s,%s,%s,\"%s\"",
                params[0], params[2], params[1], str) < 0) {
            goto error;
        }
    } else if (paramCount > 4 && 0 == strcmp(params[0], "0")) {  // Anr
        if (asprintf(&cmd, "AT+EPBUM=2,%s,%s,%s,\"%s\",%s,%s",
                params[0], params[2], params[1], params[3], params[4], params[5]) < 0) {
            goto error;
        }
    } else {  // Error
...
void requestDeleteUPBEntry(void *data, size_t datalen, RIL_Token t) {
    ...
    if (asprintf(&cmd, "AT+EPBUM=3,%d,%d,%d", params[0], params[2], params[1]) < 0) {
        goto error;
    }
    err = at_send_command(cmd, &p_response, SIM_CHANNEL_CTX);
    free(cmd);
...
void requestReadGasList(void *data, size_t datalen, RIL_Token t) {
    ...
    for (i = params[0]; i <= params[1]; i++) {
        if (asprintf(&cmd, "AT+EPBUM=1,4,%d,0", i) < 0) {
            goto error;
        }
        ...
        if (p_response[i - 1] == NULL || p_response[i - 1]->p_intermediates == NULL) {
            continue;
        }
...
void requestReadUpbGrpEntry(void *data, size_t datalen, RIL_Token t) {
    ...
    if (asprintf(&cmd, "AT+EPBUM=1,5,%d,1", params[0]) < 0) {
        goto error;
    }
    ...
    RIL_onRequestComplete(t, RIL_E_SUCCESS, grpIds, grpCount * sizeof(int));
...
void requestWriteUpbGrpEntry(void *data, size_t datalen, RIL_Token t) {
    ...
    if (paramCount == 1) {
        // delete
        if (asprintf(&tmp, "AT+EPBUM=3,5,%d,1", params[0]) < 0) {
            goto error;
        }
        ...
        // write
        if (asprintf(&tmp, "AT+EPBUM=2,5,%d,1", params[0]) < 0) {
            goto error;
        }
        ...
        for (i = 1; i < paramCount; i ++) {
            if (asprintf(&tmp, ",%d", params[i]) < 0) {
                goto error;
            }

            if (tmp != NULL) {
                LOGD("sizeof(cmd)= %zu, strlen(cmd)=%zu, strlen(tmp)=%zu",
                        sizeof(cmd), strlen(cmd), strlen(tmp));
...
void requestGetPhoneBookStringsLength(void *data, size_t datalen, RIL_Token t) {
    ...
    err = at_send_command_singleline("AT+EPBUM=0,2,1", "+EPBUM:", &p_response, SIM_CHANNEL_CTX);
...
void requestSetPhoneBookMemStorage(void *data, size_t datalen, RIL_Token t) {
    ...
    if (password != NULL) {
        if(asprintf(&cmd, "AT+EPIN2=\"%s\"", password) < 0) {
            goto error;
        }
...
void requestReadPhoneBookEntryExt(void *data, size_t datalen, RIL_Token t) {
    ...
    for (i = 0, p_cur = p_response->p_intermediates ; p_cur != NULL ; p_cur = p_cur->p_next, i++) {
        ...
        entry->text[strlen(out)] = '\0';
        entry->hidden = 0;
        DLOGD("PhbEntry %d: %s, %d, %s", entry->index, entry->number, entry->type, entry->text);
        // ANR
        if (maxAnrNum[rid] > 0 && (strlen(entry->text) > 0 || strlen(entry->number) > 0)) {
            if (bIsTc1() == true) {
                if (asprintf(&cmd, "at+epbum=1,0,%d,%d", entry->index, 1) <0) {
                    goto error;
                }
            } else {
                if (asprintf(&cmd, "at+epbum=1,0,%d,%d", entry->index, maxAnrNum[rid]) < 0) {
                    goto error;
                }
            }
            ...
            if (p_res != NULL && p_res->success != 0) {
                ...
                p_res = NULL;
                DLOGD("PhbEntry anr %d: %s, %d", entry->index, entry->adnumber, entry->adtype);
            }
        }
        // email
        if (maxEmailNum[rid] > 0 && (strlen(entry->text) > 0 || strlen(entry->number) > 0)) {
            if (bIsTc1() == true) {
                if (asprintf(&cmd, "at+epbum=1,1,%d,%d", entry->index, 1) < 0) {
                    goto error;
                }
            } else {
                if (asprintf(&cmd, "at+epbum=1,1,%d,%d", entry->index, maxEmailNum[rid]) < 0) {
                    goto error;
                }
            }
...
void requestWritePhoneBookEntryExt(void *data, size_t datalen, RIL_Token t) {
    ...
    if (entry->text != NULL || entry->number != NULL) {
        ...
        LOGD("requestWritePhoneBookEntryExt - i = %zu, len = %zu", i, len);
        if (i != len) {
            len /= 4;
            ...
            if (bIsTc1() == true) {
                ...
                LOGD("requestWritePhoneBookEntryExt - sizeof(alphaId) = %zu , strlen(line) = %zu",
                        sizeof(alphaId), strlen(line));
                LOGD("requestWritePhoneBookEntryExt - line    = %s", line);
                LOGD("requestWritePhoneBookEntryExt - alphaId = %s", alphaId);
            } else {
                encodeMethod = CPBW_ENCODE_IRA;
                // ASCII coding
            }
        ...
        requestClearPhbEntry(entry->index, t);
        if (asprintf(&cmd, "AT+EPBW=%d, \"%s\", %d, \"%s\", 0, %d",
                entry->index, number, entry->type, alphaId, encodeMethod) < 0) {
            goto error;
        }
    ...
    if (ret == RIL_E_SUCCESS && maxEmailNum[rid] > 0) {
        if (bIsTc1() == true) {
            if (entry->email != NULL && strlen(entry->email) > 0) {
                ...
                if (asprintf(&cmd, "at+epbum=2,1,%d,%d,\"%s\"", entry->index, 1, entry->email) < 0) {
                    goto error;
                }
            } else {
                if (asprintf(&cmd, "at+epbum=3,1,%d,%d", entry->index, 1) < 0) {
                    goto error;
                }
            }
        } else {
            if (entry->email != NULL && strlen(entry->email) > 0) {
                // at+epbum=<op>,<type>,<adn_index>,<ef_index>,<email>
                if (asprintf(&cmd, "at+epbum=2,1,%d,%d,\"%s\"",
                        entry->index, maxEmailNum[rid], entry->email) < 0) {
                    goto error;
                }
            } else {
                if (asprintf(&cmd, "at+epbum=3,1,%d,%d", entry->index, maxEmailNum[rid]) < 0) {
                    goto error;
                }
            }
        }
...
void requestQueryUPBAvailable(void *data, size_t datalen, RIL_Token t) {
    ...
    if (asprintf(&cmd, "AT+EPBUM=0,%d,%d", params[0], params[1]) < 0) {
        goto error;
    }
...
void requestReadUPBEmail(void *data, size_t datalen, RIL_Token t) {
    ...
    if (maxEmailNum[rid] > 0) {
        if (asprintf(&cmd, "at+epbum=1,1,%d,%d", params[0], params[1]) < 0) {
            goto error;
        }
...
void requestReadUPBSne(void *data, size_t datalen, RIL_Token t) {
    ...
    // SNE
    if (asprintf(&cmd, "at+epbum=1,2,%d,%d", params[0], 1) < 0) {
        goto error;
    }
...
void requestReadUPBAnr(void *data, size_t datalen, RIL_Token t) {
    ...
    if ( maxAnrNum[rid] > 0 ) {
        ...
        if (asprintf(&cmd, "at+epbum=1,0,%d,%d", params[0], params[1]) <0) {
            goto error;
        }
        // +epbum:<type>,<adn_index>,<ef_index>,<anr>,<type>,<aas>
        err = at_send_command_singleline(cmd, "+EPBUM:", &p_response, SIM_CHANNEL_CTX);
...
void requestReadAasList(void *data, size_t datalen, RIL_Token t) {
    ...
    for (i = params[0]; i <= params[1]; i++) {
        if (asprintf(&cmd, "AT+EPBUM=1,3,%d,0", i) < 0) {
            ret = RIL_E_NO_MEMORY;
            goto error;
        }
