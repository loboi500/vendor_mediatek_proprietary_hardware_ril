...
void requestSubsidyLockGetModemStatus(void *data, size_t datalen, RIL_Token t) {
    ...
    err = at_send_command_singleline("AT+ESLBLOB=2", "+ESLBLOB:",
    ...
    if (sprintf(str, "%d", result[1]) < 0 ) {
        LOGE("[Subsidy Lock] sprintf failed");
        goto done;
    }
...
void requestSubsidyLockUpdateSettings(void *data, size_t datalen, RIL_Token t) {
    ...
    if (asprintf(&cmd, "AT+ESLBLOB=1,%d,\"%s\"", strlen(data) + 1, ((char*)data)) < 0) {
        LOGE("[Subsidy Lock] asprintf failed");
        goto done;
    }
