    ...
    if (!success && cpinRetry < 15) {
        ...
        if (pEfIdString) {
            ...
            for (i = 0; i < files_num; i++) {
                ...
                if(efId < 0) {
                    RLOGD("strtol failed, efId error");
                    break;
                }
...
char* decodeStkRefreshFileChange(char *str, int **cmd, int *cmd_length)
{
    ...
    if (file_num == ULONG_MAX) {
        LOGE("decodeStkRefreshFileChange: strtoul fail !!!");
        return NULL;
    }
...
char* buildOpenChannelTRStr(char *cmd) {
    ...
    if (pOpenChannelFirstPart == NULL) {
        LOGE("malloc buildOpenChannelTRStr fail");
        return NULL;
    }
    ...
    *(pOpenChannelFirstPart + 23) = '2';
    ...
    do {
        ...
        if (0x35 == cmdTag || 0xB5 == cmdTag) {
            ...
            if (0 < cmdLen) {
                ...
                if (pBearerDesc == NULL) {
                    LOGE("malloc pBearerDesc fail");
                    if (pOpenChannelFirstPart != NULL) {
                        free(pOpenChannelFirstPart);
                        pOpenChannelFirstPart = NULL;
                    }
                    return NULL;
                }
        ...
        if (0x39 == cmdTag || 0xB9 == cmdTag) {
            ...
            if (0 < cmdLen) {
                ...
                if (NULL == pBufferSize) {
                    LOGE("buildOpenChannelTRStr: pBufferSize is NULL !!!");
                    if (pOpenChannelFirstPart != NULL) {
                        free(pOpenChannelFirstPart);
                        pOpenChannelFirstPart = NULL;
                    }
    ...
    if (NULL == pOpenChannelTR) {
        LOGE("buildOpenChannelTRStr: pOpenChannelTR is NULL !!!");
        if (pOpenChannelFirstPart != NULL) {
            free(pOpenChannelFirstPart);
            pOpenChannelFirstPart = NULL;
        }
...
void onStkProactiveCommand(char* urc, RILChannelCtx* p_channel)
{
    ...
    if (NULL == temp_str) {
        LOGE("onStkProactiveCommand: temp_str is NULL !!!");
        return;
    }
    ...
    while(length > 0) {
        if(str[offset] == '8' && str[offset + 1] == '4') {
            ...
            if(str[offset + 5] == '0') {
                ...
                if (temp == ULONG_MAX) {
                    LOGE("findStkCallDuration: strtoul fail !!!");
                    break;
                }
                duration = temp * 60000;
