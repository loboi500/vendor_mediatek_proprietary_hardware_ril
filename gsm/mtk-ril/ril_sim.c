...
const static char noSim[4] = {"N/A"};
const static char tempSim[2] = {""};
...
static char sCachedIccid[MAX_SIM_COUNT][PROPERTY_VALUE_MAX] = {{0}, {0}, {0}, {0}};

static int uiccEnabledState[MAX_SIM_COUNT] = {-1};
...
void onHandleIccidInd(const char *s, RIL_SOCKET_ID rid);

//IRadio1.5
static void requestEnterSimDepersonalization(void *data, size_t datalen, RIL_Token t);
...
SIM_Status getSIMStatus(RIL_SOCKET_ID rid)
{
    ...
    if (uiccEnabledState[rid] == UICC_APPLICATIONS_ENABLE_STATE_DISABLED) {
        int slot = (1 << getMappingSIMByCurrentMode(rid));
        RLOGI("getSIMStatus uiccEnabled: %d slot: %d rid: %d", uiccEnabledState[rid], slot, rid);
        if (!(slot & sim_inserted_status)) {
            ret = SIM_ABSENT;
            goto done;
        } else {
...
bool getIccIdbySimSlot(char *iccid, RIL_SOCKET_ID rid) {
    ...
    if (strlen(iccIdProp) > 0) {
        ...
        if (strcmp(iccIdProp, "N/A") != 0) {
            if (sprintf(iccid, "%s", sCachedIccid[getMappingSIMByCurrentMode(rid)]) < 0) {
                RLOGE("getIccIdbySimSlot copy string fail");
            }
        }
    }
...
//MTK-END [mtkXXXXX][120208][APLS00109092] Replace "RIL_UNSOL_SIM_MISSING in RIL.java" with "acively query SIM missing status"
    ...
    char iccId[PROPERTY_VALUE_MAX] = {0};
    int uiccEnabled = uiccEnabledState[rid] != UICC_APPLICATIONS_ENABLE_STATE_DISABLED;
    ...
    if (((sim_status == USIM_NOT_READY || sim_status == SIM_NOT_READY) && uiccEnabled) ||
    ...
    // MTK-END
        ...
        num_apps = 1;
    } else if (sim_status == USIM_NOT_READY || sim_status == SIM_NOT_READY) {
        card_state = RIL_CARDSTATE_PRESENT;
        num_apps = 0;
    } else {
        card_state = RIL_CARDSTATE_PRESENT;
    ...
    if (p_card_status->iccId == NULL) {
        RLOGE("getCardStatus, fail to allocate memory");
        free(p_card_status);
        return RIL_E_GENERIC_FAILURE;
    }
...
static void requestGetSimStatus(void *data, size_t datalen, RIL_Token t)
{
    ...
    property_get(PROPERTY_SIM_POWER_STATE[rid], tmp, "");
    RLOGE("getCardStatus sim power state s is %s", tmp);
    if (strlen(tmp) > 0) {
        int state = atoi(tmp);
        if (state == 3 || state == 2) { //sim power off or pass through mode
            ...
            if (!getIccIdbySimSlot(p_card_status->iccId, rid)) {
                RLOGE("getCardStatus no iccid");
            }
            ...
            p_card_status->physicalSlotId = rid;
            p_buffer = (char *)p_card_status;
            buffer_size = sizeof(*p_card_status);
        }
        RLOGE("getCardStatus sim power state d is %d, result:%d", state, result);
    }

    if (result == -1) {
        result = getCardStatus(&p_card_status,rid);
        if (result == RIL_E_SUCCESS) {
            ...
            if (RIL_CARDSTATE_PRESENT == p_card_status->card_state && !s_md_off &&
                    uiccEnabledState[rid] != UICC_APPLICATIONS_ENABLE_STATE_DISABLED) {
...
static RIL_Errno simIo(RIL_SIM_IO_EX_v6 *p_args, RIL_SIM_IO_Response *p_res,RIL_Token t)
{
    ...
    if (p_args->sessionId == 0 && ret == RIL_E_SUCCESS) {
...
static void requestSimAuthentication(void *data, size_t datalen, RIL_Token t) {
    ...
    if (out_put == NULL) {
        err = RIL_E_GENERIC_FAILURE;
        RLOGE("[requestSimAuthentication] base64_decode_to_str return null");
        goto error;
    }
    ...
    */
    ...
    err = sscanf(&(payload[len - 4]), "%02x%02x", &(sim_auth_response.sw1),
        &(sim_auth_response.sw2));
    if (err < 0) {
        RLOGE("requestSimAuthentication sscanf error");
    }
    payload[len - 4] = '\0';
...
static void freeCarrier(RIL_Carrier *carrier, int len) {
    for(int i = 0; i < len; i++) {
...
static void requestGetAllowedCarriers(void *data, size_t datalen, RIL_Token t) {
    ...
    if (allowed_tmp == NULL) {
         RLOGE("requestGetAllowedCarriers, fail to allocate memory");
         goto error;
    }
...
static void requestEnableUiccApplications(void *data, size_t datalen, RIL_Token t) {
    ...
    asprintf(&cmd, "AT+ESIMPOWER=%d", enable);

    err = at_send_command(cmd, &p_response, SIM_CHANNEL_CTX);
    if (err < 0 || NULL == p_response) {
        LOGE("%s() fail: Undesirable", __FUNCTION__);
        ret = RIL_E_OPERATION_NOT_ALLOWED;
    } else if (0 == p_response->success) {
        switch (at_get_cme_error(p_response)) {
             case CME_OPERATION_NOT_ALLOWED_ERR:
                ret = RIL_E_OPERATION_NOT_ALLOWED;
                break;
...
static void requestAreUiccApplicationsEnabled(void *data, size_t datalen, RIL_Token t) {
        ATResponse *p_response = NULL;
        char *cmd = NULL;
        char *line = NULL;
        int err;
        ...
        asprintf(&cmd, "AT+ESIMPOWER?");
        err = at_send_command_singleline(cmd, "+ESIMPOWER:", &p_response, SIM_CHANNEL_CTX);
        ...
        do {
            if (err < 0 || NULL == p_response) {
            ...
            } else if (NULL == p_response->p_intermediates) {
                LOGE("%s() fail: Undesirable 1", __FUNCTION__);
                ret = RIL_E_OPERATION_NOT_ALLOWED;
            } else {
                line = p_response->p_intermediates->line;
                ...
                err = at_tok_start(&line);
                if (err < 0) {
                    LOGE("%s() fail: Undesirable 2", __FUNCTION__);
                    ret = RIL_E_OPERATION_NOT_ALLOWED;
                    break;
                }
                ...
                if (err < 0) {
                    LOGE("%s() fail: Undesirable 3", __FUNCTION__);
                    ret = RIL_E_OPERATION_NOT_ALLOWED;
                    enable = 0;
                } else {
                    if (enable == 10) {
                        ...
                        LOGE("%s() fail: Undesirable 4", __FUNCTION__);
                        ret = RIL_E_OPERATION_NOT_ALLOWED;
                        enable = 0;
                    }
                }
        ...
        RIL_onRequestComplete(t, ret, &enable, sizeof(int));
}

static void onUiccApplicationsEnablementChanged(const char *s, RIL_SOCKET_ID rid) {
    /* if (MtkSimOnOffSupport) {
        return;
    }*/
    char *propertyState = NULL;

    // +ESIMPOWER: 10: Uicc Applications Disabled
    // +ESIMPOWER: 11: Uicc Applications Enabled
    if ((0 == strcmp(s, "+ESIMS: 0,28")) || (0 == strcmp(s, "+ESIMS: 1,29"))) {
        int enable = 0;
        int state = -1;
        if (strStartsWith(s, "+ESIMS: 0,28")) {
            enable = 0;
        ...
        LOGD("%s(): enable = %d", __FUNCTION__, enable);
        uiccEnabledState[rid] = state;
        asprintf(&propertyState, "%d", state);
        property_set(PROPERTY_UICC_APPLICATIONS_ENABLE_STATE[rid], propertyState);
        RIL_UNSOL_RESPONSE(RIL_UNSOL_UICC_APPLICATIONS_ENABLEMENT_CHANGED, &enable, sizeof(int),
                rid);
...
void requestSimMeLockGetSharedKey(void *data, size_t datalen, RIL_Token t) {
    ...
    ATResponse *p_response = NULL;
    int err, key_len;
    char *cmd = NULL, *line = NULL, *key = NULL;
    RIL_Errno ret = RIL_E_SUCCESS;
...
void requestSimMeLockGetSettingsInfo(void *data, size_t datalen, RIL_Token t) {
    ATResponse *p_response = NULL;
    int err, info_len;
    char *cmd = NULL, *line = NULL, *info = NULL;
    RIL_Errno ret = RIL_E_SUCCESS;
...
void requestSimMeLockGetModemStatus(void *data, size_t datalen, RIL_Token t) {
    ...
    ATResponse *p_response = NULL;
    int err, status_len;
    char *cmd = NULL, *line = NULL, *status = NULL;
    RIL_Errno ret = RIL_E_SUCCESS;
...
extern int rilSimMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request)
    {
        ...
        case RIL_REQUEST_ENABLE_UICC_APPLICATIONS:
            requestEnableUiccApplications(data, datalen, t);
            break;
        case RIL_REQUEST_ARE_UICC_APPLICATIONS_ENABLED:
            requestAreUiccApplicationsEnabled(data, datalen, t);
            break;
        case RIL_REQUEST_ENTER_SIM_DEPERSONALIZATION:
            requestEnterSimDepersonalization(data, datalen, t);
            break;
        default:
            return 0; /* no match */
        break;
...
extern int rilSimUnsolicited(const char *s, const char *sms_pdu, RILChannelCtx* p_channel)
{
    ...
    } else if (strStartsWith(s, "+ICCID:")) {
        onHandleIccidInd(s, rid);
    } else if (strStartsWith(s, "+EICCID:")) {
        onHandleIccidInd(s, rid);
    } else {
        return 0;
    }
...
static char* convertBtSapIntToHexString(uint8_t *data, size_t datalen) {
    ...
    for (i = 0; i < datalen; i++) {
        ...
        if (sprintf(pOut, "%02x", data[i]) < 0) {
            RLOGE("[BTSAP] convertBtSapIntToHexString, Copy string fail");
        }
    }
...
AidInfo* getAidInfo(RIL_SOCKET_ID rid, AidIndex index) {
    if (index < 0) {
        index = AID_USIM;
        RLOGE("getAidInfo invalid index");
    }
...
void requestIccId(void *data, size_t datalen, RIL_Token t)
{
    ...
    char *line;
    char *iccId;
    char hashedIccid[PROPERTY_VALUE_MAX] = { 0 };
    RIL_Errno ret = RIL_E_GENERIC_FAILURE;
    RIL_SOCKET_ID rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));
    ...
    stringToUpper(iccId);
    hash256(iccId, hashedIccid, PROPERTY_VALUE_MAX);
    property_set(PROPERTY_ICCID_SIM[rid], hashedIccid);
    RLOGD("requestIccId,rid: %d, iccid: %s", rid, hashedIccid);
    if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "%s", iccId) < 0) {
        RLOGE("requestIccId: failed to save iccid");
    }

    RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, iccId, strlen(iccId), rid);
...
error:
    ...
    if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "N/A") < 0) {
        RLOGE("requestIccId: failed to save iccid N/A");
    }
    RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, noSim, strlen(noSim), rid);
    RIL_onRequestComplete(t, ret, NULL, 0);
    at_response_free(p_response);
}
...
void requestSIM_TransmitBasic(void *data, size_t datalen, RIL_Token t)
{
    ...
    err = sscanf(&(sr.simResponse[len - 4]), "%02x%02x", &(sr.sw1), &(sr.sw2));
    if (err < 0) {
        RLOGE("requestSIM_TransmitBasic sscanf error");
    }
...
void requestSIM_TransmitChannel(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (sscanf(&(sr.simResponse[len - 4]), "%02x%02x", &(sr.sw1), &(sr.sw2)) < 0) {
        LOGE("requestSIM_TransmitChannel sscanf error");
    }
    sr.simResponse[len - 4] = '\0';
...
int requrstSimChannelAccess(RILChannelCtx *p_channel, int sessionid, char* senddata, RIL_SIM_IO_Response* output){
    ...
    if (sscanf(&(output->simResponse[len - 4]), "%02x%02x", &(output->sw1), &(output->sw2)) < 0) {
        RLOGE("requrstSimChannelAccess sscanf error");
    }
...
void simNetworkLock(void *data, size_t datalen, RIL_Token t)
{
    ...
    RLOGI("simNetworkLock: %s,%s/n", strings[0], strings[1]);
    ...
    }else if (0 == strcmp (strings[1],"0") ) //unlock
    {
        ...
        asprintf(&cmd, "AT+ESMLCK=%s,%s,\"%s\"", strings[0], strings[1], key);
         ...
         ret = RIL_E_INVALID_ARGUMENTS;
         RLOGE("simNetworkLock invalid operation %s.", strings[1]);
         goto error;
    }
...
redo:
    *len = 0;
    ...
    if (sscanf(&(sr->simResponse[*len - 4]), "%02x%02x", &(sr->sw1), &(sr->sw2)) < 0) {
        RLOGE("requestCGLA_OpenChannelWithSw sscanf error");
    }
    sr->simResponse[*len - 4] = '\0';
...
static void queryIccId(void *param) {
    ...
    hash256(iccId, hashedIccid, PROPERTY_VALUE_MAX);
    property_set(PROPERTY_ICCID_SIM[rid], hashedIccid);
    RLOGD("queryIccId,rid: %d, iccid: %s", rid, hashedIccid);
    if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "%s", iccId) < 0) {
        RLOGE("queryIccId: failed to save iccid");
    }
    RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, iccId, strlen(iccId), rid);
...
error:
    ...
    if (ret == RIL_E_REQUEST_NOT_SUPPORTED) {
        ...
        property_set(PROPERTY_ICCID_SIM[rid], NULL);
        if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "") < 0) {
            RLOGE("queryIccId: failed to save iccid empty");
        }
        RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, tempSim, strlen(tempSim), rid);
        ...
        property_set(PROPERTY_ICCID_SIM[rid], "N/A");
        if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "N/A") < 0) {
            RLOGE("queryIccId: failed to save iccid N/A");
        }
        RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, noSim, strlen(noSim), rid);
    }
            ...
            if (isSupportC2kSim()) {
                ...
                if (isCDMADetect[rid] == -1) {
                    ...
                    if (isSupportCommonSlot()) {
                        ...
                        property_set(PROPERTY_ICCID_SIM[rid], "N/A");
                        if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "N/A") < 0) {
                            RLOGE("detectSim: failed to save iccid N/A");
                        }
                        RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, noSim, strlen(noSim), rid);
...
static void onCdmaDetected(const char *s, RIL_SOCKET_ID rid) {
    ...
    if ((isSvlteSupport() || isSrlteSupport()) && (isUsimDetect[rid] == 2 &&
        ...
        property_set(PROPERTY_ICCID_SIM[rid], NULL);
        if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "") < 0) {
            RLOGE("onCdmaDetected: failed to save iccid empty");
        }
        RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, tempSim, strlen(tempSim), rid);
...
void onUsimDetected(const char *s, RIL_SOCKET_ID rid) {
    ...
    int simId = getMappingSIMByCurrentMode(rid);
    if (simId < 0) {
        RLOGE("onUsimDetected fail: simId=%d/n", simId);
        return;
    }
...
void onSimInsertChanged(const char *s,RIL_SOCKET_ID rid)
{
    ...
    if ((supportC2k == 1) && strStartsWith(s, "+ESIMS: 0,11")) {
        ...
        if (param != NULL) {
            ...
            RLOGE("onSimInsertChanged, fail to allocate memory");
        }
    }
    ...
    if (strStartsWith(s, "+ESIMS: 0,0") || strStartsWith(s, "+ESIMS: 0,13") || strStartsWith(s, "+ESIMS: 0,10") || strStartsWith(s, "+ESIMS: 0,11") || strStartsWith(s, "+ESIMS: 0,15")) {
        ...
        if (strStartsWith(s, "+ESIMS: 0,11") || strStartsWith(s, "+ESIMS: 0,13") || strStartsWith(s, "+ESIMS: 0,10")) {
            ...
            property_set(PROPERTY_ICCID_SIM[rid], "N/A");
            if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "N/A") < 0) {
                RLOGE("onSimInsertChanged: failed to save iccid N/A");
            }
            RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, noSim, strlen(noSim), rid);

            char *propertyState = NULL;
            int enable = 1;
            int state = UICC_APPLICATIONS_ENABLE_STATE_ENABLED;
            uiccEnabledState[rid] = state;
            asprintf(&propertyState, "%d", state);
            property_set(PROPERTY_UICC_APPLICATIONS_ENABLE_STATE[rid], propertyState);
            free(propertyState);
    ...
    // MTK-END
        ...
        if (getSimCount()>= 2) {
            ...
            if(isSupportCommonSlot() == 1 && (strStartsWith(s, "+ESIMS: 1,12"))) {
                ...
                // External SIM [End]
                    ...
                    property_set(PROPERTY_ICCID_SIM[rid], NULL);
                    if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "") < 0) {
                        RLOGE("onSimInsertChanged: failed to save iccid empty");
                    }
                    RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, tempSim, strlen(tempSim), rid);
        ...
        RIL_requestProxyTimedCallback (detectSim, param, &TIMEVAL_PIN_SIM,
                getRILChannelId(RIL_SIM, rid), "detectSim");
    } else if (strStartsWith(s, "+ESIMS: 0,28") || strStartsWith(s, "+ESIMS: 1,29")) {
        property_set(PROPERTY_SIM_POWER_STATE[rid], "-1");
        onUiccApplicationsEnablementChanged(s, rid);
...
void requestSetSimCardPower(void *data, size_t datalen, RIL_Token t) {
    ...
    if (err < 0 || NULL == p_response || at_get_cme_error(p_response) != CME_SUCCESS) {
        ...
        RIL_SOCKET_ID rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));
        char *propertyState = NULL;
        asprintf(&propertyState, "%d", mode);
        property_set(PROPERTY_SIM_POWER_STATE[rid], propertyState);
        free(propertyState);
        sleepMsec(500);
    }
...
void requestGeneralSimAuth(void *data, size_t datalen, RIL_Token t)
{
    ...
    } else if (auth->mode == 0 || auth->mode == 1) {
        err = at_send_command_singleline(cmd, "+ESIMAUTH:", &p_response, SIM_CHANNEL_CTX);
    }
    if (cmd != NULL) {
        free(cmd);
    ...
    if (isTestSim[simId] == 1) {
        if (eraseCdmaCapabilityMode() != 5) {
            ...
            if (param != NULL) {
                ...
                RLOGE("onOp016mCardDetected, fail to allocate memory");
            }
        } else {
            modifyCdmaCardType(simId);
            handleOp016mCardType(rid);
...
void onHandleIccidInd(const char *s, RIL_SOCKET_ID rid) {
    ...
    if ((state = 1) && (at_tok_hasmore(&line) >= 0)) {
        err = at_tok_nextstr(&line, &iccId);
        if (err < 0) {
            goto error;
        }
        ...
        hash256(iccId, hashedIccid, PROPERTY_VALUE_MAX);
        property_set(PROPERTY_ICCID_SIM[rid], hashedIccid);
        RLOGD("onHandleIccidInd, rid: %d, iccid: %s", rid, hashedIccid);
        if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "%s", iccId) < 0) {
            RLOGE("onHandleIccidInd: failed to save iccid");
        }
        RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, iccId, strlen(iccId), rid);
    } else {
        property_set(PROPERTY_ICCID_SIM[rid], "N/A");
        if (snprintf(sCachedIccid[rid], PROPERTY_VALUE_MAX, "N/A") < 0) {
            RLOGE("onHandleIccidInd: failed to save iccid N/A");
        }
        RIL_UNSOL_RESPONSE(RIL_UNSOL_ICCID_CHANGED, noSim, strlen(noSim), rid);
    }
    ...
    return;
error:
    RLOGE("onHandleIccidInd(), fail to get iccid!");
}
...
void hash256(const char *input, char *output, int outputLength) {
    unsigned char hash[SHA256_DIGEST_LENGTH] = {0};
    char temp[SHA256_DIGEST_LENGTH * 2 + 1] = {0};
    SHA256_CTX sha256;
    ...
    for(int i = 0; i < SHA256_DIGEST_LENGTH; i++) {
        if (sprintf(temp + i * 2, "%02x", hash[i]) < 0) {
            RLOGE("hash256: failed to copy character");
        }
    }
    if (snprintf(output, outputLength, "%s", temp) < 0) {
        RLOGE("hash256: failed to copy string");
    }
    return;
}
 ...
 */
static int convertPersoStateToCat(int perso_substate) {
    int cat = -1;
    switch (perso_substate) {
        ...
        case RIL_PERSOSUBSTATE_SIM_NS_SP_PUK:
            cat = 5;
            break;
 ...
 *         -1 : unknown
 */
#ifndef MAX_SIM_ME_LOCK_CAT_NUM
#define MAX_SIM_ME_LOCK_CAT_NUM 7
#endif
static int getPersoRemainRetry(int cat, RIL_Token t) {
    ATResponse*     p_response = NULL;
    int             err = -1;
    char            *line;
    int             i = 0;
    ...
    if (cat < 0 || cat >= MAX_SIM_ME_LOCK_CAT_NUM) {
        RLOGE("getPersoRemainRetry: invalid cat %d", cat);
        return retryCount;
    }
    ...
    err = at_send_command_singleline("AT+ESMLCK", "+ESMLCK:", &p_response, SIM_CHANNEL_CTX);
    if (err < 0 || p_response->success == 0) {
        RLOGE("getPersoRemainRetry AT+ESMLCK error!");
        goto error;
    }
    ...
    err = at_tok_start(&line);
    if (err < 0) {
        RLOGE("getPersoRemainRetry at_tok_start error!");
        goto error;
    }
    ...
    for (i=0; i<=cat; i++)
    {
        err = at_tok_char(&line);
        if (err < 0) {
            RLOGE("getPersoRemainRetry parse entry %d error!", i);
            break;
        }
...
error:
    ...
    RLOGD("getPersoRemainRetry retryCount = %d", retryCount);
    return retryCount;

}

/**
 * If current sim status is locked by the perso_substate.
 *
 * return:
 *  0 -- not match
 *  1 -- match
 */
static int isPersoSubStateMatchSimStatus(int perso_substate, int sim_status) {
    int ret = 0;
    switch (perso_substate) {
        case RIL_PERSOSUBSTATE_SIM_NETWORK:
        case RIL_PERSOSUBSTATE_SIM_NETWORK_PUK:
            if (sim_status == SIM_NP || sim_status == USIM_NP) {
                ret = 1;
            }
            break;
        case RIL_PERSOSUBSTATE_SIM_NETWORK_SUBSET:
        case RIL_PERSOSUBSTATE_SIM_NETWORK_SUBSET_PUK:
            if (sim_status == SIM_NSP || sim_status == USIM_NSP) {
                ret = 1;
            }
            break;
        case RIL_PERSOSUBSTATE_SIM_SERVICE_PROVIDER:
        case RIL_PERSOSUBSTATE_SIM_SERVICE_PROVIDER_PUK:
            if (sim_status == SIM_SP || sim_status == USIM_SP) {
                ret = 1;
            }
            break;
        case RIL_PERSOSUBSTATE_SIM_CORPORATE:
        case RIL_PERSOSUBSTATE_SIM_CORPORATE_PUK:
            if (sim_status == SIM_CP || sim_status == USIM_CP) {
                ret = 1;
            }
            break;
        case RIL_PERSOSUBSTATE_SIM_SIM:
        case RIL_PERSOSUBSTATE_SIM_SIM_PUK:
            if (sim_status == SIM_SIMP || sim_status == USIM_SIMP) {
                ret = 1;
            }
            break;
        case RIL_PERSOSUBSTATE_SIM_NS_SP:
...
static void requestEnterSimDepersonalization(void *data, size_t datalen, RIL_Token t)
{
    ...
    RIL_SOCKET_ID           rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));
    SIM_Status  sim_status;
    int perso_substate = 0;
    int cat = 0;
    int retry_count = -1;
    RIL_Errno     ret = RIL_E_MODEM_ERR;
    int response_data[2];
    int isCurrentLock = 0;

    RIL_SIM_UNUSED_PARM(datalen);
    RLOGD("handleEnterSimDepersonalization data=%p, datalen=%d",
            data, (int)datalen);

    if (strings != NULL) {
        RLOGD("handleEnterSimDepersonalization data[0]=%p, data[1]=%p",
                strings[0], strings[1]);
        if (strings[0] != NULL) {
            RLOGD("handleEnterSimDepersonalization data[0]=%s",
                    strings[0]);
        }
        if (strings[1] != NULL) {
            RLOGD("handleEnterSimDepersonalization data[1]=%s",
                    strings[1]);
        }
    }
    ...
    if (strings == NULL || strings[0] == NULL || strings[1] == NULL) {
         ret = RIL_E_INVALID_ARGUMENTS;
         RLOGE("handleEnterSimDepersonalization invalid arguments.");
         goto error;
    }

    sim_status = getSIMStatus(rid);
    RLOGD("handleEnterSimDepersonalization perso_substate : %s, status: %d",
            strings[1], sim_status);
    key = (strings[0] == NULL) ? (char*)"" : (char*)strings[0];
    perso_substate = atoi(strings[1]);
    if (perso_substate < RIL_PERSOSUBSTATE_SIM_NETWORK ||
            perso_substate > RIL_PERSOSUBSTATE_SIM_NS_SP_PUK) {
        ret = RIL_E_INVALID_ARGUMENTS;
        RLOGE("handleEnterSimDepersonalization invalid persoState.");
        goto error;
    }
    cat = convertPersoStateToCat(perso_substate);
    if (cat == -1) {
        ret = RIL_E_REQUEST_NOT_SUPPORTED;
        RLOGE("handleEnterSimDepersonalization not supported persoState.");
        goto error;
    }

    // Sim status will update on the current lock is depersonalized.
    isCurrentLock = isPersoSubStateMatchSimStatus(perso_substate, sim_status);
    if (isCurrentLock == 1) {
        asprintf(&cmd, "AT+CPIN=\"%s\"", key);
        if (cmd == NULL) {
            RLOGE("cmd is NULL.");
        ...
        asprintf(&cmd, "AT+ESMLCK=%d,%s,\"%s\"", cat, "0", key);
        if (cmd == NULL) {
            RLOGE("cmd is NULL.");
            ret = RIL_E_GENERIC_FAILURE;
            goto error;
    ...
    if (err < 0 || NULL == p_response) {
        ...
        RLOGE("handleEnterSimDepersonalization send AT error: %d", err);
        goto error;
    }

    if (p_response->success == 0) {
        RLOGE("Response with err = %d", at_get_cme_error(p_response) );
        switch (at_get_cme_error(p_response)) {
            case CME_INCORRECT_PASSWORD:
                ret = RIL_E_PASSWORD_INCORRECT;
                break;
        ...
        ret = RIL_E_SUCCESS;
        if (isCurrentLock == 1) {
            RLOGD("handleEnterSimDepersonalization: unlock current state");
            setSimStatusChanged(rid);
            //TODO: need to check. Dynamic-SBP feature: ALPS02832079
            RLOGD("Setup Dynamic-SBP for RIL:%d", rid);
            setupDynamicSBP(rid);
        }
    }
...
error:
    at_response_free(p_response);
    p_response = NULL;
    retry_count = getPersoRemainRetry(cat, t);
    response_data[0] = perso_substate;
    response_data[1] = retry_count;
    RLOGD("handleEnterSimDepersonalization: ret: %d, retry:%d", ret, retry_count);
    RIL_onRequestComplete(t, ret, &response_data, sizeof(response_data));
}
