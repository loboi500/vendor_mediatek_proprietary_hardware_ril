...
RIL_Errno handleERATResultPlus(const char* cmd, RIL_Token t, int request) {
    int err = -1;
    RIL_Errno ril_errno = (request == RIL_REQUEST_SET_ALLOWED_NETWORK_TYPES_BITMAP) ? RIL_E_REQUEST_NOT_SUPPORTED : RIL_E_SUCCESS;
    ATResponse *p_response = NULL;
...
void requestSetPreferredNetworkType(void * data, size_t datalen, RIL_Token t, int request)
{
    RIL_SOCKET_ID rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));
    ...
    if(rat >= 0) {
        ...
        if (err >= 0) {
            ...
            ril_errno = handleERATResultPlus(cmd, t, request);
    ...
    */
    ...
    for(i=0 ; i < num ; i++) {
        ...
        if (mcc != INVALID && mnc != INVALID) {
            ...
            int len = -1;
            if (mnc_len == 2) {
                len = sprintf(mccmnc_string, "%03d%02d", mcc, mnc);
            } else {
                len = sprintf(mccmnc_string, "%03d%03d", mcc, mnc);
            }
            if (len < 0) {
                LOGE("getCellInfoListV12 sprintf mccmnc_string fail");
            }
            err = getOperatorNamesFromNumericCode(
...
void handleSimulatedNetworkScan() {
    ATResponse *p_response = NULL;
    RIL_CellInfo_v12 *p_network_infos = NULL;
    RIL_SOCKET_ID rid = getRILIdByChannelCtx(getChannelCtxbyProxy());
    LOGD("%s rid:%d", __FUNCTION__, rid);
    RIL_NetworkScanResult resp;
    resp.status = COMPLETE;
    resp.network_infos_length = 0;
    resp.network_infos = NULL;
    int num = 0;
    int err = at_send_command_singleline("AT+COPS=?", "+COPS:", &p_response, getChannelCtxbyProxy());
    if (err < 0 || p_response->success == 0) {
        goto error;
    }
    ...
    resp.network_infos_length = num;
    p_network_infos = (RIL_CellInfo_v12*)calloc(num, sizeof(RIL_CellInfo_v12));
    if (p_network_infos == NULL) {
        LOGE("%s malloc fail", __FUNCTION__);
        goto error;
    }
    resp.network_infos = p_network_infos;
    // +COPS: (2,"Far EasTone","FET","46601",0),(...),...,,(0, 1, 3),(0-2)
    err = at_tok_start(&line);
    if (err < 0) goto error;
    ...
    const int INVALID_SIG = 0x7FFFFFFF;
    for (int i = 0, mcc = 0, mnc = 0, mnc_len = 0, rat = 0; i < num; i++) {
        lac = 0;
        err = at_tok_nextstr(&line, &tmp);
        if (err < 0) goto error;
        if (tmp[1] == '2') {
            resp.network_infos[i].registered = 1;
            resp.network_infos[i].connectionStatus = PRIMARY_SERVING;
        } else {
            resp.network_infos[i].registered = 0;
            resp.network_infos[i].connectionStatus = NONE_SERVING;
        }
        ...
        if (err < 0) {
            LOGE("%s snprintf mccmnc fail", __FUNCTION__);
        }
        ...
        if (plmn_list_format[rid] == 1) {
            ...
            err = at_tok_nextstr(&line, &tmp);
            if (err < 0){
                LOGE("%s: No <lac> in +COPS response", __FUNCTION__);
                goto error;
            }
        ...
        err = getOperatorNamesFromNumericCode(mccmnc, longname, shortname,
                                              MAX_OPER_NAME_LENGTH, rid);
        if (err < 0) goto error;
        switch(rat) {
        case 0:
            resp.network_infos[i].cellInfoType = RIL_CELL_INFO_TYPE_GSM;
            resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.mcc = mcc;
            resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.mnc = mnc;
            resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.mnc_len = mnc_len;
            resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.lac = lac;
            asprintf(&(resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.operName.long_name),
                     "%s", longname);
            asprintf(&(resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.operName.short_name),
                     "%s", shortname);
            resp.network_infos[i].CellInfo.gsm.signalStrengthGsm.signalStrength = 99;
            resp.network_infos[i].CellInfo.gsm.signalStrengthGsm.bitErrorRate = 99;
            resp.network_infos[i].CellInfo.gsm.signalStrengthGsm.timingAdvance = INVALID_SIG;
            break;

        case 2:
            resp.network_infos[i].cellInfoType = RIL_CELL_INFO_TYPE_WCDMA;
            resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.mcc = mcc;
            resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.mnc = mnc;
            resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.mnc_len = mnc_len;
            resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.lac = lac;
            asprintf(&(resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.operName.long_name),
                     "%s", longname);
            asprintf(&(resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.operName.short_name),
                     "%s", shortname);
            resp.network_infos[i].CellInfo.wcdma.signalStrengthWcdma.signalStrength = 99;
            resp.network_infos[i].CellInfo.wcdma.signalStrengthWcdma.bitErrorRate = 99;
            resp.network_infos[i].CellInfo.wcdma.signalStrengthWcdma.rscp = 255;
            resp.network_infos[i].CellInfo.wcdma.signalStrengthWcdma.ecno = 255;
            break;

        case 7:    //for  LTE
            resp.network_infos[i].cellInfoType = RIL_CELL_INFO_TYPE_LTE;
            resp.network_infos[i].CellInfo.lte.cellIdentityLte.mcc = mcc;
            resp.network_infos[i].CellInfo.lte.cellIdentityLte.mnc = mnc;
            resp.network_infos[i].CellInfo.lte.cellIdentityLte.mnc_len = mnc_len;
            resp.network_infos[i].CellInfo.lte.cellIdentityLte.tac = lac;
            asprintf(&(resp.network_infos[i].CellInfo.lte.cellIdentityLte.operName.long_name),
                     "%s", longname);
            asprintf(&(resp.network_infos[i].CellInfo.lte.cellIdentityLte.operName.short_name),
                     "%s", shortname);
            resp.network_infos[i].CellInfo.lte.signalStrengthLte.signalStrength = 99;
            resp.network_infos[i].CellInfo.lte.signalStrengthLte.rsrp = INVALID_SIG;
            resp.network_infos[i].CellInfo.lte.signalStrengthLte.rsrq = INVALID_SIG;
            resp.network_infos[i].CellInfo.lte.signalStrengthLte.rssnr = INVALID_SIG;
            resp.network_infos[i].CellInfo.lte.signalStrengthLte.cqi = INVALID_SIG;
            resp.network_infos[i].CellInfo.lte.signalStrengthLte.timingAdvance = INVALID_SIG;
            break;

        default:
            resp.network_infos[i].cellInfoType = RIL_CELL_INFO_TYPE_NONE;
            goto error;
        }
    }

    RIL_UNSOL_RESPONSE(RIL_UNSOL_NETWORK_SCAN_RESULT, &resp,  sizeof(RIL_NetworkScanResult), rid);
    LOGD("%s success, rid:%d, num:%d", __FUNCTION__, rid, num);
    goto final;
...
final:
    if (p_response != NULL) at_response_free(p_response);
    if (num > 0 && resp.network_infos != NULL) {
        for (int i = 0; i < num; i++) {
            switch (resp.network_infos[i].cellInfoType) {
            case RIL_CELL_INFO_TYPE_GSM:
                if (resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.operName.long_name)
                    free(resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.operName.long_name);
                if (resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.operName.short_name)
                    free(resp.network_infos[i].CellInfo.gsm.cellIdentityGsm.operName.short_name);
                break;
            case RIL_CELL_INFO_TYPE_WCDMA:
                if (resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.operName.long_name)
                    free(resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.operName.long_name);
                if (resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.operName.short_name)
                    free(resp.network_infos[i].CellInfo.wcdma.cellIdentityWcdma.operName.short_name);
                break;

            case RIL_CELL_INFO_TYPE_LTE:
                if (resp.network_infos[i].CellInfo.lte.cellIdentityLte.operName.long_name)
                    free(resp.network_infos[i].CellInfo.lte.cellIdentityLte.operName.long_name);
                if (resp.network_infos[i].CellInfo.lte.cellIdentityLte.operName.short_name)
                    free(resp.network_infos[i].CellInfo.lte.cellIdentityLte.operName.short_name);
                break;
    ...
    if (p_network_infos != NULL) free(p_network_infos);
}
...
void requestStartNetworkScan(void * data, size_t datalen __unused, RIL_Token t) {
    ...
    LOGD("requestStartNetworkScan:: type: %d", p_args->type);
    RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
    RIL_requestProxyTimedCallback(handleSimulatedNetworkScan, NULL, &TIMEVAL_0,
                                  getChannelCtxbyProxy()->id, "handleSimulatedNetworkScan");
    return;
}
...
void onNitzTzReceived(char *urc, const RIL_SOCKET_ID rid)
{
    ...
    if(strlen(urc) > 0)
    {
        ...
        *line = '\0';
        ...
        len = sprintf(nitz_string, "%02d/%02d/%02d,%02d:%02d:%02d%s",  //"yy/mm/dd,hh:mm:ss"
                (t_info->tm_year)%100,
                t_info->tm_mon+1,
                t_info->tm_mday,
                t_info->tm_hour,
...
int hexCharToIntNw(char c) {
    if (c >= '0' && c <= '9') return (c - '0');
    if (c >= 'A' && c <= 'F') return (c - 'A' + 10);
    if (c >= 'a' && c <= 'f') return (c - 'a' + 10);

    LOGE("[%s] invalid char:%c", __FUNCTION__, c);
    return -1;
}

int hexCharsToUcs2Encode(char* src) {
    int result = 0;
    for (unsigned int i = 0; i < 4; i++) {
        int tmp = hexCharToIntNw(src[i]);
        if (tmp < 0) return -1;
...
void convertToUtf8String(char *src, size_t n) {
    int len = strlen(src);
    char buf[MAX_OPER_NAME_LENGTH] = {0};

    if (len % 4 != 0) {
        LOGE("[%s] invalid hex char length:%d", __FUNCTION__, len);
    }
...
void onNitzOperNameReceived(char* urc, const RIL_SOCKET_ID rid)
{
    ...
    strncpy(ril_nw_nitz_oper_code[rid], oper_code, MAX_OPER_NAME_LENGTH - 1);
    strncpy(ril_nw_nitz_oper_lname[rid], oper_lname, MAX_OPER_NAME_LENGTH - 1);
    strncpy(ril_nw_nitz_oper_sname[rid], oper_sname, MAX_OPER_NAME_LENGTH - 1);
...
void isFemtoCell(int regState, int cid, int act, int rid) {
    ...
    if (rid < 0) {
        LOGE("isFemtoCell, rid isn't correct value");
        return;
    }
...
extern int rilNwMain(int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    case RIL_REQUEST_SET_ALLOWED_NETWORK_TYPES_BITMAP:
        requestSetPreferredNetworkType(data, datalen, t, request);
        break;
...
void onSib16TimeInfoReceived(char* urc, const RIL_SOCKET_ID rid)
{
    ...
    if (dst != -1) {
        ...
        len = sprintf(nitz_string, "%02d/%02d/%02d,%02d:%02d:%02d%s,%s",  //"yy/mm/dd,hh:mm:ss(+/-)tz[,dst]"
                (ts->tm_year)%100,
                ts->tm_mon+1,
                ts->tm_mday,
                ts->tm_hour,
        ...
        len = sprintf(sib16_time_string, "%02d/%02d/%02d,%02d:%02d:%02d%s,%s,%lli",
                (ts->tm_year)%100,
                ts->tm_mon+1,
                ts->tm_mday,
                ts->tm_hour,
                ...
                abs_time);
        if (len < -1) {
            LOGE("format sib16_time_string fail");
        }
    } else {
        //nitz_srting without dst
        len = sprintf(nitz_string, "%02d/%02d/%02d,%02d:%02d:%02d%s",  //"yy/mm/dd,hh:mm:ss(+/-)tz"
                (ts->tm_year)%100,
                ts->tm_mon+1,
                ts->tm_mday,
                ts->tm_hour,
        ...
        // "yy/mm/dd,hh:mm:ss(+/-)tz,dst,abs_time"
        len = sprintf(sib16_time_string, "%02d/%02d/%02d,%02d:%02d:%02d%s,0,%lli",  //"yy/mm/dd,hh:mm:ss(+/-)tz"
                (ts->tm_year)%100,
                ts->tm_mon+1,
                ts->tm_mday,
                ts->tm_hour,
