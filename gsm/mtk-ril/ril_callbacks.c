...
/* logic modem id
 **/
char s_logicalModemId[RIL_SOCKET_NUM][MAX_UUID_LENGTH];
...
static void onRequest(int request, void *data, size_t datalen, RIL_Token t, RIL_SOCKET_ID socket_id __unused)
{
    ...
    if (radioState == RADIO_STATE_OFF &&
        !(request == RIL_REQUEST_RADIO_POWER ||
          ...
          request == RIL_REQUEST_ENABLE_UICC_APPLICATIONS ||
          request == RIL_REQUEST_ARE_UICC_APPLICATIONS_ENABLED ||
          // MTK-START: SIM GBA
          request == RIL_REQUEST_GENERAL_SIM_AUTH ||
          // MTK-END
          ...
          request == RIL_REQUEST_SET_ALLOWED_NETWORK_TYPES_BITMAP ||
          ...
          request == RIL_REQUEST_ENABLE_MODEM ||
          request == RIL_REQUEST_RESTART_RILD ||
          request == RIL_REQUEST_SET_MODEM_CONFIG ||
          request == RIL_REQUEST_ENTER_SIM_DEPERSONALIZATION
         )) {
...
/* M: End - abnormal event logging for logger */
    ...
    /// M: SIM: Enable +ICCID URC
    at_send_command("AT+ICCID=1", NULL, pChannel);
    ...
    char onoffConfig[PROPERTY_VALUE_MAX] = { 0 };
    property_get(PROPERTY_SIM_CARD_ONOFF, onoffConfig, "0");
    err = at_send_command_singleline("AT+ESIMPOWER?", "+ESIMPOWER:", &p_response, pChannel);
    if (err >= 0 && p_response->success != 0) {
        property_set(PROPERTY_SIM_ONOFF_SUPPORT, "1");
        if ((atoi(onoffConfig) != 1) && (atoi(onoffConfig) != 2)) {
            property_set(PROPERTY_UICC_APPLICATIONS_ENABLE_SUPPORT, "1");
        } else {
            property_set(PROPERTY_UICC_APPLICATIONS_ENABLE_SUPPORT, "0");
        }
    } else {
        property_set(PROPERTY_UICC_APPLICATIONS_ENABLE_SUPPORT, "0");
    }
...
void switchMuxPath() {
    ...
    if (SIM_COUNT >= 2) {
        ...
        int targetSim = 0;
        unsigned int i;
        unsigned int buf_size = RIL_SUPPORT_CHANNELS_MAX_NAME_LEN;
        ...
        if (targetSim*RIL_CHANNEL_OFFSET > getSupportChannels()) {
            ...
            for(i = 0; i < RIL_CHANNEL_OFFSET; i++) {
                unsigned int temp = (targetSim - 1) * RIL_CHANNEL_OFFSET + i;
...
void sendRadioCapabilityDoneIfNeeded () {
    ...
    if (rc != NULL) {
        ...
        RLOGD("sendRadioCapabilityDoneIfNeeded, malloc error");
        assert(1);
    }
}
...
void setSimSwitchProp(int SimId)
{
    ...
    int tmpCapability;
    unsigned int oldMajorSimRid = RIL_get3GSIM() - 1;
    ...
    if ((oldMajorSimRid != (SimId - 1)) && (oldMajorSimRid >= 0)) {
        ...
        if (SimId != 1) {
            ...
            s_logicalModemId[(unsigned int)(SimId) - 1][MAX_UUID_LENGTH - 1] = '\0';
        }
    }
    ...
    if (dir) {
        ...
        while ((de = readdir(dir)) != 0) {
            ...
            if (res == 1) {
                ...
                ret = sprintf(cmdline_file, "%s/%d/cmdline", directory, pid);
                if (ret < 0) {
                    free(taskName);
                    closedir(dir);
                    RLOGE("cmdline_file sprintf fail");
                    return 0;
                }
                ...
                if (cmdline == NULL) {
                    free(taskName);
                    closedir(dir);
                    RLOGE("cmdline_file open file fail");
                    return 0;
                }
