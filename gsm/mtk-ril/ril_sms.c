...
bool rild_sms_deliver_pdu_check(int pdu_len, char *pdu, TPDU_ERROR_CAUSE_ENUM *error_cause) {
    ...
    int pdu_array[pdu_len/2];
    int i=0;
    bool has_user_data_header;
    unsigned char udhl, user_data_header_length;
    ...
    *error_cause = TPDU_NO_ERROR;
    ...
    if (((pdu_array[off] & TPDU_MTI_BITS) != TPDU_MTI_DELIVER) &&
        ...
        LOGE("first octet check fail");
        return false;
    }

    /* check the user data header */
    has_user_data_header = ((pdu_array[off] & 0x40) == 0x40) ? true : false;
    if (has_user_data_header) {
        LOGD("has user data header");
    }
    ...
    if (user_data_len > TPDU_ONE_MSG_OCTET || (user_data_len != (pdu_len/2 - off))) {
        LOGE("user data length check fail, user_data_len=%d", user_data_len);
        *error_cause = TPDU_MSG_LEN_EXCEEDED;
        return false;
    }
    ...
    if (user_data_len > 0 && has_user_data_header) {
        udhl = pdu_array[off++];
        user_data_header_length = (unsigned char) rild_sms_msg_len_in_octet(dcs, udhl);
        if (user_data_header_length > (pdu_len/2 - off)) {
           LOGE("user data header length check fail, user_data_header_length=%d remain=%d",
