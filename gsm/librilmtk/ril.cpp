...
void RIL_setRilSocketName(const char * s) {
    strncpy(rild, s, MAX_SERVICE_NAME_LENGTH-1);
}
...
static int responseCallList(Parcel &p, void *response, size_t responselen) {
    ...
    for (int i = 0 ; i < num ; i++) {
        ...
        writeStringToParcel(p, p_cur->forwardedNumber);
    ...
    if (s_callbacks.version <= LAST_IMPRECISE_RIL_VERSION) {
        ...
        if (responselen % sizeof(RIL_SimRefreshResponse_v7) != 0) {
            ...
            if (!isDebuggable()) {
                ...
                RLOGE("Data structure expected is RIL_SimRefreshResponse_v7, fail");
                assert(0);
            }
        }
        responseSimRefreshV7(p, response);
...
static int at_tok_nextint_base(char **p_cur, int *p_out, int base, int  uns)
{
    ...
    if (ret == NULL) {
        ...
        if (l < 0) {
            RLOGD("strtoul or strtol failed");
            return -1;
        }
        *p_out = (int)l;
...
static int handleSpecialRequestWithArgs(int argCount, char** args){
    ...
    {
        line = args[0];
        if (strlen(args[0]) > 768) {
            LOGE("args overflow, larger than org_args");
            goto error;
        }
...
static void vsimCallback (int fd, short flags, void *param)
{
    ...
    while (true) {
        ...
        p_rs = record_stream_new(p_info->fdCommand, MAX_COMMAND_BYTES);
        if (p_rs == NULL) {
            LOGE ("[vsimCallback] record_stream_new OOM");
            free(p_info);
            p_info = NULL;
            return;
        }
...
static int findFd(int socket_id) {
    int fd = s_ril_param_socket[(unsigned int)socket_id].fdCommand;
    return fd;
}
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_ENTER_SIM_DEPERSONALIZATION: return "RIL_REQUEST_ENTER_SIM_DEPERSONALIZATION";
