...
#include "RmmRcsDataThreadController.h"
...
#define RFX_LOG_TAG "RmmRcsDataThrd"

RmmRcsSharedMemory::RmmRcsSharedMemory(void) {
...
RmmRcsSharedMemory::~RmmRcsSharedMemory() {
    if (mData != NULL) {
        free(mData);
...
void RmmRcsSharedMemory::setState(int state) {
...
int RmmRcsSharedMemory::getState() {
    return mDataReadyCount;
}

bool RmmRcsSharedMemory::checkState(int want_state) {

    if (mDataReadyCount == want_state) {
        if (RmmRcsDataThreadController::isLogEnable()) {
...
void RmmRcsSharedMemory::setSize(int size) {
    mDatalen = size;
}

void RmmRcsSharedMemory::setData(char* data, int len) {
...
int RmmRcsSharedMemory::getSize(void) {
    return mDatalen;
}

void RmmRcsSharedMemory::getData(char** data) {
    (*data) = mData;
}

void RmmRcsSharedMemory::clearData() {
    free(mData);
}

void RmmRcsSharedMemory::lock(const char* user) {
...
void RmmRcsSharedMemory::unlock(const char* user) {
...
void RmmRcsSharedMemory::wait(const char* user, int stay_state) {
...
void RmmRcsSharedMemory::signal(const char* user) {
...
sp<RmmRcsSharedMemory> RmmRcsDataThreadController::sShareMemory = new RmmRcsSharedMemory();
pthread_t  RmmRcsDataThreadController::sShareMemoryThd = 0;
pthread_t  RmmRcsDataThreadController::sRilServerThd = 0;
pthread_mutex_t RmmRcsDataThreadController::sMutex = PTHREAD_MUTEX_INITIALIZER;
Ril_Channel_Obj_t *RmmRcsDataThreadController::sChannelObj = NULL;
RmmRcsDataThreadController *RmmRcsDataThreadController::sInstance = NULL;

RmmRcsDataThreadController::RmmRcsDataThreadController() {
}

RmmRcsDataThreadController:: ~RmmRcsDataThreadController() {
}

sp<RmmRcsSharedMemory> RmmRcsDataThreadController::getSharedMem(void) {
    return sShareMemory;
}

void RmmRcsDataThreadController::init() {
    pthread_mutex_lock(&sMutex);
    if (sInstance == NULL) {
        RFX_LOG_I(RFX_LOG_TAG, "RmmRcsDataThreadController init, create instance");
        sInstance = new RmmRcsDataThreadController();
        startThread();
    }
    pthread_mutex_unlock(&sMutex);
}

void RmmRcsDataThreadController::startThread() {
...
void* RmmRcsDataThreadController::rilServerThread(void *arg) {
...
void RmmRcsDataThreadController::processUaEvent(Ril_Channel_Client_t *client) {
...
int RmmRcsDataThreadController::getClientIndex(int account, int uaType) {
    if (!sChannelObj) {
        return -1;
    }
    if (!sChannelObj->channels) {
...
void * RmmRcsDataThreadController::shareMemoryThread(void *arg) {
...
int RmmRcsDataThreadController::sendDataToUA(UaChannel_Data_t *data, int account, int uaType) {
...
int RmmRcsDataThreadController::fsend(int fd, char *data, int len) {
    int size = 0, ret = 0;
    int write_size = 0;
    int local_errno;
...
void RmmRcsDataThreadController::sendMsgToShareMemory(char* outBuffer, int length, const char* user) {
    sp<RmmRcsSharedMemory> Mem  = getSharedMem();
...
bool RmmRcsDataThreadController::isLogEnable() {
