...
#ifndef __RMM_RCS_DATA_THREAD_CONTROLLER_H__
#define __RMM_RCS_DATA_THREAD_CONTROLLER_H__
...
#define UA_NUM (1)
#define MAX_CLIENT (4)
#define UA_CHANNEL_HEADER_LEN      (4 * sizeof(int))/*(offsetof(Channel_Data_t, data))*/
...
typedef struct _Ril_Channel_Client {
    ...
    UaChannel_Data_t uaEvent;
    char buff[CHANNEL_BUFFER_SIZE];
} Ril_Channel_Client_t;

typedef struct _Ril_Channel_Obj {
    int                         fd;
    pthread_mutex_t             mutex;
    Ril_Channel_Client_t       *channels;
    int                         channels_size;
} Ril_Channel_Obj_t;

/*****************************************************************************
 * Class RmmRcsSharedMemory
 *****************************************************************************/
class RmmRcsSharedMemory: virtual public RefBase {

    public:
        RmmRcsSharedMemory(void);
        virtual ~RmmRcsSharedMemory();
...
/*****************************************************************************
 * Class RmmRcsDataThreadController
 *****************************************************************************/
class RmmRcsDataThreadController: virtual public RefBase {

    public:
        RmmRcsDataThreadController();
        virtual ~RmmRcsDataThreadController();
        static void init();
    ...
    private:
        ...
        static void processUaEvent(Ril_Channel_Client_t *client);
        static int sendDataToUA(UaChannel_Data_t *data, int account, int uaType);
        static int fsend(int fd, char *data, int len);
        static int getClientIndex(int account, int uaType);
        static sp<RmmRcsSharedMemory> getSharedMem(void);

        static pthread_t  sRilServerThd; //process UA data
        static pthread_t  sShareMemoryThd; //process Md data
        static sp<RmmRcsSharedMemory> sShareMemory;
        static pthread_mutex_t sMutex;
        static Ril_Channel_Obj_t *sChannelObj;
        static RmmRcsDataThreadController *sInstance;
};

#endif /* __RMM_RCS_DATA_THREAD_CONTROLLER_H__ */
