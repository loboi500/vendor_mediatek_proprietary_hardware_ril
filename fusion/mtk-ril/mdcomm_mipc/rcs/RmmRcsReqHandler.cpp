...
#define RFX_LOG_TAG "RCS RIL RMM"
...
: RfxBaseHandler(slot_id, channel_id) {
    ...
    RFX_LOG_I(RFX_LOG_TAG, "RmmRcsReqHandler create (slot_id = %d)", slot_id);
    ...
    if (slot_id == 0) {
        RmmRcsDataThreadController::init();
    }
}
...
void RmmRcsReqHandler::handleEventRcsSendMsg(const sp<RfxMclMessage>& msg) {
...
void RmmRcsReqHandler::handleEventRcsReceiveMsg(const sp<RfxMclMessage>& msg) {
    const char *pRecvMsg = (const char *)msg->getData()->getData();
    int length = msg->getData()->getDataLength();

    RFX_LOG_I(RFX_LOG_TAG, "recv data length = %d, slot = %d\n", length, m_slot_id);

    RmmRcsDataThreadController::sendMsgToShareMemory((char*)pRecvMsg, length, "handleEventRcsReceiveMsg");
    return;
}

bool RmmRcsReqHandler::isImsSubmarineSupport() {
