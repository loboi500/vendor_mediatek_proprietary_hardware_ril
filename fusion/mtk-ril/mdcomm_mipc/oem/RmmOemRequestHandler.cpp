...
void RmmOemRequestHandler::onHandleEvent(const sp<RfxMclMessage>& msg) {
    int id = msg->getId();
    logD(RFX_LOG_TAG, "onHandleRequest: %s(%d)", idToString(id), id);
    switch (id) {
...
void RmmOemRequestHandler::requestDeviceIdentity(const sp<RfxMclMessage>& msg) {
    ...
    // Query IMEI, the length os IMEI is fixed, 15
    char imei[15 + 1] = {0};
    ...
    t_result_ptr = (char*)cnfMipc->getMipcVal(MIPC_SYS_GET_INFO_CNF_T_DEVICE_ID, &t_system_len);

    if (NULL != t_result_ptr) {
        strncpy(imei, t_result_ptr, 15);
        imei[15] = '\0';
        // logE(RFX_LOG_TAG, "requestDeviceIdentity IMEI: %s", imei);
        identity.imei = imei;
    } else {
        logE(RFX_LOG_TAG, "requestDeviceIdentity IMEI Fail");
        goto error;
    }
    ...
    if (RatConfig_isC2kSupported()) {
        ...
        if (identity.meidHex != NULL) {
            ...
            // logD(RFX_LOG_TAG, "after identity.meidHex = %s", identity.meidHex);
        }
...
void RmmOemRequestHandler::requestGetImei() {
    char* t_result_ptr;
    uint16_t t_system_len = 0;
    ...
    if (cnfMipc == NULL || cnfMipc->getResult() != MIPC_RESULT_SUCCESS) {
        logE(RFX_LOG_TAG, "requestGetImei mipc error");
        return;
    }
    ...
    if (NULL != t_result_ptr) {
        strncpy(imei, t_result_ptr, 15);
        imei[15] = '\0';
        logE(RFX_LOG_TAG, "requestGetImei %s", imei);
        mImei = String8(imei);
    } else {
        logE(RFX_LOG_TAG, "requestGetImei Fail");
    }
}

void RmmOemRequestHandler::requestGetImei(const sp<RfxMclMessage>& msg) {
    sp<RfxMclMessage> responseMsg;
...
void RmmOemRequestHandler::requestGetImeisv() {
    ...
    sp<RfxMipcData> cnfMipc = callToMipcMsgSync(initMipc);
    if (cnfMipc == NULL || cnfMipc->getResult() != MIPC_RESULT_SUCCESS) {
        logE(RFX_LOG_TAG, "requestGetImeisv() mipc error");
        return;
    }

    t_result_ptr = (char *)cnfMipc->getMipcVal(MIPC_SYS_GET_INFO_CNF_T_IMEISV, &t_system_len);
    if (NULL != t_result_ptr) {
        strncpy(imeiSv, t_result_ptr, 2);
        imeiSv[2] = '\0';
        logE(RFX_LOG_TAG, "requestGetImeisv %s", imeiSv);
        mImeiSv = String8(imeiSv);
    } else {
        logE(RFX_LOG_TAG, "requestGetImeisv Fail");
    }
}

void RmmOemRequestHandler::requestGetImeisv(const sp<RfxMclMessage>& msg) {
    sp<RfxMclMessage> responseMsg;
...
void RmmOemRequestHandler::requestBasebandVersion(const sp<RfxMclMessage>& msg) {
    ...
    baseband[64]= '\0';
    logE(RFX_LOG_TAG, "requestBasebandVersion:%s", baseband);
    temp = baseband;
    len = strlen(temp);
    if (len == 0) {
        goto error;
    ...
    logE(RFX_LOG_TAG, "requestBasebandVersion last result:%s", temp);
    responseMsg = RfxMclMessage::obtainResponse(msg->getId(), RIL_E_SUCCESS,
...
void RmmOemRequestHandler::bootupGetBasebandProject() {
    char* result_ptr;
    ...
    result_ptr = (char *)cnfMipc->getMipcVal(MIPC_SYS_GET_INFO_CNF_T_SERIAL_NUMBER, &t_system_len);
    if (NULL != result_ptr) {
        strncpy(SN, result_ptr, 64);
        SN[64]= '\0';
        rfx_property_set(PROPERTY_SERIAL_NUMBER, SN);
    }

    result_ptr = (char *)cnfMipc->getMipcVal(MIPC_SYS_GET_INFO_CNF_T_PROJECT_NAME, &t_system_len);
    if (NULL == result_ptr) {
        goto error;
    }

    strncpy(proj, result_ptr, 64);
    proj[64]= '\0';

    result_ptr = (char *)cnfMipc->getMipcVal(MIPC_SYS_GET_INFO_CNF_T_FLAVOR_NAME, &t_system_len);
    if (NULL == result_ptr) {
        goto error;
    }

    strncpy(flavor, result_ptr, 64);
    flavor[64]= '\0';
...
void RmmOemRequestHandler::requestQueryThermal(const sp<RfxMclMessage>& msg) {
    sp<RfxAtResponse> pResponse;
...
void RmmOemRequestHandler::handleRawUrc(const sp<RfxMclMessage>& msg) {
