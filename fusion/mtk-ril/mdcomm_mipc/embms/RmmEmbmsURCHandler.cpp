...
RFX_IMPLEMENT_HANDLER_CLASS(RmmEmbmsURCHandler, RIL_CMD_PROXY_URC);

RmmEmbmsURCHandler::RmmEmbmsURCHandler(int slot_id, int channel_id) :
...
RmmEmbmsURCHandler::~RmmEmbmsURCHandler() {
}

bool RmmEmbmsURCHandler::isHvolteDisable() {
    bool ret = false;
...
void RmmEmbmsURCHandler::onHandleUrc(const sp<RfxMclMessage>& msg) {
...
void RmmEmbmsURCHandler::onEmbmsHvolteUpdate(const sp<RfxMclMessage>& msg) {
    // +EHVOLTE: <mode>
    // <mode>: integer.
    // 0 SRLTE mode
...
void RmmEmbmsURCHandler::onEpsNetworkUpdate(const sp<RfxMclMessage>& msg) {
...
void RmmEmbmsURCHandler::onEmbmsSessionActiveUpdate(const sp<RfxMclMessage>& msg) {
    int err;
    int status;
    char* session_id;
    char* raw_tmgi;
...
void RmmEmbmsURCHandler::onEmbmsSaiListUpdate(const sp<RfxMclMessage>& msg) {
    int num;
    int index = 0;;
    int err = 0;
    RfxAtLine *line = msg->getRawUrc();
...
void RmmEmbmsURCHandler::onEmbmsSessionListUpdate(const sp<RfxMclMessage>& msg) {
    /*
    %MBMSEV:<event>
        List of <event>:
...
void RmmEmbmsURCHandler::onEmbmsSrvStatus(const sp<RfxMclMessage>& msg) {
    int err;
    int status;
    sp<RfxMclMessage> response;
    RfxAtLine *line = msg->getRawUrc();
...
void RmmEmbmsURCHandler::onHandleEvent(const sp<RfxMclMessage>& msg) {
...
void RmmEmbmsURCHandler::handleInitialVariable(const sp<RfxMclMessage>& msg) {
    RFX_UNUSED(msg);
    memset(&oos_tmgi, 0, sizeof(RIL_EMBMS_LocalOosNotify));
    oos_tmgi_count = 0;
}
