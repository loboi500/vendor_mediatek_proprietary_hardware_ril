...
void setRadioState(RIL_RadioState newState, RIL_SOCKET_ID rid) {
    ...
    pthread_mutex_lock(&s_state_mutex[(unsigned int)rid]);
    ...
    pthread_mutex_unlock(&s_state_mutex[(unsigned int)rid]);
...
RIL_RadioState getRadioState(RIL_SOCKET_ID rid)
{
    RIL_RadioState radioState = s_radio_state[(unsigned int)rid];
