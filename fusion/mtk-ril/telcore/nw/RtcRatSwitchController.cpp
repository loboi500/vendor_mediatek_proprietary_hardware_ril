        ...
        case RIL_LOCAL_REQUEST_OEM_HOOK_ATCI_INTERNAL:
            ...
            if (dataLength > 0 && strncmp(data, "AT+ERAT=", 8) == 0) {
                ...
                if (targetSlotId == m_slot_id) {
                    err = sscanf(data, "AT+ERAT=%d", &rat);
                    if (err < 0) {
                        logE(RAT_CTRL_TAG, "%s, sscan get error!", __FUNCTION__);
                        return false;
                    }
                    switch (rat) {
                        case 0:
...
void RtcRatSwitchController::setPreferredNetworkType(RatSwitchInfo ratSwtichInfo) {
    ...
    unsigned int targetRaf = (unsigned int) defaultRaf;
...
void RtcRatSwitchController::requestSetSystemSelectionChannels(const sp<RfxMessage>& message) {
    ...
    if (ssc->specifyChannels > 0) {
        for (int i = 0; i < ssc->specifiers_length; i++) {
            if (ssc->specifiers[i].radio_access_network == EUTRAN) mIsNeedTurnOffRadio = true;
        }
    }
...
bool RtcRatSwitchController::onCheckIfRejectMessage(const sp<RfxMessage>& message,
    ...
    if (RFX_MSG_REQUEST_SET_PREFERRED_NETWORK_TYPE == id
            || RFX_MSG_REQUEST_SET_ALLOWED_NETWORK_TYPES_BITMAP == id) {
    ...
    if (RFX_MSG_REQUEST_GET_PREFERRED_NETWORK_TYPE == id
            || RFX_MSG_REQUEST_GET_ALLOWED_NETWORK_TYPES_BITMAP == id
...
int RtcRatSwitchController::getDefaultNetWorkType(int slotId) {
    ...
    // logV(RAT_CTRL_TAG, "getDefaultNetWorkType, C2k=%d, LteFdd=%d, LteTdd=%d, Nr=%d, return=%d",
...
/*
  ...
  -1: do noting
   ...
   1: disable 2G
*/
int RtcRatSwitchController::disable2GOption(int reqType) {
    int ret = -1, mode = 0;
    int reqRaf = RAF_UNKNOWN;
    int cardType;
    ...
    reqRaf = RtcCapabilitySwitchUtil::getRafFromNetworkType(reqType);
    if ((reqRaf & RAF_GSM_GROUP) > 0) {
        mode = 0;
    }
    else {
        mode = 1;
