...
void RfxChannelManager::initMuxPath() {
    ...
    struct stat st;
    if (stat(muxPathInit[RIL_CMD_IMS], &st) == -1) {
    // don't use open to check, ptty will not work after...
    // int fd_legacy = open(muxPathInit[RIL_CMD_IMS], O_RDWR);
        ...
        RFX_LOG_I(RFX_LOG_TAG, "%s TTY channel exists!", muxPathInit[RIL_CMD_IMS]);
    }
#endif
