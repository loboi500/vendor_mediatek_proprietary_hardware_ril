...
#define PROPERTY_SMS_MD_READY "vendor.sms.md.ready"
...
RtmGsmSmsController::RtmGsmSmsController() :m_timer(NULL) {
    ...
    mSmsMdReady = false;
    setMSimProperty(getSlotId(), (char *)PROPERTY_SMS_MD_READY, (char*)"0");
...
bool RtmGsmSmsController::onHandleUrc(const sp<RfxMessage>& msg) {
    ...
    switch (msg_id) {
        ...
        case RFX_MSG_URC_SMS_READY_NOTIFICATION: {
                mSmsMdReady = true;
                setMSimProperty(getSlotId(), (char *)PROPERTY_SMS_MD_READY, (char*)"1");
                if (mSmsMdReadyTimerHandle != NULL) {
    ...
    if (msg_id ==  RFX_MSG_URC_RESPONSE_NEW_SMS ||
        ...
        m_timer = RfxTimer::start(RfxCallback0(this, &RtmGsmSmsController::onTimer),
...
void RtmGsmSmsController::onHidlStateChanged(RfxStatusKeyEnum key,
    ...
    logD(mTag, "onHidlStateChanged (%s, %s, %s) (slot %d)", boolToString(oldState),
            boolToString(newState), boolToString(smsFwkReady), getSlotId());
    ...
    } else if (newState && !smsFwkReady && !isUnderCryptKeeper()) {
