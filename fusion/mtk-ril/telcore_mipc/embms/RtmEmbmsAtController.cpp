...
#include "RtmEmbmsAtController.h"
#include "RtmEmbmsUtils.h"
...
RFX_IMPLEMENT_CLASS("RtmEmbmsAtController", RtmEmbmsAtController, RfxController);

RtmEmbmsAtController::RtmEmbmsAtController() :
...
void RtmEmbmsAtController::onInit() {
    RfxController::onInit();  // Required: invoke super class implementation

    logI(RFX_LOG_TAG, "onInit:mSdkVersion:%d", mSdkVersion);
    mSessionInfoList = new Vector<RtmEmbmsSessionInfo*>();
...
void RtmEmbmsAtController::onDeinit() {
    logI(RFX_LOG_TAG, "onDeinit");
    RtmEmbmsUtils::freeSessionList(mSessionInfoList);
    delete mSessionInfoList;
    mSessionInfoList = NULL;
    RfxController::onDeinit();
}

bool RtmEmbmsAtController::onHandleRequest(const sp<RfxMessage>& message) {
...
bool RtmEmbmsAtController::onHandleResponse(const sp<RfxMessage>& message) {
...
bool RtmEmbmsAtController::onHandleUrc(const sp<RfxMessage>& message) {
    logI(RFX_LOG_TAG, "Handle URC %s Slot %d (DefaultDataSlot:%d)"
        , idToString(message->getId()),
        message->getSlotId(),
        RtmEmbmsUtils::getDefaultDataSlotId());

    if (getSlotId() != RtmEmbmsUtils::getDefaultDataSlotId()) {
...
bool RtmEmbmsAtController::onCheckIfRejectMessage(
...
int RtmEmbmsAtController::strStartsWith(const char *line, const char *prefix) {
    return RfxMisc::strStartsWith(line, prefix);
}

void RtmEmbmsAtController::handleAtRequest(const sp<RfxMessage>& request) {
    int need_forward = 0;
    ...
    // --- Special handle area ---
        ...
        if (RtmEmbmsUtils::isAtCmdEnableSupport()) {
            logV(RFX_LOG_TAG, "Pass to rild");
            need_forward = 1;
        } else {
...
void RtmEmbmsAtController::responseErrorString(const sp<RfxMessage>& msg) {
    String8 responseStr(EMBMS_ERROR);
    responseToSocket(msg, responseStr.string());
}

void RtmEmbmsAtController::urcToSocket(const sp<RfxMessage>& msg,
...
void RtmEmbmsAtController::responseToSocket(const sp<RfxMessage>& msg,
...
void RtmEmbmsAtController::handleAtResponse(const sp<RfxMessage>& response) {
...
void RtmEmbmsAtController::handleRequestDefault(const sp<RfxMessage>& request) {
...
void RtmEmbmsAtController::requestAtEnable(const sp<RfxMessage>& request) {
...
void RtmEmbmsAtController::handleEnableResponse(const sp<RfxMessage>& response) {
    ...
    if (response->getError() == RIL_E_SUCCESS && entry->response == EMBMS_GENERAL_SUCCESS) {
        ...
        if (RtmEmbmsUtils::isRjilSupport()) {
...
void RtmEmbmsAtController::handleDisableResponse(const sp<RfxMessage>& response) {
...
void RtmEmbmsAtController::handleStartSessionResponse(const sp<RfxMessage>& response) {
    ...
    if (response->getError() == RIL_E_SUCCESS) {
        ...
        if (entry->response != EMBMS_GENERAL_SUCCESS) {
            int index;
            RtmEmbmsSessionInfo* sessionInfo =
                RtmEmbmsUtils::findSessionByTransId(mSessionInfoList, entry->trans_id, &index);
            if (sessionInfo != NULL) {
...
void RtmEmbmsAtController::handleStartSessionUrc(const sp<RfxMessage>& response) {
    RIL_EMBMS_LocalStartSessionResp* entry = NULL;
    int index;
    entry = (RIL_EMBMS_LocalStartSessionResp *) response->getData()->getData();
    ...
    if (entry->tmgi_info_valid == 1) {
        ...
        RtmEmbmsSessionInfo* sessionInfo = RtmEmbmsUtils::findSessionByTmgi(
...
void RtmEmbmsAtController::handleStopSessionResponse(const sp<RfxMessage>& response) {
    RIL_EMBMS_LocalStartSessionResp* entry = NULL;
    int id = response->getId();
    char *responseStr = NULL;
...
void RtmEmbmsAtController::requestAtSetAvailbility(const sp<RfxMessage>& request) {
...
void RtmEmbmsAtController::requestAtActivate(const sp<RfxMessage>& request
...
void RtmEmbmsAtController::requestAtGetCoverage(const sp<RfxMessage>& request) {
    // AT%MBMSEV?
    // No parameter parse.
...
void RtmEmbmsAtController::handleGetCoverageResponse(const sp<RfxMessage>& response) {
...
void RtmEmbmsAtController::requestAtGetNetworkTime(const sp<RfxMessage>& request) {
...
void RtmEmbmsAtController::handleGetNetworkTimeResponse(const sp<RfxMessage>& response) {
...
void RtmEmbmsAtController::handleTriggerCellInfoNotifyResponse(const sp<RfxMessage>& response) {
    int id = response->getId();
    // Just ignore, nothing to do
    logI(RFX_LOG_TAG, "handleTriggerCellInfoNotifyResponse, id:%d", id);
}

void RtmEmbmsAtController::handleAtUrc(const sp<RfxMessage>& response) {
    char *responseStr = NULL;
...
void RtmEmbmsAtController::handleCellInfoUrc(const sp<RfxMessage>& response) {
    int32_t cell_id;
    int32_t status;
    int32_t plmn;
    ...
    status = data[1];
    plmn = data[2];
    cell_id_mask = RtmEmbmsUtils::addLogMask(cell_id);
...
void RtmEmbmsAtController::handleCoverageUrc(const sp<RfxMessage>& response) {
    int32_t srv_status;
    int     result_status;
...
void RtmEmbmsAtController::handleSessionListUrc(const sp<RfxMessage>& response) {
    ...
    //
    if (RtmEmbmsUtils::isRjilSupport()) { //version 1.1.1x~1.1.20c
...
void RtmEmbmsAtController::handleActiveSessionStatus(const sp<RfxMessage>& response) {
...
void RtmEmbmsAtController::handleOosUrc(const sp<RfxMessage>& response) {
...
void RtmEmbmsAtController::handleSaiUrc(const sp<RfxMessage>& response) {
...
int RtmEmbmsAtController::getCoverageState(int srv_status) {
    /*
...
bool RtmEmbmsAtController::isEmbmsSupported() {
...
void RtmEmbmsAtController::onMainCapabilityChanged(RfxStatusKeyEnum key,
...
void RtmEmbmsAtController::onDefaultDataChanged(RfxStatusKeyEnum key,
    RfxVariant old_value, RfxVariant new_value) {
