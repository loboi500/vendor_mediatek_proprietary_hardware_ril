...
void RtmImsController::onInit() {
    ...
    getStatusManager()->registerStatusChanged(RFX_STATUS_KEY_RADIO_STATE,
        RfxStatusChangeCallback(this, &RtmImsController::onRadioStateChanged));
...
void RtmImsController::handleXui(const sp<RfxMessage>& message) {
    ...
    std::vector<std::string> uris = RtmImsConferenceHandler::splitString(xui, ",");
    Vector<String8> xuis;
    for(int i = 0; i < (int)uris.size(); i++) {
        std::string uri = RtmImsConferenceHandler::getUserNameFromSipTelUriString(uris[i]);
        if (!uri.empty()) {
...
void RtmImsController::onRadioStateChanged(RfxStatusKeyEnum key,
...
void RtmImsController::handleImsDisableDoneUrc(const sp<RfxMessage>& message) {
    RFX_UNUSED(message);
    mIsImsDisabling = false;
