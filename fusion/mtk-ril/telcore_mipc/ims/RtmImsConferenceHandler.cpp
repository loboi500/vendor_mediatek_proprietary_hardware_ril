...
#include "RtmImsConferenceHandler.h"

#define RFX_LOG_TAG "RtmImsConference"

const string RtmImsConferenceHandler::TAG_NEXT_LINE("<ascii_10>");
const string RtmImsConferenceHandler::TAG_RETURN("<ascii_13>");
const string RtmImsConferenceHandler::TAG_DOUBLE_QUOTE("<ascii_34>");

#define ANONYMOUS_URI  "sip:anonymous@anonymous.invalid"

RtmImsConferenceHandler::RtmImsConferenceHandler(int slot) :
...
{
    m_slot_id = slot;
    RFX_LOG_D(RFX_LOG_TAG, "RtmImsConferenceHandler()");
}

RtmImsConferenceHandler::~RtmImsConferenceHandler() {
    RFX_LOG_D(RFX_LOG_TAG, "~RtmImsConferenceHandler()");
}

void RtmImsConferenceHandler::startConference() {
...
void RtmImsConferenceHandler::closeConference() {
...
void RtmImsConferenceHandler::firstMerge(string callId_1, string callId_2, string num_1, string num_2) {
...
void RtmImsConferenceHandler::tryAddParticipant(string addr) {
...
void RtmImsConferenceHandler::tryOneKeyAddParticipant(string addr) {
    string number = getUserNameFromSipTelUriString(addr);
    number = encodeSpecialChars(normalizeNumberFromCLIR(number));
...
void RtmImsConferenceHandler::tryRemoveParticipant(string addr) {
    string number = getUserNameFromSipTelUriString(addr);
    number = normalizeNumberFromCLIR(number);
    mRemovingParticipant = number;
}

bool RtmImsConferenceHandler::modifyParticipantComplete() {
...
void RtmImsConferenceHandler::modifyParticipantFailed() {
...
void RtmImsConferenceHandler::handleOneKeyInviteNotifyResult(const sp<RfxMessage>& message) {
...
string RtmImsConferenceHandler::getUserNameFromSipTelUriString(string uriString) {
    RFX_LOG_D(RFX_LOG_TAG,
...
RtmImsConferenceCallMessageHandler* RtmImsConferenceHandler::parseXmlPackage(string data) {
    RtmImsConferenceCallMessageHandler* parsedData = new RtmImsConferenceCallMessageHandler();
...
void RtmImsConferenceHandler::updateConferenceStateWithLocalCache() {
    ...
    for (int i = 0; i < count; i++) {
        ...
        user->mEndPoint = addr;
        user->mStatus = RtmImsConferenceCallMessageHandler::STATUS_CONNECTED;
        user->mEntity = addr;
    ...
    RtmImsConferenceController *ctrl =
            (RtmImsConferenceController *)root->findController(m_slot_id,
...
void RtmImsConferenceHandler::setupHost(RtmImsConferenceCallMessageHandler* xmlData){
...
bool RtmImsConferenceHandler::isSelfAddress(string address) {
...
bool RtmImsConferenceHandler::updateParticipants(sp<ConferenceCallUser> oldParticipant, string to) {
    bool updated = false;
...
void RtmImsConferenceHandler::insertParticipantsAddrMap(string key, string value) {
...
void RtmImsConferenceHandler::showCacheAndXmlData(string callerName) {
...
void RtmImsConferenceHandler::restoreParticipantsAddressByLocalCache() {
    RFX_LOG_D(
            RFX_LOG_TAG,
    ...
    for (int i = 0; i < (int)XmlParticipants.size(); i++) {
        ...
        bool isSelf = isSelfAddress(userHandle);

        if (participant->mStatus == RtmImsConferenceCallMessageHandler::STATUS_DISCONNECTED
                || isSelf) {
...
void RtmImsConferenceHandler::restoreUnknowParticipants(vector<string> LocalUnMatchParticipants) {
    //copy mConfParticipants
...
void RtmImsConferenceHandler::fullUpdateParticipants(vector<sp<ConferenceCallUser>> users) {
...
void RtmImsConferenceHandler::partialUpdateParticipants(vector<sp<ConferenceCallUser>> users) {
    ...
    for (int i = 0; i < (int)users.size(); i++) {
        ...
        if (originalUserAddr.empty() && matchedIndex < 0) {
            ...
            if (status == RtmImsConferenceCallMessageHandler::STATUS_CONNECTED) {
            ...
            //         Remove an unknown participant.
                    ...
                    RtmImsConferenceCallMessageHandler::STATUS_DISCONNECTED) {
        ...
        if (!(status == RtmImsConferenceCallMessageHandler::STATUS_DIALING_OUT)) {
            if (matchedIndex >= 0) {
...
void RtmImsConferenceHandler::tryOneKeyAddLocalList(vector<string> users) {
    for (int i = 0; i < (int)users.size(); i++) {
        tryOneKeyAddParticipant(users[i]);
    }
}

void RtmImsConferenceHandler::updateLocalCache(int cepState) {
    int disconnectedCount = 0;
    int connectedCount = 0;
    for (int i = 0; i < (int)mConfParticipants.size(); i++) {
        if (RtmImsConferenceCallMessageHandler::STATUS_DISCONNECTED.compare(
    ...
    if (disconnectedCount && localParticipantsSize > connectedCount) {
        ...
        for (int i = 0; i < (int)mConfParticipants.size(); i++) {
            if (RtmImsConferenceCallMessageHandler::STATUS_DISCONNECTED.compare(
                ...
                // new disconnected.
                if (cepState == RtmImsConferenceCallMessageHandler::CEP_STATE_FULL) {
...
void RtmImsConferenceHandler::notifyConfStateUpdate() {
...
vector<sp<ConferenceCallUser>> RtmImsConferenceHandler::getConfParticipantsInfo() {
    return mParticipants;
}

string RtmImsConferenceHandler::getConfParticipantUri(string addr, bool isRetry) {
...
bool RtmImsConferenceHandler::isEmptyConference() {
    ...
    for (int i = 0; i < (int)mConfParticipants.size(); i++) {
        ...
        if (status != RtmImsConferenceCallMessageHandler::STATUS_DISCONNECTED) {
 ...
 */
void RtmImsConferenceHandler::handleImsConfCallMessage(const sp<RfxMessage>& message) {
    ...
    RtmImsConferenceCallMessageHandler* xmlData = parseXmlPackage(mCepData);
    if (xmlData == NULL) {
    ...
    isPartialCEP = (cepState == RtmImsConferenceCallMessageHandler::CEP_STATE_PARTIAL);
    ...
    switch (cepState) {
        case RtmImsConferenceCallMessageHandler::CEP_STATE_FULL:
            fullUpdateParticipants(users);
            break;
        case RtmImsConferenceCallMessageHandler::CEP_STATE_PARTIAL:
...
bool RtmImsConferenceHandler::checkCarrierConfig(const RfxStatusKeyEnum key) {
    RfxRootController* root = RFX_OBJ_GET_INSTANCE(RfxRootController);
...
bool RtmImsConferenceHandler::isContainParticipant(vector<string> participants, string participant) {
...
void RtmImsConferenceHandler::removeParticipant(vector<string>& participants, string participant,
        bool compareLoosely) {
    for (int i = 0; i < (int)participants.size(); i++) {
...
string RtmImsConferenceHandler::getPairedAddress(const string &addr) {
...
string RtmImsConferenceHandler::concatData(int isFirst, string origData, string appendData) {
    if (isFirst) {
        return appendData;
    }
...
vector<string> RtmImsConferenceHandler::splitString(string str, string c) {
...
string RtmImsConferenceHandler::recoverDataFromAsciiTag(string data) {
...
string RtmImsConferenceHandler::normalizeNumberFromCLIR(string number) {
    number = replaceAll(number, "*31#", "");
    number = replaceAll(number, "#31#", "");
    return number;
}

string RtmImsConferenceHandler::replaceAll(string &str,
...
string RtmImsConferenceHandler::getPairedRestoredAddress(string userAddr) {
...
void RtmImsConferenceHandler::dumpParticipantsAddrMap() {
 ...
 */
string RtmImsConferenceHandler::encodeSpecialChars(string number) {
...
void RtmImsConferenceHandler::addFirstMergeParticipant(string callId) {
