...
void RmcDcImsDataChannel2ReqHandler::handleImsBearerConfirm(const sp<RfxMclMessage>& msg) {
    ...
    if (imsPdnState == RFX_ONGOING_IMS_PDN_ACT_DONE
        ...
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] since ims activated: %d,no need to send confirm again",
    ...
    if (eImsPdnState == RFX_ONGOING_EIMS_PDN_ACT_DONE
        ...
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] since eims activated: %d,no need to send confirm again",
    ...
    if (action == 1) {
        ...
        if (aid == tempImsAid) {
            ...
            RFX_LOG_D(RFX_LOG_TAG, "[%d][%s], no match aid=%d", rid, __FUNCTION__, aid);
        }
    }
    if (action == 0) {
            ...
            if (action == ACTION_IMS_BEARER_ACTIVATION) {
                ...
                if (err >= 0) {
                    ...
                    if (strcmp(currProp,"") == 0) { //first property
                        ...
                        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] current vendor.ims.eims.pdn.info= %s", rid, __FUNCTION__, currProp);
                    ...
                    rfx_property_set(IMS_EIMS_PDN_INFO,currProp);
                    RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] updated vendor.ims.eims.pdn.info= %s", rid, __FUNCTION__, currProp);
                } else {
...
void RmcDcImsDataChannel2ReqHandler::notifyUnsyncImsBearerRequest() {
    ...
    if (imsState == IMS_OR_EIMS_PDN_ACTIVATED) {
        aid = getMclStatusManager()->getIntValue(RFX_STATUS_KEY_IMS_PDN_ACT_AID, -1);
        RFX_LOG_D(RFX_LOG_TAG, "[%d][%s] aid=%d, imsPdnState=%d", m_slot_id, __FUNCTION__, aid,imsPdnState);
        if (imsPdnState == RFX_ONGOING_IMS_PDN_ACT || imsPdnState == RFX_ONGOING_IMS_PDN_ACT_DONE) {
            notification->aid = aid;
        ...
        RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] no ongoing ACTIVATED or DEACTIVATED event",
                m_slot_id, __FUNCTION__);
    }
}
...
void RmcDcImsDataChannel2ReqHandler::handleResetImsConnection(const sp<RfxMclMessage>& msg) {
    ...
    if (eimsConnState == IMS_OR_EIMS_PDN_ACTIVATED) {
        ...
        if (eimsAid != RFX_NO_ONGOING_IMS_PDN_AID && eImsPdnState == RFX_ONGOING_EIMS_PDN_ACT_DONE) {
            ...
            RFX_LOG_I(RFX_LOG_TAG, "[%d][%s] no ongoing EIMS PDN ACT aid",
                    m_slot_id, __FUNCTION__);
        }
    }
