...
void RmcOemRequestHandler::requestSetHsrMode(const sp<RfxMclMessage>& msg) {
    ...
    RFX_LOG_I(RFX_LOG_TAG, "requestSetHsrMode: %d, %s", mode, hsrMode);
    ...
    */
    if (strcmp(hsrMode, "0") != 0 && strcmp(hsrMode, "2") != 0 && strcmp(hsrMode, "3") != 0) {
        RFX_LOG_E(RFX_LOG_TAG, "requestSetHsrMode ERROR hsrMode: %s", hsrMode);
    ...
    if (mode == 0) {
        if (strcmp(hsrMode, "0") == 0 || strcmp(hsrMode, "2") == 0) {
            ...
            if (pResponse->isATCmdRspErr()) {
                ...
                RFX_LOG_E(RFX_LOG_TAG, "requestSetHsrMode 343 ERROR: %d,%d", cmdError, mode);
                errNo = RIL_E_GENERIC_FAILURE;
            }
        }
        ...
        if (strcmp(hsrMode, "0") == 0 || strcmp(hsrMode, "3") == 0) {
            ...
            if (pResponse->isATCmdRspErr()) {
                cmdError = (int) pResponse->atGetCmeError();
                RFX_LOG_E(RFX_LOG_TAG, "requestSetHsrMode 450 ERROR: %d,%d", cmdError, mode);
                errNo = RIL_E_GENERIC_FAILURE;
            }
            ...
            if (pResponse->isATCmdRspErr()) {
                cmdError = (int) pResponse->atGetCmeError();
                RFX_LOG_E(RFX_LOG_TAG, "requestSetHsrMode 470 ERROR: %d,%d", cmdError, mode);
                errNo = RIL_E_GENERIC_FAILURE;
            }
        }
    } else if (mode == 1) {
